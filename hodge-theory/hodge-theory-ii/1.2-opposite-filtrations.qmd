...
::: {.rmenv #env-ii-1.2.1 title="1.2.1" latex="{1.2.1}"}
Let $A$ be an object of $\mathscr{A}$ endowed with filtrations $F$ and $G$.
By definition, $\Gr_F^n(A)$ is a quotient of a sub-object of $A$ and, as such, is endowed with a filtration induced by $G$ @env-ii.1.1.10.
Passing to the associated graded defines a bigraded object $(\Gr_G^n\Gr_F^m(A))_{n,m\in\ZZ}$.
By a lemma of Zassenhaus, $\Gr_G^n\Gr_F^m(A)$ and $\Gr_F^m\Gr_G^n(A)$ are canonically isomorphic: if we define the induced filtrations @env-ii.1.1.10 as quotient filtrations of the induced filtrations on a sub-object, then we have
$$
  \begin{aligned}
    \Gr_G^n\Gr_F^m(A)
...
::: {.rmenv #env-ii-1.2.2 title="1.2.2" latex="{1.2.2}"}
...
We note that these filtrations do not in general correspond to one another under the isomorphism @ii-1.2.1.
In the expression $\Gr_H\Gr_G\Gr_F(A)$, $G$ and $H$ thus play a symmetric role, but not $F$ and $G$.
:::

::: {.rmenv #env-ii-1.2.3 title="Definition 1.2.3" latex="{Definition 1.2.3}"}
Two **finite** filtrations $F$ and $\overline{F}$ on $A$ are said to be **$n$-opposite** if $\Gr_F^p\Gr_{\overline{F}}^q(A)=0$ for $p+q\neq n$.
:::

::: {.rmenv #env-ii-1.2.4 title="1.2.4" latex="{1.2.4}"}
...
::: {.itenv #env-ii-1.2.5 title="Proposition 1.2.5" latex="{Proposition 1.2.5}"}
...
ii. If $F$ and $\overline{F}$ are $n$-opposite, and if we set
    $$
      ...
      \end{cases}
    $$
    then $A$ is the direct sum of the $A^{p,q}$, and $F$ and $\overline{F}$ come from the bigrading $A^{p,q}$ of $A$ by the procedure of @env-ii-1.2.4.
:::
...
i. The condition $\Gr_F^p\Gr_{\overline{F}}^q(A)=0$ for $p+q>n$ implies that $F^p\cap\overline{F}^q=(F^{p+1}\cap\overline{F}^q)+(F^p\cap\overline{F}^{q+1})$ for $p+q>n$.
    ...
    Dually (@env-ii-1.1.6, @env-ii-1.1.7, @env-ii-1.1.10), the condition $\Gr_F^p\Gr_{\overline{F}}^q(A)=0$ for $p+q<n$ is equivalent to the condition $A=F^p(A)+\overline{F}^q(A)} for $(1-p)+(1-q)>-n$, i.e. for $p+q\leqslant n+1$, and the claim then follows.
...
::: {.rmenv #env-ii-1.2.6 title="1.2.6" latex="{1.2.6}"}
The constructions @env-ii-1.2.4 and @env-ii-1.2.5 establish equivalences of categories that are quasi-inverse to one another between objects of $\mathscr{A}$ endowed with two finite $n$-opposite filtrations and bigraded objects of $\mathscr{A}$ of the type considered in @env-ii-1.2.4.
:::

::: {.rmenv #env-ii-1.2.7 title="1.2.7" latex="{1.2.7}"}
\oldpage{11}
...
$$
whence decompositions @env-ii-1.2.4, @env-ii-1.2.5
$$
  W^n(A)/W^{n+1}(A)
...
::: {.itenv #env-ii-1.2.8 title="Lemma 1.2.8" latex="{Lemma 1.2.8}"}
...
::: {.itenv #env-ii-1.2.9 title="Lemma 1.2.9" latex="{Lemma 1.2.9}"}
...
This finishes the proof of @env-ii-1.2.8, noting that @env-ii-1.2.8 is equivalent to ($*_k$) for large $k$.

::: {.itenv #env-ii-1.2.10 title="Theorem 1.2.10" latex="{Theorem 1.2.10}"}
...
\end{aligned}
$$
and, with the notation of @env-ii-1.2.8, set
$$
A_i^{p,q}
= A_{\sigma_i(p,q)}
\qquad\text{for } i=0,1.
...
::: {.itenv #env-ii-1.2.11 title="Lemma 1.2.11" latex="{Lemma 1.2.11}"}
...
Set $A_0=\bigoplus A_0^{p,q}$ and define filtrations $W$ and $F$ on $A_0$ by the equations of @env-ii-1.2.11.
The canonical map $i$ from $A_0$ to $A$ is compatible with the filtrations $W$ and $F$.
Furthermore, by @env-ii-1.2.8, $\Gr_W(i)$ is an isomorphism, and induces isomorphisms of graded objects
$$
  \sum_{p+q=n} A_0^{p,q}
  \simto \Gr_W^{-n}(A)
  = \sum_{p+q=n} A^{p,q}.
...
::: {.rmenv #env-ii-1.2.12 title="1.2.12" latex="{1.2.12}"}
We now prove @env-ii-1.2.10.
Let $f\colon A\to B$ in $\mathscr{A}'$ and endow $K=\Ker(f)$ with the filtrations induced by those of $A$.
By @ii-1.2.11, $\Gr_W(K)\hookrightarrow\Gr_W(A)$;
...
The exactness of the other functors in (iv) follows immediately from (ii), (iii), and (i) or (ii) of @env-ii.1.1.11.
:::

::: {.rmenv #env-ii-1.2.13 title="1.2.13" latex="{1.2.13}"}
...
The construction @env-ii-1.1.3 associates to $W_\bullet$ a decreasing filtration $W^\bullet$.
...
@env-ii-1.2.10 translates trivially to this variation.
:::
