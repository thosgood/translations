...
\oldpage{143}
  ...
  \item
    $\cal{V}$ is the category of all non-singular algebraic spaces.
  ...
  \item
    The base field $k$ is the field of complex numbers, $\cal{V}$ is the category of non-singular algebraic spaces, and $A(X)=\HH^\bullet(X,\mathbb{Z})$ (with $X$ being endowed with its ``usual'' topology).
    The definition of \hyperref[datab]{b.} (either by Poincar\'{e} duality on divisor classes, or as an obstruction class in the classical exact sequence $0\to\mathbb{Z}\to\scr{O}_X\to\scr{O}_X^\times\to0$ of sheaves on $X$, endowed with its usual topology) is well known.
...
\begin{thebibliography}{4}
  ...
  \newblock {\em S\'{e}minaire Chevalley}, {\bf 1} (1956--58).
