...
\begin{proof}
  ...
  \[
    \sh{A}_\mathfrak{p}
    = \varinjlim_{U_f\ni\mathfrak{p}} \sh{A}^*(U_f)
    = \varinjlim_{f\not\in\mathfrak{p}} A_f
    = A_\mathfrak{p};
  \]
  the second claim of (a) can be proven in the same way.

  We now show that the map $M_f\to\sh{M}(U_f)$ is \emph{injective}:
  Since $U_f=V(A_f)$, we can always assume that $f=1$ and $U_f=V(A)$.
  The kernel of the map $M\to\sh{M}(V)$ then consists of the $m$ such that $m\otimes_A1_{A_\mathfrak{p}}=0$ for every prime ideal $\mathfrak{p}$, i.e. the $m$ whose annihilator is not contained in any prime ideal:
  the kernel is thus zero.

  Similarly, the map $M_f\to\sh{M}(U_f)$ is \emph{surjective}:
  it suffices to prove this in the case where $f=1$ and $U_f=V$.
  By the above, $\sh{M}(V)=\Gamma(\sh{M})$ is the union of the $\HH^0(\mathfrak{U},\sh{M}^*)$ where $\mathfrak{U}$ runs over the finite covers of $V$ by special open subsets;
  it thus suffices to prove that the map $M\to\HH^0(\mathfrak{U},\sh{M})$ is surjective for every finite cover by special open subsets.
...
\begin{corollary}{1}
\label{corollary1}
  If $S$ is a multiplicatively-stable subset of $A$, then the restriction of the sheaf $\sh{M}$ to $E_S$ can be identified with the sheaf $\sh{M}_S$ on $V(A_S)$ associated to $M_S$.
\end{corollary}
