...
$M_\AAf$ is endowed with an action of $\Gal(\cQQ/\QQ)$ which respects $W$.
:::
...
There exists a finite set $S$ of prime numbers such that, for each $\ell$, the representation $M_\ell$ of $\Gal(\cQQ/\QQ)$ is unramified outsied of $S$ and $\ell$.
:::
...
Let $c\in\Gal(\cQQ/\QQ)$ be complex conjugation.
...
ii. The data of $M_\AAf$, $\comp_{\AAf,\BB}$, and the Galois action, all together, are equivalent to the data of a $\QQ_\ell$-vector space $M_\ell$ for all $\ell$, along with a Galois action on $M_\ell$ and comparison isomorphisms $\comp_{\ell,\BB}\colon M_\BB\otimes\QQ\simto M_\ell$.
    ...
    By [(M6)](#M6) and [(M9)](#M9), the filtration of $M_\BB\otimes\QQ_\ell$ (resp. $M_\BB\otimes\AAf$) induced by $W$ must be stable under $\Gal(\cQQ/\QQ)$.
...
$M_\ell=\HH^i(X\otimes\cQQ,\QQ_\ell)$ is the $\ell$-adic étale cohomology of the scheme over $\cQQ$ induced from $X$ by extension of scalars, and the action of $\Gal(\cQQ,\QQ)$ is given by structure transport.
_Notation:_ $X\otimes\cQQ$, cf. [(0.4)](#chunk-0.4).
...
From [(M'3)](#Mp3), we deduce the data of [(M3)](#M3) and [(M9)](#M9) by setting $M_\AAf\coloneqq(M_\AAf)_\AAf(\cQQ)$.
...
$$
  ...
  \\\comp_{\AAf,\BB}&\colon M_\BB(C)\otimes\AAf \simto M_\AAf(\cQQ_C)
  \\\comp_{\crisp,\DR}&\colon M_\DR\otimes\QQ_p \simto M_\crisp
  \end{aligned}
$$
that are functorial in $C$, where $\cQQ_C$ is the algebraic closure of $\QQ$ in $C$.
...
In [(M'3)](#Mp3), instead of giving the $M_\AAf$, we could have given only the $\QQ_\ell$-sheaves $M_\ell$, replacing $\comp_{\AAf,\BB}$ by the $\comp_{\ell,\BB}\colon M_B(C)\otimes\QQ_\ell\simto M_\ell(\cQQ_C)$ and requiring the existence of an integer lattice $L\subset M_B(\CC)$ such that the $\comp_{\ell,\BB}(L\otimes\ZZ_\ell)$ be stable under $\Gal(\cQQ/\QQ)$.
...
::: {.rmenv #chunk-1.5 title="1.15 Variant" latex="{1.15 Variant}"}
...
We have that $A_\BB=\QQ^{\Hom(E,\CC)}$, with $F_\intfy$ induced by the complex conjugation of $\CC$.
...
Since every embedding of $E$ into $\CC$ factors through $\cQQ$, $\Gal(\cQQ/\QQ)$ acts on $\Hom(E,\CC)$,
...
::: {.trmenv #M17E title="(M1,7)<sub>E</sub>" latex="{(M1,7)\textsubscript{E}}"}
...
::: {.trmenv #Mp1E title="(M'1)<sub>E</sub>" latex="{(M'1)\textsubscript{E}}"}
For each embedding of $E$ into an algebraic closure $C$ of $\RR$, a vector space $M_\sigma$ over $\QQ$, functorial in $C$.
:::

::: {.trmenv #Mp2E title="(M'2)<sub>E</sub>" latex="{(M'2)\textsubscript{E}}"}
An $E$-vector space $M_\DR$.
:::

::: {.trmenv #Mp8E title="(M'8)<sub>E</sub>" latex="{(M'8)\textsubscript{E}}"}
...
::: {.trmenv #Mp3E title="(M'3)<sub>E</sub>" latex="{(M'3)\textsubscript{E}}"}
(cf. [(M3)](#M3), [(M9)](#M9)) An $\AAf$-sheaf $M_\AAf$ on $\Spec(E)$ (cf. [(1.14)](#chunk-1.14)).
:::

We define $N_\AAf$ as its direct image over $\Spec(\QQ)$: $N_\AAf(\cQQ)$ is the sum over the $\sigma\colon E\to\cQQ$ of the $M_AAf(\cQQ)$.
...
::: {.trmenv #Mp5E title="(M'5)<sub>E</sub>" latex="{(M'5)\textsubscript{E}}"}
Comparison isomorphisms
$$
  \begin{aligned}
...
::: {.trmenv #Mp6E title="(M'6)<sub>E</sub>" latex="{(M'6)\textsubscript{E}}"}
$W$ is a filtration of the objects [(M'1)<sub>E</sub>](#Mp1E), [(M'2)<sub>E</sub>](#Mp2E), [(M'3)<sub>E</sub>](#Mp3E), and [(M'4)<sub>E</sub>](#Mp4E), respected by the comparison isomorphisms.
:::
