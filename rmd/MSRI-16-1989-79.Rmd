...
\providecommand{\vp}{\operatorname{vp}}
...
\oldpage{3 (81)}The present article owes much to A. Grothendieck.
...
\oldpage{4 (82)}This close relation with cohomology hints that the study of nilpotent versions of $\hat{\pi}_1$ is far from the "anabelian" dream of A. Grothendieck.
...
\oldpage{5 (83)}I have also been helped by the answer by O. Gabber to my question "How can we construct an extension of $\ZZ_\ell$ by $\ZZ_\ell(3)$, uniformly in $\ell$?": "By a class in $K_5(\QQ)$", as well as by the conjectures of A. Beilinson on the values of $L$-functions.
...
\oldpage{6 (84)}The crystalline aspect will be neglected in the rest of the introduction.
...
_Terminology:_ $M$-torsor, or torsor for $M$.
_Example:_ the Kummer $\ZZ(1)$-torsor, where $\ZZ(1)$ is the Tate motive.

In [§3](#section-3) we describe certain remarkable torsors, which can be said to be cyclotomic, for the Tate motive $\ZZ(k)$.
...
\oldpage{7 (85)}Our solution will be to define a motivic linear group as being an Ind-object in the category of motives, endowed with the structure of a commutative Hopf algebra.
...
\oldpage{8 (86)}They allow us, in the definition of the motivic $\pi_1$ of $X$, to take a base point "at infinity", like the tangent vector $v$ at $s$.
...
A _$G$-torsor_, or _torsor for $G$_, is a sheaf $P$ endowed with a right $G$-action such that $P$ is locally isomorphic to $G$ acting on itself by translations on the right.
We also call such an object a _right $G$-principal homogeneous space_, or a _right principal homogeneous space for $G$_.
\oldpage{10 (88)}If $P$ is a $G$-torsor, then a sheaf $X$ on which $G$ acts can be _twisted_ by $P$.
...
An _$(H,G)$-bitorsor_ (cf. SGA 7, VII.1, or Girard, _Cohomologie non abelienne_, III 1.5) is a space which is simultaneously a left principal homogeneous space for $H$ and a right principal homogeneous space for $G$, with the $G$- and $H$-actions commuting with one another.
...
- **$\ell$-adic cohomology.**
  ...
  \oldpage{12 (90)}This follows by passing to the limit in the base change theorem for a smooth morphism [SGA 4, XVI, 1.2]: $k'$ is the filtrant inductive limit of the $k$-algebras $A$ with $\Spec(A)$ smooth over $k$.
...
\tag{1.1.5}
$$
\oldpage{13 (91)}giving rise to commutative diagrams
$$
  \begin{CD}
...
\oldpage{14 (92)}In each of the known theories, the $\HH^i(X)$ are endowed with an increasing filtration $W$, known as the _weight filtration_ [@D5], as well as comparison isomorphisms such that (1.1.1) and (1.1.2) are compatible with $W$.
...
\oldpage{15 (93)}"_almost every_ prime number" means "all, except for a finite number".
...
$$
\oldpage{16 (94)}In other words, $c_\DR$ respects $M_\BB\subset M_\BB\otimes\CC=M_\DR\otimes\CC$, and $c_\DR|M_\BB=F_\infty$.
:::
...
\oldpage{17 (95)}In this section, we will partially describe the system [(M1)--(M10)](#chunk-1.4) of realisations of $M$ in the case where $X$ is separated and smooth over $\QQ$.
...
The comparison isomorphism $\comp_{\DR,\BB}$ is (1.1.3), and the comparison isomorphism $\comp_{\ell,\BB}$ is (1.1.4).
...
- **Semi-stable case.**
  ...
  \oldpage{18 (96)}We complete it to a projective line $\overline{T}_p$ over $\FF_p$, and we can lift $(\overline{T}_p,0,\infty)$ to a projective line endowed with two marked points over $\ZZ_p$: $(\overline{T}_p^\sim,0,\infty)$.
...
\oldpage{19 (97)}We hope that the realisation functors define a fully faithful functor from the category of motives over $\QQ$ to the category of realisation systems.
...
ii. We would like to complete [(AM4)](#AM4) by a condition for all $p$, cf. 1.8.5 in [P. Deligne, "La conjecture de Weil II", _Publ. Math. IHÉS_ **52** (1980), 137--252].
...
\oldpage{20 (98)}The treatment of crystalline structures is not satisfying.
...
\oldpage{21 (99)}Comparison isomorphisms
$$
  \begin{aligned}
...
\oldpage{22 (100)}In the language of [(1.14)](#chunk-1.14), in [(M'3)](#Mp3) we need to give $M_\ell$ as a smooth $\QQ_\ell$-sheaf on $\Spec(\ZZ[1/\ell])\setminus P$ instead of on $\Spec(\QQ)$, and we need to modify [(M'4)](#Mp4) like [(M4)](#M4).
...
\oldpage{23 (101)}The motive $A$ is endowed with a product
$$
  A\otimes A\to A
\tag{1.16.1}
$$
...
\oldpage{24 (102)}We have $E\otimes\QQ_p=\bigoplus_{v|p}E_v$, and $N_\crisp$ is the sum of the $M_\crisv$.
...
\oldpage{25 (103)}A vector bundle $M_\DR$ with integrable connection on $S_\QQ$, assumed to be regularly singular at infinity.
:::
...
\oldpage{26 (104)}Let $E$ be a finite extension of $\QQ$.
...
\oldpage{27 (105)}The *Tate motive* $\ZZ(1)$ is a motive over $\Spec(\ZZ)$ [(1.15)](#chunk-1.15) with integer coefficients [(1.23)](#chunk-1.23).
...
\oldpage{28 (106)}The functor $A\mapsto T(A)$ is fully faithful: from $T(A)$ we can recover $\Lie(A)=T(A)_\DR/F^0$ and the complex torus
$$
  A(\CC)
...
\oldpage{29 (107)}A point $a$ of an abelian variety $A$ over $\QQ$ defines a $1$-motive $[\ZZ\xrightarrow{u}A]$ with $u(1)=a$.
...
\oldpage{30 (108)}The data of $F_\infty$ for $E$ is equivalent to that of an involution of $P_\BB$ that is compatible with that of $M_\BB$.
...
The data of [(M1)](#M1), [(M2)](#M2), $\comp_{\DR/\BB}$, and [(M8)](#M8) are equivalent to the data of a torsor $F^0P_\DR$ for $F^0M_\DR$ and, in the $M_\DR\otimes\CC=M_\BB\otimes\CC$ torsor $P_{\DR,\CC}$ which is induced by $F^0M_\DR\to M_\DR\otimes\CC$, a $\QQ$-structure $P_\BB\subset P_{\DR,\CC}$ that is an $M_\BB$ torsor.
:::
...
\oldpage{31 (109)}As in [(1.24)](#chunk-1.24) and [(1.25)](#chunk-1.25), this corresponds to modifying [(2.5)](#chunk-2.5) as follows:
...
\oldpage{32 (110)}On $\Gm=\Spec\ZZ[q,q^{-1}]$, the universal section $q$ of $\Gm$ also defines a torsor in motives $K(q)$ for $\ZZ(1)$, on the base $\Gm$.
We now describe it.
...
$$
\oldpage{32 (111)}For $x\in\QQ$ a unit at $p$, and $\widetilde{F}$ such that $\widetilde{F}(x)=x$, we thus have $\phi_p^{-1}[\widetilde{F}](0)=\log(x/x^p)$ at $x$, as required by [(2.9)](#chunk-2.9)
:::
...
\oldpage{33 (112)}The study of the fundamental group of $\PP^1\setminus\{0,1,\infty\}$ reveals, for each $k\geq2$, a torsor $P_{1,k}$ for $\ZZ(k)$ on $\Spec(\ZZ)$.
...
\tag{3.1.2k}
$$
\oldpage{34 (113)}It is equivalent to sum over the $\alpha\neq1$.
...
If $z$ is not real and $\geq1$, then we have principal determinations
$$
  \prod_{u^M=z} (1-u)^{1/N}
  = (1-z)^{1/N}
\tag{3.1.5}
...
\oldpage{35 (114)}Some preliminaries, before defining $\phi_p$.
The $k$-logarithm is defined for $|z|\leq1$ by
$$
  \ell_k(z)
  = \sum z^n/n^k.
\tag{3.2.1}
