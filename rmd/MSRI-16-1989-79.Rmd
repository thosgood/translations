...
\oldpage{81}The present article owes much to A. Grothendieck.
...
\oldpage{82}This close relation with cohomology hints that the study of nilpotent versions of $\hat{\pi}_1$ is far from the "anabelian" dream of A. Grothendieck.
...
\oldpage{83}I have also been helped by the answer by O. Gabber to my question "How can we construct an extension of $\ZZ_\ell$ by $\ZZ_\ell(3)$, uniformly in $\ell$?": "By a class in $K_5(\QQ)$", as well as by the conjectures of A. Beilinson on the values of $L$-functions.
...
\oldpage{85}Our solution will be to define a motivic linear group as being an Ind-object in the category of motives, endowed with the structure of a commutative Hopf algebra.
...
\oldpage{86}They allow us, in the definition of the motivic $\pi_1$ of $X$, to take a base point "at infinity", like the tangent vector $v$ at $s$.
...
\oldpage{88}If $P$ is a $G$-torsor, then a sheaf $X$ on which $G$ acts can be _twisted_ by $P$.
...
- **$\ell$-adic cohomology.**
  ...
  \oldpage{90}This follows by passing to the limit in the base change theorem for a smooth morphism [SGA 4, XVI, 1.2]: $k'$ is the filtrant inductive limit of the $k$-algebras $A$ with $\Spec(A)$ smooth over $k$.
...
\tag{1.1.5}
$$
\oldpage{91}giving rise to commutative diagrams
$$
  \begin{CD}
...
\oldpage{92}In each of the known theories, the $\HH^i(X)$ are endowed with an increasing filtration $W$, known as the _weight filtration_ [@D5], as well as comparison isomorphisms such that (1.1.1) and (1.1.2) are compatible with $W$.
...
\oldpage{93}"_almost every_ prime number" means "all, except for a finite number".
...
$$
\oldpage{94}In other words, $c_\DR$ respects $M_\BB\subset M_\BB\otimes\CC=M_\DR\otimes\CC$, and $c_\DR|M_\BB=F_\infty$.
:::
...
\oldpage{95}In this section, we will partially describe the system [(M1)--(M10)](#chunk-1.4) of realisations of $M$ in the case where $X$ is separated and smooth over $\QQ$.
...
- **Semi-stable case.**
  ...
  \oldpage{96}We complete it to a projective line $\overline{T}_p$ over $\FF_p$, and we can lift $(\overline{T}_p,0,\infty)$ to a projective line endowed with two marked points over $\ZZ_p$: $(\overline{T}_p^\sim,0,\infty)$.
...
\oldpage{97}We hope that the realisation functors define a fully faithful functor from the category of motives over $\QQ$ to the category of realisation systems.
...
\oldpage{98}The treatment of crystalline structures is not satisfying.
...
\oldpage{99}Comparison isomorphisms
$$
  \begin{aligned}
...
\oldpage{100}In the language of [(1.14)](#chunk-1.14), in [(M'3)](#Mp3) we need to give $M_\ell$ as a smooth $\QQ_\ell$-sheaf on $\Spec(\ZZ[1/\ell])\setminus P$ instead of on $\Spec(\QQ)$, and we need to modify [(M'4)](#Mp4) like [(M4)](#M4).
...
\oldpage{101}The motive $A$ is endowed with a product
$$
  A\otimes A\to A
\tag{1.16.1}
$$
...
\oldpage{104}Let $E$ be a finite extension of $\QQ$.
...
\oldpage{105}The *Tate motive* $\ZZ(1)$ is a motive over $\Spec(\ZZ)$ [(1.15)](#chunk-1.15) with integer coefficients [(1.23)](#chunk-1.23).
...
\oldpage{112}The study of the fundamental group of $\PP^1\setminus\{0,1,\infty\}$ reveals, for each $k\geq2$, a torsor $P_{1,k}$ for $\ZZ(k)$ on $\Spec(\ZZ)$.
...
For $M$ of weight $\leq-1$, this implies:
