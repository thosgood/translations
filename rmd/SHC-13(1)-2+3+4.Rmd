...
date: "November 1960"
original: 'Douady, A. _Séminaire Henri Cartan_ **13 (1)** (1960--61), Talks no. 2 ("Variétés et espaces mixtes"), 3 ("Déformations régulières"), and 4 ("Obstruction primaire à la déformation"). [`http://www.numdam.org/item/SHC_1960-1961__13_1`](http://www.numdam.org/item/SHC_1960-1961__13_1)'
...
**!!to-do: explain why there is no talk no. 1, and why we're only doing the first three talks after this!!**

# (PART) 2. Mixed manifolds and mixed spaces {.unnumbered}

# I. Category of models {.unnumbered #2-section-I}
...
# II. The definition of mixed spaces and mixed varieties {.unnumbered #2-section-II}

## 1. First definition {.unnumbered #2-section-II.1}
...
## 2. An equivalent definition {.unnumbered #2-section-II.2}
...
## 3. Deformations {.unnumbered #2-section-II.3}
...
# III. Vector fields {.unnumbered #2-section-III}

## 1. Study on models {.unnumbered #2-section-III.1}
...
## 2. Vector fields on a mixed manifold {.unnumbered #2-section-III.2}
...
::: {.itenv #2-proposition title="Proposition" latex="{Proposition}"}
...
# IV. The Spencer--Kodaira map {.unnumbered #2-section-IV}
...
::: {.rmenv #2-definition title="Definition" latex="{Definition}"}
...
# (PART) 3. Regular deformations {.unnumbered}
...
## I. The map $\widetilde{\rho}$ {.unnumbered #3-section-I}
...
::: {.itenv #3-theorem-1 title="Theorem 1" latex="{Theorem 1}"}
...
# II. The regular case {.unnumbered #3-section-II}
...
::: {.rmenv #3-definition title="Definition" latex="{Definition}"}
...
Then [Theorem 1](#3-theorem-1) has the following corollary:

::: {.itenv #3-proposition-1 title="Proposition 1" latex="{Proposition 1}"}
...
# III. An example of non-regular deformation: Hopf manifolds {.unnumbered #3-section-III}

## 1. Hopf manifolds {.unnumbered #3-section-III.1}
...
Note that $V_b$ and $V_{b'}$ are isomorphic if and only if there exists some $a$ such that $b'=aba^{-1}$ or $b'=ab^{-1}a^{-1}$ (cf. [Appendix](#3-appendix)).

Let $\Theta$ be the sheaf of germs of holomorphic fields of tangent vectors on $V_b$.

::: {.itenv #3-proposition-2 title="Proposition 2" latex="{Proposition 2}"}
...
::: {.itenv #3-lemma-1 title="Lemma 1" latex="{Lemma 1}"}
...
Then $\beta-b_*\beta=\gamma$, whence [Lemma 1](#3-lemma-1).
:::

Now, to finish the proof of [Proposition 2](#3-proposition-2).
From [Lemma 1](#3-lemma-1), we have the following exact sequence:
$$
  0
  \to \HH^0(V_b;\Theta)
...
Then [Proposition 2](#3-proposition-2) follows from remarking that, if an element of $L$ is represented by a matrix $a$, then $b_*a=bab^{-1}$.
::::


## 2. Mixed manifolds whose fibres are Hopf manifolds {.unnumbered #3-section-III.2}
...
## 3. Calculation of $\rho$ {.unnumbered #3-section-III.3}

We have $T_b=\Hom(\CC^n,\CC^n)=L\subset\HH^0(\widetilde{V};\Theta)$, and we defined, to prove [Proposition 2](#3-proposition-2), a surjective map $\delta_*\colon L\to\HH^1(V_b;\Theta)$.

::: {.itenv #3-proposition-3 title="Proposition 3" latex="{Proposition 3}"}
...
$$
We thus deduce that $\theta=\delta_*(ab^{-1})$, which proves [Proposition 3](#3-proposition-3).
:::


## 4. A counter-example {.unnumbered #3-section-III.4}
...
It follows from [Proposition 3](#3-proposition-3) that the Spencer--Kodaira map
$$
...
## 5. Question (K. Srinivasacharyulu) {.unnumbered #3-section-III.5}
...
# Appendix {.unnumbered #3-appendix}

With the notation of [§III.1](#3-section-III.1), let $f\colon V_b\to V_{b'}$ be an isomorphism of $\CC$-analytic manifolds.
...
# (PART) 4. The primary obstruction to deformation {.unnumbered}

# Introduction {.unnumbered #4-introduction}
...
# I. Exact sequences of sheaves of algebras {.unnumbered #4-section-I}
...
::: {.itenv #4-proposition-1 title="Proposition 1" latex="{Proposition 1}"}
...
# II. The primary obstruction {.unnumbered #4-section-II}
...
::: {.itenv #4-theorem-1 title="Theorem 1" latex="{Theorem 1}"}
...
This is simply a particular case of [Theorem 1](#4-theorem-1);
...
$$
by [Proposition 1](#4-proposition-1).
...
but it is not sufficient: we can define a sequence of set-theoretic maps $\omega_n$, called *obstructions*, with $\omega_1\colon\HH^1(V_0,\Theta)\to\HH^2(V_0,\Theta)$ given by $\omega_1(a)=[a\smile a]$, and with $\omega_{k+1}$ defined on the subset of $\HH^1(V_0,\Theta)$ where $\omega_k$ vanishes, with values in varying quotients^[See the [Appendix](#4-appendix).] of $\HH^2(V_0,\Theta)$, and a necessary condition for $a$ to be a deformation vector is that all the $\omega_k(a)$ be defined and real.
...
# III. An example of obstruction {.unnumbered #4-section-III}

## 1. The manifold $V_0$ {.unnumbered #4-section-III.1}
...
## 2. The mixed space $V$ {.unnumbered #4-section-III.2}
...
::: {.itenv #4-proposition-2 title="Proposition 2" latex="{Proposition 2}"}
There exists a mixed space $\pi\colon V\to B$ and a point $b_0\in B$ such that

1. $\pi^{-1}(b_0)=V_0$ (the manifold defined in [§III.1](#4-section-III.1));
...
## 3. Calculating $\rho_0$ {.unnumbered #4-section-III.3}
...
# (PART) Appendix: Higher obstructions {.unnumbered #4-appendix}

# I. Definition of obstructions {.unnumbered #4-appendix-I}

## 1. The sheaf of germs of vertical automorphisms {.unnumbered #4-appendix-I.1}
