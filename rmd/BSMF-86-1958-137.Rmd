...
b. a homomorphism $p_X\colon\PP(X)\to A^2(X)$ that is functorial in $X\in\cal{V}$; and
...
Under the conditions of [Lemma 2](#lemma-2), if $s$ is non-zero at every point, then $\prod_{1\leq i\leq p}\xi_i=0$.
:::

::: {.rmenv title="Remark" data-latex="{Remark}"}
The introduction of the operation $i$ in [c.](#data), and the axioms [A2](#axiom-A2), [A3](#axiom-A3), and [A4](#axiom-A4) concerning this operation, only serve to provide us with technical means of being able to prove [the corollary to Lemma 2](#lemma-2-corollary).
(Axiom [A4](#axiom-A4) would still have sufficed even if we had assumed that $b=1_Y$.)
For the theory of Chern classes in [the following section](#section-3), we will only need the data of [a.](#data) and [b.](#data), axiom [A2](#axiom-A1), and [the corollary to Lemma 2](#lemma-2-corollary).
:::
...
3. The base field $k$ is the field of complex numbers, $\cal{V}$ is the category of non-singular algebraic spaces, and $A(X)=\HH^\bullet(X,\mathbb{Z})$ (with $X$ being endowed with its "usual" topology).
    The definition of [b.](#data) (either by Poincaré duality on divisor classes, or as an obstruction class in the classical exact sequence $0\to\mathbb{Z}\to\scr{O}_X\to\scr{O}_X^\times\to0$ of sheaves on $X$, endowed with its usual topology) is well known.
    The definition of [c.](#data) classically comes
...
Suppose that we have the data of [a.](#data), [b.](#data), and [c.](#data) from the previous section, satisfying axioms [A1](#axiom-A1) to [A4](#axiom-A4).
...
So we know $c(E)$ if we know $f(c(E))$, which, by i., is equal to $c(f^{-1}(E))$.
But $f^{-1}(E)$ splits completely.
We can thus reduced to determining $c(E)$ when $E$ is a rank-$p$ vector bundle that splits completely, and which is thus endowed with a composition series $(E_i)_{0\leq i\leq p}$, with $\rank E_i=p-i$,
...
i. Let $X,Y\in\cal{V}$, and let $E$ be a vector bundle on $Y$, and $F$ its inverse image under $f$.
    Then $\mathbb{P}(F)$ is the inverse image of $\mathbb{P}(E)$ under $f$, and so we have a commutative diagram of morphisms
    $$
      \begin{CD}
        \mathbb{P}(F) @>\overline{f}>> \mathbb{P}(E)
      \\@VpVV @VVqV
      \\X @>>f> Y
      \end{CD}
    $$
...
**TO-DO: REPLACE Data AND Theorem 1 ENUMS WITH ENVS (LIKE FOR A1, ..., A4)**