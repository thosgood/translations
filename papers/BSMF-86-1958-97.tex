...
\begin{proof}[Proof of {\hyperref[theorem2]{Theorem~2}}]
  ...
  \begin{itenv}{Lemma 11}
  \label{lemma11}
    $\gamma_1(\scr{L})$ depends only on $\scr{F}$.
  \end{itenv}
  \begin{itenv}{Lemma 12}
  \label{lemma12}
...
\begin{proof}{Proof of part (d) of \hyperref[lemma19]{Lemma~19}}
  By \hyperref[lemma22]{Lemma~22}, we have that
  \[
    (-1)^{p-1}\lambda_{-1}F^* = \lambda^{p-1}(F^*-1).
  \]
  But $E'^*/F^*=L^*$, so $F^*-1\equiv E'^*-2\mod(1-L^*)$, whence (\hyperref[lemma22]{Lemma~22})
  \[
  ...
  \]
\oldpage{135}
  It thus suffices to show that $\lambda^{p-1}(E^*-2) = 0$ if $p\geq\dim Y+2$, which follows from \hyperref[lemma21]{Lemma~21}.
\end{proof}
