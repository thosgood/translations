...
\section{Fibred categories, descent data, \texorpdfstring{$\cat{F}$}{F}-descent morphisms}
\label{A.1}
...
\label{definition:A.1.1}
  A \emph{fibred category $\cat{F}$ with base $\cat{C}$} (or \emph{over $\cat{C}$}) consists of
  \begin{itemize}
    \item a category $\cat{C}$
    \item for every $X\in\cat{C}$, a category $\cat{F}_X$
    \item for every $\cat{C}$-morphism $f\colon X\to Y$, a functor $f^*\colon\cat{F}_Y\to\cat{F}_X$, which we also write as
      \[
        f^*(\xi) = \xi \times_Y X
      \]
      for $\xi\in\cat{F}_Y$ (with $X$ being thought of as an ``object of $\cat{C}$ over $Y$'', i.e. as being endowed with the morphism $f$)
...
\label{example:A.1.1(1)}
  ...
  We then define a fibred category $\cat{F}$ with base $\cat{C}$ by setting $\cat{F}_X$ to be the category of objects of $\cat{C}$ over $X$, and the functor $f^*\colon\cat{F}_Y\to\cat{F}_X$ corresponding to a morphism $f\colon X\to Y$ being defined by the \emph{fibre product} $Z\rightsquigarrow Z\times_Y X$.
\end{example}

\begin{example}{2}
\label{example:A.1.1(2)}
  Let $\cat{C}$ be the category of preschemes, and, for $X\in\cat{C}$, let $\cat{F}_X$ be the category of quasi-coherent sheaves of modules on $X$.
  If $f\colon X\to Y$ is a morphism of preschemes, then $f^*\colon\cat{F}_Y\to\cat{F}_X$ is the
\oldpage{190-03}
...
\label{definition:A.1.3}
  Let $\cat{F}$ be a fibred category with base $\cat{C}$, and consider a diagram
  \[
    \begin{tikzcd}
      S
      & S' \lar[swap,"\alpha"]
  ...
  this diagram is said to be \emph{$\cat{F}$-exact} if, for every pair $(\xi,\eta)$ of elements of $\cat{F}_S$, the diagram
  \[
...
\begin{definition}{1.4}
  Let $\cat{F}$ be a fibred category with base $\cat{C}$, and consider morphisms $\beta_1,\beta_2\colon S''\to S'$ in $\cat{C}$.
  Let $\xi'\in\cat{F}_{S'}$.
  ...
  If $\xi',\eta'\in\cat{F}_{S'}$ are both endowed with gluing data, then a morphism $u\colon\xi'\to\eta'$ in $\cat{F}_{S'}$ is said to be \emph{compatible with the gluing data} if the following diagram commutes:
  \[
    \begin{tikzcd}
      \beta_1^*(\xi') \rar \dar
      &\beta_2^*(\xi') \dar
...
With this definition, the objects of $\cat{F}_{S'}$ that are endowed with gluing data (with respect to $\beta_1$ and $\beta_2$) then form a \emph{category}.
If $\alpha\colon S'\to S$ is a morphism such that $\alpha\beta_1=\alpha\beta_2$, then, for every $\xi\in\cat{F}_{S'}$, the object $\xi'=\alpha^*(\xi)$
\oldpage{190-04}
of $\cat{F}_{S'}$ is endowed with a canonical gluing data, since
\[
  \beta_i^*\alpha^*(\xi)
...
furthermore, if $u\colon\xi\to\eta$ is a morphism in $\cat{F}_s$, then
\[
  \alpha^*(u)\colon \alpha^*(\xi) \to \alpha^*(\eta)
\]
is a morphism in $\cat{F}_{S'}$ that is compatible with the canonical gluing data.
We thus obtain a \emph{canonical functor} from the category $\cat{F}_S$ to the category of objects of $\cat{F}_{S'}$ endowed with gluing data with respect to the pair $(\beta_1,\beta_2)$.
With this, we can also rephrase \cref{definition:A.1.3} by saying that the diagram~\cref{equation-definition:A.1.3} is \emph{$\cat{F}$-exact} if the above functor is \emph{fully faithful}, i.e. if the above functor defines an equivalence between the category $\cat{F}_S$ and a subcategory of the category of objects of $\cat{F}_S$ endowed with gluing data with respect to $(\beta_1,\beta_2)$.

\begin{definition}{1.5}
\label{definition:A.1.5}
  We say that a gluing data on $\xi'\in\cat{F}_{S'}$ is \emph{effective} (with respect to $\alpha$) if $\xi'$, endowed with this data, is isomorphic to $\alpha^*(\xi)$ for some $\xi\in\cat{F}_S$.
\end{definition}

In the case where the diagram~\cref{equation-definition:A.1.3} is $\cat{F}$-exact, the object $\xi$ in \cref{definition:A.1.5} is then determined up to unique isomorphism, and \emph{the category $\cat{F}_S$ is equivalent to the category of objects of $\cat{F}_{S'}$ endowed with effective gluing data}.
...
We then have two immediate necessary conditions for a gluing data $\varphi\colon p_1^*(\xi')\to p_2^*(\xi')$ on some $\xi'\in\cat{F}_S$ to be effective:
\begin{enumerate}[(i)]
...
\label{definition:A.1.6}
  We define \emph{descent data} on $\xi'\in\cat{F}_{S'}$, with respect to the morphism $\alpha\colon S'\to S$, to be a gluing data on $\xi'$ with respect to the pair $(p_1,p_2)$ of canonical projections $S'\times_S S'\to S'$ that satisfies conditions~(i) and (ii) above.
\end{definition}

\begin{definition}{1.7}
\label{definition:A.1.7}
  A morphism $\alpha\colon S'\to S$ is said to be an \emph{$\cat{F}$-descent morphism} if the diagram
  \[
    \begin{tikzcd}
      S
      & S' \lar[swap,"\alpha"]
    ...
    \end{tikzcd}
  \]
  of morphisms is $\cat{F}$-exact (\cref{definition:A.1.3});
  we say that $\alpha$ is a \emph{strict $\cat{F}$-descent morphism} if, further, every descent data (\cref{definition:A.1.6}) on any object of $\cat{F}_{S'}$ is effective.

  This latter condition (of strictness) can also be stated in a more evocative way:
  ``giving an object of $\cat{F}_S$ is equivalent to giving an object of $\cat{F}_{S'}$ endowed with a descent data''.
\end{definition}

Note that, if an $\cat{F}$-descent morphism\footnote{\emph{[Comp.] It is useless to assume here that $\alpha$ is an $\cat{F}$-descent morphism.}} $\alpha\colon S'\to S$ admits a \emph{section} $s\colon S\to S'$ (i.e. a morphism $s$ such that $\alpha s=\id_S$), then it is a strict $\cat{F}$-descent morphism:
if $\xi'\in\cat{F}_{S'}$ is endowed with descent data with respect to $\alpha$, then ``it comes from'' $\xi=s^*(\xi')$.
...
Given an object $\xi'\in\cat{F}_{S'}$, there then corresponds, to every $t\in S'(T)$, an object $t^*(\xi')$ of $\cat{F}_T$, which will also be denoted by $\xi'_t$.
...
To make the relation between the ideas of $\cat{F}$-descent morphisms and strict epimorphisms more precise, we introduce the following definitions:
...
\label{definition:A.2.4}
  A morphism $\alpha\colon S'\to S$ is said to be a \emph{descent morphism} (resp. a \emph{strict descent morphism}) if it is an $\cat{F}$-descent morphism (resp. a strict $\cat{F}$-descent morphism) (cf. \cref{definition:A.1.7}), where $\cat{F}$ denotes the fibred category over $\cat{C}$ of objects of $\cat{C}$ over objects of $\cat{C}$ (\cref{example:A.1.1(1)}).
\end{definition}
...
\[
  ...
  \end{tikzcd}
\]
In particular, if $\cat{F}$ denotes the fibred category (over the category $\cat{C}$ of preschemes) such that, for $X\in\cat{C}$, $\cat{F}_X$ is the category of \emph{flat} $X$-preschemes, then the diagram~\cref{equation-example:A.2.b} is $\cat{F}$-exact.
...
We similarly see that \cref{equation-example:A.2.b} is $\cat{F}$-exact if $\cat{F}$ denotes the fibred category for which $\cat{F}_X$ is the category of \emph{flat} quasi-coherent sheaves on the prescheme $X$ (here, again, the
\oldpage{190-09}
...
\[
  ...
  \end{tikzcd}
\]
is exact, which is equivalent to saying that the corresponding morphism $S'\to S$ between the spectra of $A'$ and $A$ is an $\cat{F}$-descent morphism, where $\cat{F}$ is the fibred category of flat quasi-coherent sheaves.
...
For example, let $\cat{F}$ be a fibred category with base $\cat{C}$.
Let $\xi,\eta\in\cat{F}_S$, and, for all $S'$ over $S$, let
\[
...
\]
\emph{is an isomorphism for every pair of elements $\xi,\eta\in\cat{F}_S$ implies that $\alpha\colon S'\to S$ is an $\cat{F}$-descent morphism} (\cref{definition:A.1.7}).


\subsection{}
\label{A.4.c}

Similarly, for $\xi\in\cat{F}_S$ and any object $S'$ of $\cat{C}$ over
\[
...
\oldpage{190-14}
groups.
With this setup, we claim that \emph{$Z^1(S'/S,G)$ is canonically identified with the set of descent data on $\xi'=\xi\times_S S'$ with respect to $S'\to S$} (\cref{definition:A.1.6}), and that \emph{$\HH^1(S'/S,G)$ can be identified with the set of isomorphism classes of objects of $\cat{F}_{S'}$ endowed with a descent data relative to $\alpha\colon S'\to S$ that are isomorphic, as objects of $\cat{F}_{S'}$, to $\xi'=\xi\times_S S'$.}
Then, \emph{if $\alpha\colon S'\to S$ is an $\cat{F}$-descent morphism} (cf. \hyperref[A.4.b]{A.4.b}), \emph{then $\HH^1(S'/S,G)$ contains as a subset the set of isomorphism classes of objects $\eta$ of $\cat{F}_S$ such that $\eta\times_S S'$ is isomorphic (in $\cat{F}_{S'}$) to $\xi\times_S S'$};
...
\oldpage{190-15}
  ...
  We can also think of $\GL(n)$ as an automorphism functor (cf. \hyperref[A.4.c]{A.4.c}) by considering the fibred category $\cat{F}$ with base $\cat{C}$ such that $\cat{F}_X$ is the category of locally free sheaves on $X$, for $X\in\cat{C}$, since then $\GL(n)(X)=\Aut_{\cat{F}_X}(\sh{O}_X^n)$.
  By \hyperref[A.4.b]{A.4.b}, it follows that, if $\alpha\colon S'\to S$ is an $\cat{F}$-descent morphism (cf. \hyperref[A.2.c]{A.2.c}), then $\HH^1(S'/S,\GL(n))$ contains the set of isomorphism classes of locally free sheaves on $S$ whose inverse image on $S'$ is isomorphic to $\sh{O}_{S'}^n$, and this inclusion is an equality if and only if every descent data on $\sh{O}_{S'}^n$ (with respect to $\alpha\colon S'\to S$) is effective.
  ...
  \begin{enumerate}[(i)]
    ...
    \item $\alpha\colon S'\to S$ is an $\cat{F}$-descent morphism (where $\cat{F}$ is the fibred category over $\cat{C}$ described above).
  ...
  \emph{In all the cases known to the speaker, $\HH^i(A'/A,\Ga)=0$ for $i>0$, and, if $A$ is local, then $\HH^1(A'/A,\Gm)=0$, and, more generally, $\HH^1(A'/A,\GL(n))=(e)$} (if $S'\to S$ is an $\cat{F}$-descent morphisms, i.e. if the diagram $A\to A'\rightrightarrows A'\otimes_A A'$ is exact, then compare this with \hyperref[A.2.c]{A.2.c}).
...
\label{theorem:B.1(1)}
  ...
  Then $\alpha$ is a \emph{strict descent morphism} (cf. \hyperref[definition:A.1.7]{A, Definition~1.7}) for the fibred category $\cat{F}$ of quasi-coherent sheaves (cf. \hyperref[example:A.1.1(2)]{A, Example~2}).
\end{theorem}

This statement implies two things:
\begin{enumerate}[(i)]
  \item If $\cat{F}$ and $\sh{G}$ are quasi-coherent sheaves on $S$, and $\cat{F}'$ and $\sh{G}'$ their inverse images on $S'$, then the natural homomorphism
    \[
      \Hom(\cat{F},\sh{G}) \to \Hom(\cat{F}',\sh{G}')
    \]
    is a bijection from the left-hand side to the subgroup of the right-hand side consisting of homomorphisms $\cat{F}'\to\sh{G}'$ that are compatible with the canonical descent data on these sheaves, i.e. whose inverse images under the two projections of $S''=S'\times_S S'$ to $S'$ give the same homomorphism $\cat{F}''\to\sh{G}''$.
  \item Every quasi-coherent sheaf $\cat{F}'$ on $S'$ endowed with a descent data with respect to the morphism $\alpha\colon S'\to S$ (cf. \hyperref[definition:A.1.6]{A, Definition~1.6}) is isomorphic (endowed with this data) to the inverse image of a quasi-coherent sheaf $\cat{F}$ on $S$.
\end{enumerate}

Setting $\cat{F}=\sh{O}_S$ in (i), we obtain:
...
The data of a quasi-coherent sheaf $\cat{F}'$ on $S'$ is equivalent to the data of the quasi-coherent sheaf $\alpha_*(\cat{F}')$ on $S$ endowed with its $\sh{A}'$-modules structure (noting that $\sh{A}'=\alpha_*(\sh{O}_{S'})$).
For simplicity, we also denote this sheaf on $S$ by $\cat{F}'$.
The two inverse images $p_i^*(\cat{F}')$ of $\cat{F}'$ on $S'\times_S S'$ similarly correspond to the quasi-coherent sheaves of $(\sh{A}'\otimes_{\sh{O}_S}\sh{A}')$-modules $\cat{F}'\otimes_{\sh{O}_S}\sh{A}'$ and $\sh{A}'\otimes_{\sh{O}_S}\cat{F}'$.
...
\[
  ...
  \to \shHom_{\sh{O}_S}(\cat{F}',\cat{F}')
\]
i.e. to the data, for every section $\xi$ of $\sh{U}$ over an open subset $V$, of a homomorphism of $\sh{O}_S$-modules $T_\xi\colon\cat{F}'|V\to\cat{F}'|V$ that satisfies the conditions
\[
...
\tag{3.1}
\]
where $f$ and $x$ are (respectively) sections of $\sh{A}'$ and $\cat{F}'$ over an open subset of $S$ that is contained inside $V$.
Conditions~(i) and (ii) of a descent data (cf. \hyperref[A.1.c]{(A.1.c)}) can then be written (respectively) as
\[
\label{equation:B.3.2}
  T_{1_U}(x) = x,
  \qquad\mbox{i.e. $T_{1_U}=\id_{\cat{F}'}$}
\tag{3.2}
\]
\[
...
\tag{3.3}
\]
In other words, \emph{a descent data on $\cat{F}'$ is equivalent to a representation of the sheaf $\sh{U}=\shHom_{\sh{O}_S}(\sh{A}',\sh{A}')$ of $\sh{O}_S$-algebras in the sheaf $\shHom_{\sh{O}_S}(\cat{F}',\cat{F}')$ of $\sh{O}_S$-algebras that satisfies the two linearity conditions \cref{equation:B.3.1}}.
If we have a pairing of quasi-coherent sheaves on $S'$:
\[
  \cat{F}'_1\times\cat{F}'_2 \to \cat{F}'_3
\]
(that we can think of as a pairing of sheaves of $\sh{A}'$-modules on $S$),
\oldpage{190-22}
and gluing data on the $\cat{F}'_i$ defined by homomorphisms $T_i\colon\sh{U}\to\shHom_{\sh{O}_S}(\cat{F}'_i,\cat{F}'_i)$ (for $i=1,2,3$), then these data are \emph{equivalent to the given pairing}, in the evident meaning of the phrase, if and only if the following condition is satisfied:
...
\label{example:B.3(1)}
  ...
  Then a descent data on a quasi-coherent sheaf $\cat{F}'$ on $S'$ (resp. on an $S'$-prescheme $X'$) is equivalent to the data of a representation of $\Gamma$ by automorphisms of $(S',\cat{F}')$ (resp. of $(S',X')$) that is compatible with the action of $\Gamma$ on $S'$.
  This result
\oldpage{190-23}
...
It follows from the above lemma that a descent data on the quasi-coherent sheaf $\cat{F}'$ on $S'$ is equivalent to the data, for all $X\in\fk{C}$, of an $\sh{O}_S$-endomorphism $\overline{X}$ of $\cat{F}'$ that satisfies the following conditions:
\[
...
\tag{3.10}
\]
(This is what we may call a \emph{linear connection on $\cat{F}$}, \emph{flat}, and \emph{compatible with the $p$-th powers}).
...
Note that we have not needed to impose any flatness, non-singular, or finiteness hypotheses on $\cat{F}'$ or $X'$.
...
If $\cat{F}$ is an \emph{invertible sheaf} (i.e. locally free of rank~$1$) on $X$, then there is a bijective correspondence between automorphisms of $\cat{F}$ (identified with the invertible sections of $\sh{O}_X$) and invertible sections of $\sh{O}_S$.
So let $s$ be a section of $X$ over $S$;
we define a \emph{section of $\cat{F}$ over $s$} to be a section of the invertible sheaf $s^*(\cat{F})$ on $S$.
It follows from the above that, if $\cat{F}_i$ (for $i=1,2$) are invertible sheaves on $X$, each endowed with a section over $s$, and \emph{if $\cat{F}_1$ and $\cat{F}_2$ are isomorphic, then there exists exactly one isomorphism from $\cat{F}_1$ to $\cat{F}_2$ that is compatible with the sections in question} (i.e. sending the first to the second).
We also, independently of the section $s$, regard two invertible sheaves $\cat{F}_1$ and $\cat{F}_2$ on $X$ as \emph{equivalent} if every point of $S$ has an open neighbourhood $U$ such that the restrictions of $\cat{F}_1$ and $\cat{F}_2$ to $X|U$ are isomorphic.
Then \emph{every invertible sheaf $\cat{F}$ on $X$ is equivalent to an invertible sheaf $\cat{F}_1$ endowed with a marked section over $s$} (we take $\cat{F}_1=Fs^*(\cat{F})^{-1}$), \emph{and $\cat{F}_1$ is determined up to isomorphic}.
...
\emph{With the prescheme $X/S$ being as above, and admitting a section $s$, let $\alpha\colon S'\to S$ be a faithfully flat and quasi-compact morphism; let $\cat{F}'$ be an invertible sheaf on $X'=X\times_S S'$.}
\emph{For $\cat{F}'$ to be equivalent to the inverse image on $X'$ of an invertible sheaf $\cat{F}'$ on $X$, it is necessary and sufficient for its inverse images $p_1^*(\cat{F}')$ and $p_2^*(\cat{F}')$ on $X'\times_X X'=X\times_S(S'\times_S S')$ to be equivalent.}
\emph{If this is the case, then $\cat{F}$ is determined up to equivalence.}
(We then say that $\cat{F}'$ is \emph{rational} on $S$).
...
The the first case, $\cat{F}'$ is rational on $S$ if and only if its images under $\Gamma$ are all equivalent to $\cat{F}'$.
...
Since the restrictions of the $\cat{F}''_i=p_i(\cat{F}')$ (for $i=1,2$) to the diagonal are isomorphic (since they are both isomorphic to $\cat{F}'$), i.e. $\cat{F}''_1(\cat{F}''_2)^{-1}=\cat{F}''$ has a restriction to the diagonal which is trivial, it follows that the restriction of $\cat{F}''$ to $(X'',\sh{O}_{X''}/\sh{I}^2)$ is given, up to isomorphism, by a well-defined element $\xi$ of
\[
  \HH^1(X'',\sh{I}/\sh{I}^2) = \HH^1(X',\Omega_{X'/X}^1).
\]
Also, being precise, we have $\Omega_{X'/X}^1=\Omega_{S'/S}^1\otimes_{\sh{O}_S}\sh{O}_X$, and thus, \emph{if $\Omega_{S'/S}^1$ is locally free on $S$} (as in the Cartier case), \emph{then $\xi$ defines a section of $\RR^1f'(\sh{O}_{X'})\otimes\Omega_{S'/S}^1$ on $S'$} (called the \emph{Atiyah--Cartier class of the invertible sheaf $\cat{F}$ on $X'/S$}) \emph{whose vanishing is necessary and sufficient for the inverse images of $\cat{F}'$ under the two projections of}
\[
...
This vanishing is thus trivially \emph{necessary} for the inverse images of $\cat{F}'$ on $X''=X\times_S S''$ itself to be equivalent, and thus also for $\cat{F}$ to be equivalent to the inverse image of an invertible sheaf $\cat{F}$ on $X$.
The Atiyah--Cartier class can also be understood as the obstruction to the existence, locally over $S'$, of a \emph{connection} of $\cat{F}'$ relative to the derivations of $X'/X$, with such a connection further being determined when we know the derivations of $\cat{F}'$ corresponding to the natural extensions of derivations of $S'/S$ to $X'$.
From this, and the results of the previous section, we easily conclude that, in the case of the aforementioned \cref{example:B.3(2)}, and when $X/S$ admits a section, the vanishing of the Atiyah--Cartier class is also sufficient for $\cat{F}'$ to be rational on $S$.
