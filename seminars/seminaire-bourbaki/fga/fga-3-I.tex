...
Note that, if an $\cat{F}$-descent morphism\footnote{[Comp.] It is useless to assume here that $\alpha$ is an $\cat{F}$-descent morphism.} $\alpha\colon S'\to S$ admits a \emph{section} $s\colon S\to S'$ (i.e. a morphism $s$ such that $\alpha s=\id_S$), then it is a strict $\cat{F}$-descent morphism:
