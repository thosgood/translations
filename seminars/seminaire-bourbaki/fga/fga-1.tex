...
We consider the abelian category $\cat{C}^\sh{O}$ of sheaves of $\sh{O}$-modules, which are also referred to as $\sh{O}$-modules.
...
\tag{1.2}
\]
where $\RR^p$ denotes the passage to right-derived functors, and where $C(\sh{B})$ denotes an arbitrary injective resolution of $\sh{B}$ in $\cat{C}^\sh{O}$.
