...
\begin{proof}
  ...
  For simplicity, we assume that $\sh{L}_0=\sh{O}_X$, and that the augmentation $\sh{L}_0\to\sh{O}_Y$ is the natural homomorphism (which we can do), so then $\alpha$ is induced from the homomorphism of complexes $\sh{O}_X\to\sh{L}$ (with $\sh{O}_X$ being thought of as a complex concentrated in degree~$0$), taking into account the fact that $\underline{\RR}^p\Gamma(\sh{K})=\RR^p\Gamma(\sh{K}_0)$ if $\sh{K}$ is a complex of sheaves concentrated in degree~$0$.
  ...
  \]
\oldpage{149-12}
  The pairings in the two columns on the right are induced from the pairing of complexes of sheaves
  \[
  ...
  The pairing in the first column
