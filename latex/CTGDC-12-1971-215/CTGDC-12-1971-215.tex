...
\newcommand{\todo}[1]{{\color{purple}\textbf{TO-DO: #1}}}
\newcommand{\unsure}[1]{{\color{purple}\textbf{UNSURE: #1}}}
...
\newcommand{\Op}[1]{\Cat{Op}(#1)}
\newcommand{\Mod}[1]{\Cat{Mod}(#1)}
\newcommand{\Str}[2]{\Cat{Str}_{#1}(#2)}
...
\begin{translator}{Note from the translator}
  ...
  {\noindent}produced with kind permission from \todo{permissions}, and made possible by the efforts of Nathanael Arkor.
...
and if we have chosen such a limit for all $\phi$, we say that $\cat{E}$ is endowed with a \emph{canonical} (or \emph{official}) choice of projective $\cat{A}$-limits. Often such a choice is classical and is made tacitly.
We use analogous conventions for products, fibre products, etc. \ldots.
...
\[
  ...
  \tag{4}
\]
and a morphism $f\colon b\to b'$ in $\Alg{\TT}$ is defined by a morphism $f\colon e\to e'$ in $\cat{E}$ such that $b$ is a $\TT$-algebra on $e$, $b'$ is a $\TT$-algebra on $e'$, and
\[
  f\cdot b
  = b'\cdot Tf.
  \tag{5}
...
\[
  ...
  \end{tikzcd}
\]
with the horizontal arrows being furthermore unique whenever they make the diagram obtained by replacing the three forgetful functors to $\cat{E}$ with their adjoints commute.
...
\[
  ...
  \tag{1}
\]
We then form a fibre product $(v_1,v_2)$ of $(a,Tb)$:
\[
  \begin{tikzcd}[sep=huge]
    T\pi
      \ar[d,swap,"Tb"]
...
\begin{proof}
  ...
  It is easy to show that we can thus define an adjoint $D$ and a coadjoint $G$ to the functor $\Gr{\TT}\to\cat{E}$, and an adjoint $\overline{D}$ and a coadjoint $\overline{G}$ to the functor $\Cat{Cat}(\TT)\to\cat{E}$, which, by restriction, gives an adjoint and a coadjoint of the functor $\Ord{\TT}\to\cat{E}$.
\end{proof}
...
\begin{enumerate}
  ...
  \item[(5)]
    ...
    \[
      \begin{aligned}
        l(e,e')\colon \pr_2
      & \to \kappa(e,e',e')\cdot\big(\iota(e')\times\Hom_\cat{D}(e,e')\big)
      \\r(e,e')\colon \pr_1
      & \to \kappa(e,e,e')\cdot\big(\Hom_\cat{D}(e,e')\times\iota(e)\big)
      \end{aligned}
    \]
    where
    \[
    ...
    In other words, for each morphism $f\colon e\to e'$ in $\cat{D}$, $l$ and $r$ give ``natural'' 2-morphisms $l(e, e')(j, 0)$ and $r(e, e')(f, 0)$, denoted in shorthand by
    \[
  ...
  \item[(6)]
    ...
    \[
      s(e_1,e_2,e_3,e_4)\colon
      \kappa_{124}\cdot(\kappa_{234}\times\cat{D}_{12})
      \to \kappa_{134}\cdot(\cat{D}_{34}\times\kappa_{123})
    \]
    for each $(e_1,e_2,e_3,e_4)\in\set{\cat{D}}^4$, where we set
    \[
    ...
    Both these functors have source category $\cat{D}_{34}\times\cat{D}_{23}\times\cat{D}_{12}$ and target category $\cat{D}_{14}$.
    ...
    \[
      s(f,g,h)\colon h\circ(g\circ f) \to (h\circ g)\circ f.
    \]
    We also say that $s$ \emph{shifts parentheses to the left}.

  \item[(7)]
    Let $\varepsilon\colon\cat{D}_{23}\times\cat{D}_{12}\to\cat{D}_{23}\times\cat{D}_{22}\times\cat{D}_{12}$ be the functor given by the composition of the trivial functor $\cat{D}_{23}\times\cat{D}_{12}\to\cat{D}_{23}\times\mathbf{1}\times\cat{D}_{12}$ and the functor $\cat{D}_{23}\times\iota_2\times\cat{D}_{12}$, using the above notation, as well as setting
    \[
      \begin{aligned}
        \iota_i
      & = \iota(e_i)
    ...
    \[
      \begin{tikzcd}
        \kappa_{123}\cdot(\kappa_{233}\times\cat{D}_{12})\cdot\varepsilon
          \ar[rr,"s_{1223}\cdot\varepsilon"]
      &&\kappa_{123}\cdot(\cat{D}_{23}\times\kappa_{122})\cdot\varepsilon
      \\&\kappa_{123}
          \ar[ul,"\kappa_{123}\cdot(r_{23}\times\cat{D}_{12})"]
          \ar[ur,swap,"\kappa_{123}\cdot(\cat{D}_{23}\times l_{12})"]
      \end{tikzcd}
    \]
    In other words, applying this diagram to a pair of morphisms $f\colon e_1\to e_2$ and $g\colon e_2\to e_3$, we should obtain a commutative diagram in the category $\Hom_\cat{D}(e_1,e_3)$:
    \[
      \begin{tikzcd}
        (g\circ\id_e)\circ f
          \ar[rr,"{s(f,\id_e,g)}"]
  ...
  \item[(8)]
    ...
    \[
      \begin{tikzcd}
      & \bullet
          \ar[dl,swap,"\kappa_{125}\cdot(s_{2345}\times\cat{D}_{12})"]
          \ar[ddr,"s_{1235}\cdot(\kappa_{345}\times\cat{D}_{23}\times\cat{D}_{12})"]
      \\\bullet
          \ar[dd,swap,"s_{1245}\cdot(\cat{D}_{45}\times\kappa_{234}\times\cat{D}_{12})"]
      \\
      &&\bullet
          \ar[ddl,"s_{1345}\cdot(\cat{D}_{45}\times\cat{D}_{34}\times\kappa_{123})"]
      \\\bullet
          \ar[dr,swap,"\kappa_{145}\cdot(\cat{D}_{45}\times s_{1234})"]
      \\
      & \bullet
      \end{tikzcd}
    \]
    ...
    \[
      ...
      \end{aligned}
    \]
     we obtain a commutative diagram in $\Hom_\cat{D}(e_1,e_5)$:
     \[
       \begin{tikzcd}[column sep=huge, row sep=huge]
          (k\circ(h\circ g))\circ f
            \ar[dd,swap,"{s(f,h\circ g,k)}"]
        & ((k\circ h)\circ g)\circ f
            \ar[l,swap,"{s(g,h,k)\circ f}"]
            \ar[d,"{s(f,gk\circ h)}"]
        \\
        & (k\circ h)\circ(g\circ f)
            \ar[d,"{s(g\circ f,h,k)}"]
        \\k\circ((h\circ g)\circ f)
            \ar[r,swap,"{k\circ s(f,g,h)}"]
        & k\circ(h\circ(g\circ f))
       \end{tikzcd}
     \]
\end{enumerate}
    ...
    \[
      \begin{tikzcd}[row sep=huge,column sep=8em]
        F(h)\circ(F(g)\circ F(f))
          \ar[d,swap,"{F(h)\circ c(f,g)}"]
      & (F(h)\circ F(g))\circ F(f)
          \ar[l,swap,"{s(F(f),F(g),F(h))}"]
          \ar[d,"{c(g,h)\circ F(f)}"]
      \\F(h)\circ F(g\circ f)
          \ar[d,swap,"{c(g\circ f,h)}"]
      & F(h\circ g)\circ F(f)
          \ar[d,"{c(f,h\circ g)}"]
      \\F(h\circ(g\circ f))
      & F((h\circ g)\circ f)
...
Let $\cat{E}$ be a category endowed with a triple $\TT=(T,I,K)$, admitting finite fibre products and endowed with a canonical choice of these fibre products.
...
\begin{enumerate}
  ...
  \item[(3)]
    For each $e\in\set{\cat{E}}$, we define a $\TT$-span $(Ie,\id_e)\colon e\to e$, which we also denote by $\id_e$ if the context clearly specifies that it is a $\TT$-span.
...
\begin{proof}
  ...
  \[
    \begin{aligned}
      p
      &= s(\theta_2,\theta_3,\theta_4)\circ\theta_1
    \\p'
      &= s(\theta_1,\theta_2\circ\theta_3,\theta_4)
    \\p''
      &= \theta_4\circ s(\theta_1,\theta_2,\theta_3)
    \\q
      &= s(\theta_1,\theta_2,\theta_4\circ\theta_3)
    \\q'
      &= s(\theta_2\circ\theta_1,\theta_3,\theta_4).
    \end{aligned}
  \]
  ...
  \]
  combined with the fibre product $(r,m'_1\cdot r')$ of $(T(a_3\cdot u'_3),Ke_3\cdot Tb_2\cdot u_1)$ gives a crochet $q$ (a morphism in $\cat{E}$) characterised by
  \[
    \begin{aligned}
      r\cdot q
  ...
  \[
    ...
    \end{aligned}
  \]
  combined with the fibre product $(o,o')$ of $(Ta_4,Ke_4\cdot Tb_3\cdot m_1)$ give a crochet $q'$ (a morphism in $\cat{E}$) characterised by
  \oldpage{253}
  \[
    \begin{aligned}
      o\cdot q'
  ...
  \[
    ...
    \tag{3}
  \]
  (this is in fact the morphism in $\cat{E}$ that defines $s(\theta_2,\theta_3,\theta_4)$).
  Now the relations
  \[
    \begin{aligned}
  ...
  \]
  \oldpage{255}
  (this is in fact the morphism in $\cat{E}$ that defines $s(\theta_1,\theta_2,\theta_3)$).
...
We say that $\TT=(T,I,K)$ is a \emph{cartesian triple} if the two following conditions are satisfied:
\begin{enumerate}
  \item[(a)]
...
\begin{proof}
  ...
  \begin{align}
    ...
    &= \overline{a}\cdot c(\theta,1)\cdot (u\circ f^*\theta)\cdot lf^*\theta
    \tag{1}
  \\\overline{f}\cdot f^*l\theta
    &= \overline{f}\cdot c(\theta,1)\cdot (u\circ f^*\theta)\cdot lf^*\theta
    \tag{2}
  \\\overline{b}\cdot f^*l\theta
    &= \overline{b}\cdot c(\theta,1)\cdot (u\circ f^*\theta)\cdot lf^*\theta.
    \tag{3}
  \end{align}
  ...
  \]
  Finally, by the definition of $c(\theta,1)$, we have that
  \[
    \begin{aligned}
      \overline{a}\cdot c(\theta,1)
      &= Ke'\cdot Ta'\cdot v_{2\circ}
    \\\overline{f}\cdot c(\theta,1)
      &= g
    \\\overline{b}\cdot c(\theta,1)
  ...
  \[
    ...
    \\&\underset{(10)}{=} \overline{a}\cdot c(\theta,1)\cdot(u\circ f^*\theta)\cdot lf^*\theta
    \end{aligned}
  \]
  which proves (1).
  We have that
  \[
    ...
    \\&\underset{(10)}{=} f\cdot c(\theta,1)\cdot(u\circ f^*\theta)\cdot lf^*\theta
    \end{aligned}
  \]
  which proves (2).
  Finally,
  \[
    ...
    \\&\underset{(10)}{=} \overline{b}\cdot c(\theta,1)\cdot(u\circ f^*\theta)\cdot lf^*\theta
    \end{aligned}
  \]
  which proves (3).
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    \to \Gr{\TT^\mathbf{2}}.
  \]
  The way in which the adjoint of \hyperref[proposition:a.1]{Proposition~A.1} is constructed leads us to define $\set{F^\mathbf{2}}$ as the extension of the maps $\set{\hat{F}_f}$, where $f$ runs over $\set{\cat{E}^\mathbf{2}}$, and where we denote by $\hat{F}_f$ the adjoint of the forgetful functor
  \[
...
\begin{itenv}{Proposition~III.1.20}
\label{proposition:iii.1.20}
  If $\TT$ is a strongly cartesian triple, then so too is the triple induced by the tripleable functor $\Cat{Cat}(\TT)\to\Gr{\TT}$.
\end{itenv}

\begin{proof}
  If $\TT'=(T',I',K')$ is the induced triple on $\Gr{\TT}$, then the proof of \hyperref[proposition:a.1]{Proposition~A.1} in the appendix shows that, for all $\theta\in\set{\Gr{\TT}}$, $T'\theta$ is a countable sum, $I'\theta$ is a canonical injection of this sum, and $K'\theta$ is a crochet of the sum $T'T'\theta$;
  we thus immediately deduce that $\TT'$ is cartesian.
  \oldpage{270}
  The rest of the proposition is proven by using the commutativity of projective limits with themselves (fibre products) and the commutativity of inductive limits with themselves (sums).
\end{proof}
...
Consider the identity triple $(\id_\cat{E},\id_\cat{E},\id_\cat{E})$ on $\cat{E}$, we denote it simply by $\cat{E}$ in this section.
The category $\Cat{Cat}(\cat{E})$ of $\cat{E}$-categories is precisely what is usually referred to by that name in the literature (or ``$\cat{E}$-structured category'' in \cite{Eh}).
The axioms given in \cref{sec:I.3} then become:
...
\[
  ...
  \tag{8}
\]
where $(v_1,v_2)$ and $(w_1,w_2)$ are the fibre products of $(a,b)$ and $(v_2,v_1)$ respectively.
Analogous remarks can be made for $\Gr{\cat{E}}$, $\Ord{\cat{E}}$, \ldots.
The category $\Alg{\cat{E}}$ is isomorphic to $\cat{E}$, and the category $\Equ{\cat{E}}$ defined in \cref{sec:I.3} is precisely the category of equivalence $\cat{E}$-relations.
...
\oldpage{271}
We denote, in principle, by $\Cat{Cat}$, $\Cat{Gr}$, \ldots, the categories of categories, of graphs, of \ldots (respectively), with these structures being defined in a ``local'' manner (\cref{sec:I.1}).
We then have known injective functors $\Cat{Cat}(\Cat{Set})\to\Cat{Cat}$, $\Gr{\Cat{Set}}\to\Cat{Gr}$, \ldots, which are equivalences of categories.
The disjunction convention made in axiom~(2) of \cref{sec:I.1} allows us to identify $\Cat{Cat}(\Cat{Set})$ with $\Cat{Cat}$, $\Gr{\Cat{Set}}$ with $\Cat{Gr}$, \ldots;
but, although this will be useful, context will make precise whether the notation $\Cat{Cat}$, $\Cat{Gr}$, \ldots refer to the ``local'' or ``global'' definitions.
Similarly, $\Cat{Ord}$ and $\Cat{Equ}$ denote the categories $\Ord{\Cat{Set}}$ and $\Equ{\Cat{Set}}$.
...
\]
Such a sequence is also called a sequence of ``consecutive'' morphisms.
The integer $n$ is called the \emph{length} of the path, for simplicity, the path is denoted simply by $F$.
Note that, if $n=0$, then this definition implies that $e=e'$ ($=e_0$) and $F=\varnothing$ is the empty sequence: we denote by $\id_e$ this path $\varnothing\colon e\to e$ of length zero.
Next,
\[
  I\cat{G}\colon\cat{G}\to N\cat{G}
  \textand
  K\cat{G}\colon NN\cat{G}\to N\cat{G}
\]
are the morphisms of graphs such that
\[
  \set{I\cat{G}}
  = \set{K\cat{G}}
  = id_{\set{\cat{G}}}
\]
and defined on morphisms in the following way:
$I\cat{G}$ associates, to each morphism $f\colon e\to e'$, the path $(f)\colon e\to e'$ of length 1, denoted sometimes simply by $f\colon e\to e'$;
$K\cat{G}$ associates, to each path $\phi\colon e\to e'$ in $N\cat{G}$, the path $\phi\colon e\to e'$ in $\cat{G}$ given by ``erasing the parentheses'' of $\phi$ (or by ``juxtaposition'').
\oldpage{272}
In other words, if $\phi=(F_p,\ldots,F_1)$, where
\[
  F_i
  = (f_{i,n_i}, \ldots, f_{i,1})
\]
for $1\leqslant i\leqslant p$, then
\[
  F
  = (f_{p,n_p},\ldots,f_{p,1},f_{p-1,n_{p-1}},\ldots,f_{1,1}).
\]
The notation of this triple $\NN$ can be confused with that for the set of positive or zero integers;
if such a confusion risks arising, then we can always identify this set with $N1$.

We sometimes denote by the same symbol a $\TT$-category and its underlying $\TT$-graph;
similarly, we also denote by $N\cat{G}$ the free category structure on the graph $N\cat{G}$ defined by the free $\NN$-algebra $K\cat{G}$;
we call these the \emph{category} and \emph{graph of paths in $\cat{G}$} (respectively).
We call $\NN$ the \emph{path triple}.

\begin{itenv}{Proposition~III.2.21}
\label{proposition:iii.2.21}
  Let $\TT=(T,I,K)$ be a cartesian triple on $\cat{E}$ and let $\theta=(b,a,i,k)$ be a $\TT$-category on $e\in\set{\cat{E}}$;
  then
  \[
    (Tb, Ke\cdot Ta, Ti, Tk)
  \]
  is an $\cat{E}$-category on $Te$.
\end{itenv}

\begin{proof}
  $(Tb,Ke\cdot Ta,Ti)$ is a pointed $\cat{E}$-graph, since
  \[
    Tb\cdot Ti = Te
    \textand
    Ke\cdot Ta\cdot Ti = Ke\cdot Ie = Te.
  \]
  Let $(v_1,v_2)$ and $(w_1,w_2)$ be the fibre products associated to $\theta$;
  since the cartesian morphisms of $\cat{E}^\mathbf{2}$ are stable under composition, $(Tv_1,K\pi\cdot Tv_2)$ and $(Pw_1,K\pi_2\cdot Tw_2)$ are the fibre products of
  \[
    (Ke\cdot Ta, Tb)
    \textand
    (K\pi\cdot Tv_2, Tv_1)
  \]
  respectively.
  If we replace $(b,a,i,k,i_1,i_2,k_1,k_2)$ by
  \[
    (Tb, Ke\cdot Ta, Ti, Tk, Ti_1, Ti_2, Tk_1, Tk_2)
  \]
...
\begin{itenv}{Scholium}
  If $\TT$ is a strongly cartesian triple, then the free $\TT$-category $\hat{\theta}=(\hat{b},\hat{a},\hat{i},\hat{k})$ generated by a $\TT$-graph $\theta=(b,a)$ on $e\in\set{\cat{E}}$ has the same objects object $e$ as $\theta$.
  The morphism object $\hat{\pi}$ of $\hat{\theta}$ is the sum of the morphism objects $\pi_n$ for $n\in\mathbb{N}$ that we define by induction: $\pi_0=e$, $\pi_1=\pi$, and $\pi_n$ for $n\geqslant2$ is the source of the fibre product
  \[
    \begin{tikzcd}
      \pi'_{n-1}
        \ar[d,swap,"b'_{n-1}"]
    ...
    \end{tikzcd}
  \]
  where $\theta'=(b',a')$ is the $\cat{E}$-graph on $Te$ such that $b'=Tb$ and $a'=Ke\cdot Ta$, and where $\pi'_{n-1}$ is the morphism object of the $\cat{E}$-graph $\bigcirc^n\theta'=(b'_n,a'_n)$.
\end{itenv}
...
a morphism $\MM$ of triples of the form $(\id_\cat{E},m)\colon\TT'\to\TT$ is defined by a natural transformation $m\colon T\to T'$ such that
\[
  m\cdot I = I'
  \textand
  m\cdot K = K'\cdot mm
\]
where $mm=mT'\cdot Tm=T'm\cdot mT$ (\cref{sec:I.2}).
We say that $\MM$ is \emph{cartesian} if $\TT$ and $\TT'$ are cartesian and $m$ is cartesian.
...
\begin{proof}
  We will first construct an adjoint
  \[
    \Cat{Gr}'(\MM)\colon
    \Gr{\TT}
    \to \Gr{\TT'}
  \]
  to the functor $\Gr{\MM}\colon\Gr{\TT'}\to\Gr{\TT}$.
  Let $\sigma=(b,a)$ be a $\TT$-graph on $e\in\set{\cat{E}}$.
  Set $a'=me\cdot a$;
  then $\sigma'=(b,a')$ is a $\TT'$-graph, and its image under $\Gr{\MM}$ is a $\TT$-graph of the form $\hat{\sigma}=(\hat{b},\hat{a})$, where $(m',\hat{a})$ is the canonical fibre product of $(a',me)$, and $\hat{b}=b\cdot m'$.
  We have a crochet $j$ characterised by
  \[
    m'\cdot j = \id_\pi
    \textand
    \hat{a}\cdot j = a
  ...
  \[
    ...
    \qquad\text{where }f'=m''\cdot f
  \]
  is a morphism of $\TT'$-graphs, and, if $(f_0,\hat{f})\colon\hat{\sigma}\to\hat{\sigma}_1$ is its image under $\Gr{\MM}$, then we easily see that $\hat{f}\cdot j=f$ in $\cat{E}$, and so
  \[
    (f_0,\hat{f})\cdot(\id_e,m')
    = (f_0,f)
  \]
  in $\Gr{\TT}$, as we can see by using the fibre product $(m'',\hat{a}_1)$ of $(a'_1,me_1)$.
  The uniqueness of the morphism $f'$ in $\cat{E}$ is guaranteed, since we have that
  \[
    \begin{aligned}
      f'
      &= f'\cdot m'\cdot j
    ...
    \end{aligned}
  \]
  and that $\Gr{\MM}$ is faithful.

  Now consider a $\TT$-category $\theta=(b,a,i,k)$ on $e$, and set, as above, $\sigma=(b,a)$.
  If $(v_1,v_2)$ and $(w_1,w_2)$ are the canonical fibre products of $(a,Tb)$ and $(v_2,Tv_1)$ respectively, then $(v_1,m\pi\cdot v_2)$ and $(w_1,m\pi_2\cdot w_2)$ are the fibre products of $(a',T'b)$ and $(m\pi\cdot v_2,T'v_1)$ respectively, since $m$ is cartesian by hypothesis.
  \oldpage{275}
...
\begin{itenv}{Corollary}
  For every cartesian triple $\TT$ on $\cat{E}$, we have a functor that is both a section and an adjoint of the functor $\Cat{Cat}(\II)$; to every $\cat{E}$-category $\theta=(b,a,i,k)$ it associates a $\TT$-category $\theta'=(b,Ie\cdot a,i,k)$.
\end{itenv}

\begin{proof}
  If, in the previous proposition, we further suppose that $m$ is a monomorphism, then we note that the adjoints of the functors $\Cat{Cat}(\MM)$ and $\Gr{\MM}$ become sections of these functors, since $(m',\hat{a})=(\id_\pi,a)$ in this case.
  But, if $I$ is cartesian, then $(Ie,Ie)$ is a fibre product of $(ITe,TIe)$, and so $Ie$ is a monomorphism, since, for example, the relation $Ke\cdot ITe=Te$ shows that $ITe$ is a monomorphism.
\end{proof}
...
An $\NN$-graph $\cat{A}=(b,a)$ on $\cat{A}^\circ$ is given by a diagram in $\Cat{Gr}$
\oldpage{277}
\[
  \begin{tikzcd}
    N\cat{A}^\circ
  ...
  \end{tikzcd}
\]
It projects, under the forgetful functor $\Cat{Gr}\to\Cat{Set}$, onto a graph that we will denote by $\cat{A}^\cdot=(b^\cdot,a^\cdot)$, by setting
\[
  \begin{gathered}
    b^\cdot = \set{b}
  \\a^\cdot = \set{a}
  \\\set{\cat{A}} = \set{\cat{A}^\circ} = \set{\cat{A}^\cdot}
  \end{gathered}
  \qquad\qquad
  \begin{tikzcd}[column sep=huge]
    {\set{\cat{A}}}
    & {\set{\pi}}
      \ar[l,bend left=20,"b^\cdot"]
      \ar[l,bend right=20,swap,"a^\cdot"]
  \end{tikzcd}
\]
and this set is called the \emph{set of objects of $\cat{A}$}.
The morphisms from $\cat{A}^\cdot$ and from $\cat{A}^\circ$ are called the \emph{morphisms} and \emph{tangles} (or arrows, or promorphisms) respectively.
The objects of the graph $\pi=(b_\pi,a_\pi)$ are precisely the morphisms of $\cat{A}$, and its morphisms are called the \emph{branches} (or multimorphisms) of $\cat{A}$.
We say that $\alpha\colon F\to G$ is a \emph{tree of height $q\geqslant1$} if
\[
  F = (f_1,\ldots,f_q)
  \textand
  G = (g_1,\ldots,g_q)
\]
are paths of length $q$ in $\cat{A}^\cdot$ and if $\alpha=(\beta,\gamma_2,\ldots,\gamma_q)$ is a sequence, where $\beta\colon f_1\to g_1$ is a branch in $\cat{A}$, that we call the \emph{trunk} of the tree $\alpha$, and where $\gamma_h\colon f_h\to g_h$, for all $2\leqslant h\leqslant q$, is a path in $\pi$, so that, for all $2\leqslant h\leqslant q-1$, we have
\[
  K\cat{A}^\circ(Na(\gamma_h))
  = Nb(\gamma_{h+1})
\]
and, furthermore,
\[
  ...
  = Nb(\gamma_1).
\]
We extend this definition, for $q=0$, in the following way: to every tangle $j\colon e\to e'$ of $\cat{A}$, we associated a tree of height zero, which we denote by $\id^j\colon\id_e\to\id_{e'}$, where $\id_e$ and $\id_{e'}$ are the paths of length zero.

Trees thus form the morphisms of a graph $\hat{\pi}=(b_{\hat{\pi}},a_{\hat{\pi}})$, whose objects are the paths in $\cat{A}^\cdot$, and we have a morphism $I'\colon\pi\to\hat{\pi}$ in $\Cat{Gr}$, which is the identity on the objects and sends branches in $\cat{A}$ to trees that consist only of their trunk: we thus identity branches with particular trees.
We thus have, in $\Cat{Gr}$, a commutative diagram
\oldpage{278}
\[
  \begin{tikzcd}
    N\cat{A}^\circ
  \\&\pi
      ...
      \ar[dl,swap,"b"]
    & \hat{\pi}
      \ar[ull,bend right=20,swap,"\hat{a}"]
      \ar[dll,bend left=20,"\hat{b}"]
  \\\cat{A}^\circ
  \end{tikzcd}
\]
which projects in $\Cat{Set}$ to the diagram
\[
  \begin{tikzcd}[column sep=huge]
    {\set{\cat{A}}}
    & {\set{\pi}}
      \ar[r,"{\set{I'}}"]
      \ar[l,bend right=20,swap,near start,"a^\cdot"]
      \ar[l,bend left=20,near start,"b^\cdot"]
    & {\set{\hat{\pi}}}
      \ar[ll,bend right=30,swap,"{\set{\hat{a}}}"]
      \ar[ll,bend left=30,"{\set{\hat{b}}}"]
  \end{tikzcd}
\]
which represents the adjunction morphism $I\cat{A}^\cdot\colon\cat{A}^\cdot\to N\cat{A}^\cdot$ in $\Cat{Gr}$.
Denoting by $N'\cat{A}$ the $\NN$-graph $(\hat{b},\hat{a})$ thus obtained:

\begin{quote}
  \itshape
  for every tree of height~$q\geqslant2$, denoted in brief by $(\beta,\gamma_2,\ldots,\gamma_2)$,
  \[
    \hat{b}(\beta,\gamma_2,\ldots,\gamma_q)
    = b(\beta)
    \textand
    \hat{a}(\beta,\gamma_2,\ldots,\gamma_q)
    = K\cat{A}^\circ(Na(\gamma_q));
  \]
  for every tree of height~$1$,
  \[
    \hat{b}(\beta) = b(\beta)
    \textand
    \hat{a}(\beta) = a(\beta);
  \]
  finally, for every tangle $j$ of $\cat{A}$,
  \[
    \hat{b}(\id^j)
    = j
    = \hat{a}(\id^j).
...
We can recognise the construction of the underlying $\NN$-graph of the free $\NN$-category generated by the $\NN$-graph $\cat{A}$, by following the method of the scholium of \hyperref[proposition:iii.2.21]{Proposition~III.2.21}.
What we have called a ``tree'' does not even deserve this name, but rather ``a bundle of sticks'': it is the scholium that allows us to cut up into pieces the morphisms, rather complicated to describe, of the graph that is the object of the morphisms of the $\NN$-graph of paths of length~$q\geqslant0$ of $\cat{A}$ (see \cref{sec:III.2});
such morphisms would be difficult to deal with, as we realise by trying to redo the below construction of the free $\NN$-category $(\hat{b},\hat{a},\hat{i},\hat{k})$ generated by $(b,a)$ by using a more direct definition of trees (for example: checking the associativity of composition of trees).

For every tangle $j\colon e\to e'$ of $\cat{A}$ (the tangles of $\cat{A}$ are the same as those of $N'\cat{A}$), we have defined a tree of height zero $\id^j\colon\id_e\to\id_{e'}$;
we define, by this correspondence, the morphism $\hat{i}\colon\cat{A}^\circ\to\hat{\pi}$ in $\Cat{Gr}$.
\oldpage{279}
The morphism $\hat{k}\colon\hat{\pi}_2\to\hat{\pi}$ is the identity on objects and sends a tree
\[
  (\alpha,(\alpha_1,\ldots,\alpha_n))
  \colon (F,F') \to (G,G')
\]
of $N'\cat{A}=(\hat{b},\hat{a})$ of height~2 to a tree in $\cat{A}$ that we denote by
\[
  \alpha\cdot(\alpha_1,\ldots,\alpha_n)
  \colon F\cdot F' \to G\cdot G'
\]
where $\alpha\cdot(\alpha_1,\ldots,\alpha_n)$ is called the \emph{composite} of $(\alpha,(\alpha_1,\ldots,\alpha_n))$ and is the tree in $\cat{A}$ defined by $(\beta,\gamma_1,\ldots,\gamma_{q+q'})$, where
\[
  \alpha = (\beta,\gamma_1,\ldots,\gamma_q)
  \textand
  \alpha_i = (\beta_i,\gamma_{i1},\ldots,\gamma_{iq'})
\]
for $1\leqslant i\leqslant q'$ (where $q'$ is the common height of the trees $\alpha_i$) when $q,q'\geqslant1$.
(It is easy to describe the composition with trees of height~zero by taking into account the identity conditions).
It is not useful to show that the quadruple $(\hat{b},\hat{a},\hat{i},\hat{k})$, which we have just defined, is an $\NN$-category;
we have only applied the general propositions of \cref{sec:III.1} and given a description with the help of the scholium.
We leave to the reader the task of spelling out the construction of $K'\cat{A}$ (which would be impractical without the scholium).

This description will help us in the ``local'' definition that follows, by applying it to a particular case (this phenomenon is, in reality, very general. For example, the ``local'' definition of a category $\cat{C}$ given in \cref{sec:I.1} makes tacit use of a category in the ``global'' sense: the ``groupoid of pairs'' $\set{\cat{C}}\times\set{\cat{C}}$ that plays the same role as, below, the ``multicategory of $n$-envelopes'' for $n\in\mathbb{N}$).
In the following, we will take the equivalence between the ``local'' and ``global'' definitions as evident.

A \emph{multicategory} is a quintuple $\cat{A}=(\cat{A}^\circ,\cat{A}^\cdot,\Hom_{\cat{A}},\iota,\kappa)$ that satisfies conditions~1 to 6 below:

\begin{enumerate}
  \item
    $\cat{A}^\circ$ and $\cat{A}^\cdot$ are graphs such that $\set{\cat{A}^\circ}=\set{\cat{A}^\cdot}$ (we suppose, to simplify notation, that they do not have any morphisms in common).
    We say that
    \[
      ((j_1,\ldots,j_n),j)
      \colon f\to f'
    \]
    is an \emph{envelope of $\cat{A}$} (or simply of $(\cat{A}^\circ,\cat{A}^\cdot)$) if $f\colon\overline{e}\to e$ and $f'\colon\overline{e}'\to e'$ are morphisms in $\cat{A}^\cdot$ and if $j_i\colon e_{i-1}\to e_i$, for $1\leqslant i\leqslant n$, are morphisms in $\cat{A}^\circ$ such that $e_0=e$ and $e_n=e'$.
    \oldpage{280}
    We also denote the above envelope by $(f,((j_1,\ldots,j_n),j),f')$.
    We denote by $\sq\cat{A}$ the set of envelopes of $\cat{A}$.
    \[
      \begin{tikzcd}
        \overline{e}'
          \ar[d]
    ...
    We define an $\NN$-graph $(b,a)$ on $\cat{A}^\circ$, in the evident way, having $\sq\cat{A}$ as its morphism object, by associating to the above envelope $j$ and $(j_1,\ldots,j_n)$ as source and target (respectively).
    We denote by $\hsq\cat{A}$ the morphism object of the free $\NN$-graph $N(b,a)$.
    Then $\hsq\cat{A}$ is a graph;
    its morphisms are the trees in $(b,a)$;
    an $n$-envelope is a tree of height~$n$ in $(b,a)$.

  \item
    $\Hom_\cat{A}$ is a family of sets indexed by $\sq\cat{A}$.
    The relation
    \[
      \alpha\in \Hom_\cat{A}\big( f,((j_1,\ldots,j_n),j),f' \big)
    \]
    is also written as a multimorphism (or branch) $\alpha\colon(j_1,\ldots,j_n)\to j\colon f\mapsto f'$ of $\cat{A}$.

  \item
    $\iota$ is a family of maps, indexed by the set of morphisms of $\cat{A}^\circ$, of the form $\{\varnothing\}\to\Hom(\id^j)$ for every morphism $j\colon e\to e'$ of $\cat{A}^\circ$, where $\id^j$ is the envelope $(\id_{e'},(j,j),\id_e)$.
    We also denote by $\id^j$, despite the risk of confusion, the image of $\varnothing$ under $\iota(j)$.

  \item
    $\kappa$ is a family of maps, indexed by the set of 2-envelopes of $\cat{A}$, of the form
    \[
      \Hom_\cat{A}(E)\times\Hom_\cat{A}(E_1,\ldots,E_n)
      \to \Hom_\cat{A}(E,(E_1,\ldots,E_n))
    \]
    where $(E,(E_1,\ldots,E_n))$ is a 2-envelope of $\cat{A}$, and where we set
    \[
      \Hom_\cat{A}(E_1,\ldots,E_n)
      = \Hom_\cat{A}(E_1)\times\ldots\times\Hom_\cat{A}(E_n).
    \]
    The image of an element $(\alpha,(\alpha_1,\ldots,\alpha_n))$ under this map is called the \emph{composite} of $(\alpha,(\alpha_1,\ldots,\alpha_n))$ in $\cat{A}$, and denoted by $\alpha\cdot(\alpha_1,\ldots,\alpha_n)$.

  \item
    (Identity).
    For every morphism $\alpha\colon(j_1,\ldots,j_n)\to j\colon f\mapsto f'$ in $\cat{A}$, we have that
    \[
      \id^j\cdot\alpha
      = \alpha
      = \alpha\cdot(\id^{j_1},\ldots,\id^{j_n}).
    \]
  ...
  \item
    ...
    \[
      (E,(E_1,\ldots,E_n))
      \textand
      (E_i,(E_{i1},\ldots,E_{ip_i}))
    \]
    (for $1\leqslant i\leqslant n$) be 2-envelopes of $\cat{A}$;
    if
    \[
      \begin{aligned}
        \alpha &\in\Hom_\cat{A}(E)
      \\\beta_i &\in\Hom_\cat{A}(E_i)
      \\\gamma_{ik} &\in\Hom_\cat{A}(E_{ik})
      \end{aligned}
    \]
    for $1\leqslant i\leqslant n$ and $1\leqslant k\leqslant p_i$, then
    \[
      \begin{aligned}
        &\alpha\cdot \big( \beta_1\cdot(\gamma_{11},\ldots,\gamma_{1p_1}), \ldots, \beta_n\cdot(\gamma_{n1},\ldots,\gamma_{np_n}) \big)
      \\= \big( &\alpha\cdot(b_1,\ldots,\beta_n) \big)\cdot(\gamma_{11},\ldots,\gamma_{1p_1},\gamma_{21},\ldots,\gamma_{2p_2},\ldots,\gamma_{n1},\ldots,\gamma_{np_n}).
      \end{aligned}
    \]
\end{enumerate}

A triple $(\cat{A}^\circ,\cat{A}^\cdot,\Hom_\cat{A})$ that satisfies conditions~1 and 2 is called a \emph{multigraph}.

We will show that this structure generalises that of Lambek \cite{La} of the same name, extracting from it the essential ideas.
We define a \emph{Lambek multicategory} to be a quadruple $\cat{L}=(\cat{L}^\circ,\Hom_\cat{L},\id,\perp)$ such that

\begin{enumerate}
  \item
    $\cat{L}^\circ$ is a graph.
    We say that $((j_1,\ldots,j_n),j)\colon e\to e'$ is an \emph{envelope of $\cat{L}$} if $j\colon e\to e'$ is a morphism in $\cat{L}^\circ$ whenever $(j_1,\ldots,j_n)\colon e\to e'$ is a path in $\cat{L}^\circ$;
    the integer $n\in\mathbb{N}$ is called the \emph{length} of the envelope.
    We denote by $\Delta_n\cat{L}$ the set of envelopes of length~$n$ of $\cat{L}$, and by $\Delta\cat{L}$ the set of all envelopes of $\cat{L}$.

  \item
    $\Hom_\cat{L}$ is a family of sets, indexed by $\Delta\cat{L}$.
    The relation $\alpha\in\Hom_\cat{L}((j_1,\ldots,j_n),j)$ is also expressed by saying that $\alpha\colon(j_1,\ldots,j_n)\to j$ is a multimorphism in $\cat{L}$.

  \item
    $\id$ is a family that, to each morphism $j\colon e\to e'$ in $\cat{L}^\circ$, associates a multimorphism of the form $\id^j\colon j\to j$.

  \item
    $\perp=(\perp_n^i \mid 1\leqslant i, n\in\mathbb{N})$, where $\perp_n^i$ is a family of maps, indexed by the set of pairs of envelopes $(E,E')$, such that, if
    \[
      E = ((j_1,\ldots,j_n),j)
      \textand
      E' = ((j'_1,\ldots,j'_n),j')
    \]
    then $j_i=j'$ (and thus $n\geqslant 1$), with these maps being of the form
    \[
      \perp_n^i(E',E)
      \colon \Hom_\cat{L}(E)\times\Hom_\cat{L}(E')
      \to \Hom_\cat{L}(E\perp E')
    \]
    where we define
    \[
      E\perp E'
      = ((j_1,\ldots,j_{i-1},j'_1,\ldots,j'_n,\ldots,j_{i+1},\ldots,j_n),j).
    \]

    \oldpage{282}
    We again note here that the ``local'' definition of a structure uses the prerequisite definition of a particular structure --- the $\perp$ law on envelopes --- which, in the work of Lambek, does not appear, thanks to a judicious abuse of language in which he denotes by the same symbol $\perp$ all the composition laws $\perp_n^i$, which we ourselves do when employing the notation $E\perp E'$, and which we will do in the rest of this text, even for $\cat{L}$.
    Thus, we pose the abuse of notation $\perp=\perp_n^i(E,E')$.

  \item
    We have that
    \[
      \id^j\perp\alpha = \alpha = \alpha\perp\id^{j_i}
    \]
    for every multimorphism $\alpha\colon(j_1,\ldots,j_n)\to j$ and every integer $1\leqslant i\leqslant n$.

  \item
    We have that $\alpha\perp(\beta\perp\gamma)=(\alpha\perp\beta)\perp\gamma$ for every triple of multimorphisms $(\alpha,\beta,\gamma)$ of the form
    \[
      \begin{aligned}
        \alpha\colon
        &(j_1,\ldots,j_n)
      ...
      \\\beta\colon
        &(j'_1,\ldots,j'_{n'})
        \to j_i
        \quad\text{where }1\leqslant i\leqslant n-1
      \\\gamma\colon
        &(j''_1,\ldots,j''_{n''})
        \to j'_{i'}
        \quad\text{where }1\leqslant i'\leqslant n'-1.
  ...
  \item
    We have that $(\alpha\perp\beta)\perp\beta'=(\alpha\perp\beta')\perp\beta$ for every triplet $(\alpha,\beta,\beta')$ of the form
    \[
      \begin{aligned}
        \alpha\colon
        &(j_1,\ldots,j_n)
      ...
      \\\beta'\colon
        &(j''_1,\ldots,j''_{n''})
        \to j_{i'}
        \quad\text{where }1\leqslant i'\leqslant n'-1\text{ and }i<i'.
      \end{aligned}
    \]
\end{enumerate}

\begin{itenv}{Proposition~III.3.23}
  The data of a Lambek multicategory is equivalent to that of a multicategory $\cat{A}$ such that $\cat{A}^\cdot$ is a discrete category.
\end{itenv}

\begin{proof}
  Let $\cat{L}$ be a Lambek multicategory;
  we associate to it a multicategory $\cat{A}$ defined in the following way: $\cat{A}^\circ=\cat{L}^\circ$, $\cat{A}^\cdot=\set{\cat{L}^\circ}$ (by identifying every set with a discrete category), $\Hom_\cat{A}=\Hom_\cat{L}$ (by identifying in the evident way the envelopes of $\cat{L}$ with the envelopes of $(\cat{A}^\circ,\cat{A}^\cdot)$).
  If $(\alpha,(\beta_1,\ldots,\beta_n))$ is a tree of height~$2$ of the multigraph $(\cat{A}^\circ,\cat{A}^\cdot,\Hom_\cat{A})$ thus defined, we define the composition of $\cat{A}$ by
  \[
    \alpha\cdot(\beta_1,\ldots,\beta_n)
    = \overline{\beta}_n
    \quad\text{if }\overline{\beta}_0=\alpha
  \]
  and, by induction on $i$,
  \oldpage{283}
  \[
    \overline{\beta}_{i+1}
    = \overline{\beta}_i \perp \beta_i
    \quad\text{for }0\leqslant i\leqslant n-1.
  \]
  Axiom~(5) is evidently a consequence of (5').
  Axiom~(6) is a consequence of axioms~(6') and (7').
  The formula of (6) is difficult to write, which is why we prove only the following particular case:
  \[
    \alpha\cdot\big( \beta\cdot(\gamma_1,\gamma_2), \beta'\cdot(\gamma'_1,\gamma'_2) \big)
    = \big( \alpha\cdot(\beta,\beta')\big) \cdot (\gamma_1,\gamma_2,\gamma'_1,\gamma'_2)
  \]
  and the general formula and its proof only differ by the introduction of a certain quantity of dotted lines, which we indicate in the figure below.

  \[
    \begin{aligned}
      \alpha\cdot \big( \beta\cdot(\gamma_1,\gamma_2), \beta'\cdot(\gamma'_1,\gamma'_2) \big)
    &\overset{\text{defn.}}{=}
      \Big(\alpha\perp\big( (\beta\perp\gamma_1)\perp\gamma_2 \big)\Big) \perp \big( (\beta'\perp\gamma'_1)\perp\gamma_2 \big)
  \\&\overset{\text{(6')}}{=}
      \Big( \big( \alpha\perp((\beta\perp\gamma_1)\perp\gamma_2) \big) \perp (\beta'\perp\gamma_1) \Big) \perp\gamma'_2
  \\&\overset{\text{(6')}}{=}
      \Big( \Big( \big(\alpha\perp((\beta\perp\gamma_1)\perp\gamma_2) \big) \perp \beta'\Big) \perp\gamma_1 \Big) \perp\gamma'_2
  \\&\overset{\text{(7')}}{=}
      \Big( \Big( (\alpha\perp\beta')\perp\big((\beta\perp\gamma_1)\perp\gamma_2\big) \Big)\perp\gamma'_1 \Big)\perp\gamma'_2
  \\&\overset{\text{(6')}}{=}
      \Big( \Big( \big( (\alpha\perp\beta')\perp(\beta\perp\gamma_1) \big) \perp\gamma_2 \Big) \perp\gamma'_1 \Big) \perp\gamma'_2
  \\&\overset{\text{(6')}}{=}
      \Big( \Big( \big(\big( (\alpha\perp\beta') \perp \beta \big) \perp\gamma_1 \big) \perp\gamma_2 \Big) \perp\gamma'_1 \Big) \perp\gamma'_2
  \\&\overset{\text{(7')}}{=}
      \Big( \Big( \big( \big( (\alpha\perp\beta) \perp\beta' \big) \perp\gamma_1 \big) \perp\gamma_2 \Big) \perp\gamma'_1 \Big) \perp\gamma'_2
  \\&\overset{\text{defn.}}{=}
      \big( \alpha\cdot(\beta,\beta')\big) \cdot (\gamma_1,\gamma_2,\gamma'_1,\gamma'_2).
  ...
  we associate to it a Lambek multicategory in the following way: $\cat{L}^\circ=\cat{A}^\circ$, $\Hom_\cat{L}=\Hom_\cat{A}$, and we set
  \[
    \alpha \perp_n^i \beta
    = \alpha\perp\beta
    = \alpha\cdot(\id^{j_1},\ldots,\id^{j_{i-1}},\beta,\id^{j_{i+1}},\ldots,\id^{j_n})
  \]
  where
  \[
    \begin{aligned}
      \alpha\colon
      &(j_1,\ldots,j_n) \to j \colon \id_e\to\id_e
    \\\beta\colon
      &(j'_1,\ldots,j'_n) \to j_i \colon \id_{\overline{e}} \to \id_{\overline{e}'}
    \end{aligned}
  \]
  (where $1\leqslant i\leqslant n$) are multimorphisms in $\cat{A}$.
  Axioms~(5'), (6'), and (7') are simple consequences of (5) and (6).
  \oldpage{284}
  We have thus defined the stated equivalence.
\end{proof}

The ``multicategories'' in \cite{La} are precisely, up to terminology, Lambek multicategories $\cat{L}$ (as above) such that $\set{\cat{L}}=1$.

\begin{rmenv}{Examples}
  \begin{enumerate}
    \item
      Categories are particular cases of Lambek multicategories.
      If $(j_1,\ldots,j_n)\colon e\to e'$ is a path in the category $\cat{C}$ and $j\colon e\to e'$ a morphism in $\cat{C}$, then $\Hom_\cat{C}((j_1,\ldots,j_n),j)$ takes the value 1 or 0 depending on whether $j=j_1\cdot j_2\cdot\ldots\cdot j_n$ or not.
      ...
      \[
        \begin{tikzcd}[column sep=6em, row sep=large]
          \phantom{x}
            \ar[d,swap,"f'"]
        & \phantom{x}
            \ar[l,swap,"j_1\cdot j_2\cdot\ldots\cdot j_n"]
            \ar[d,"f"]
            \ar[dl,phantom,"\alpha"description]
        \\\phantom{x}
        & \phantom{x}
            \ar[l,"j"]
        \end{tikzcd}
    ...
    \item
      ...
      But we will limit ourselves here to a conjecture, since there is delicate work to be done on the ``coherences'', which would lead us to too much development: we will return to this in another work.
    ...
    \item
      Functors and the ``profunctors'' of Bénabou (relative to a universe) form the morphisms of two different graphs on the same set of objects, $\set{\Cat{Cat}}$.
      These are (respectively) the graph of morphisms and the graph of tangles (or promorphisms) of a multicategory;
      the multimorphisms of this multicategory correspond precisely to those that we call, in these particular cases, the ``multimorphisms'';
      this justifies, if justification were needed, the terminology introduced here.
  \end{enumerate}
\end{rmenv}

The first three examples suggest the following general definition:
If $\TT$ is an arbitrary triple on a category $\cat{E}$ that admits fibre products, then we call $(b,a,i,k,t,d)$ a \emph{pre-tensorial $\TT$-category} if $(b,a,i,k)$ is a $\TT$-category on an object $e$ and, adopting the notation of \todo{Figure~1} from \cref{sec:I.3}, $t\colon Te\to\pi$ and $d\colon\pi\to\pi_2$ are morphisms in $\cat{E}$ that satisfy the following axioms:
\begin{enumerate}
  \item[(1)] $a\cdot t = \id_{Te}$
  \item[(2)] $v_2\cdot d = I\pi\cdot t\cdot a$ and $k\cdot d = \id_\pi$.
\end{enumerate}
The first three examples are, in an evident way, pre-tensorial multicategories.
...
\subsection{Categories of operators}
\label{sec:III.4}

\oldpage{286}
We define a \emph{category of operators}, or \emph{action of a category}, to be the data of a triple $\phi=(\cat{C},\hom_\phi,\kappa)$ satisfying the following conditions:
\begin{enumerate}
  \item
    $\cat{C}$ is a category.

  \item
    $\hom_\phi$ is a family of sets indexed by $\set{\Cat{C}}$.

  \item
    $\kappa$ is a family of maps, indexed by $\set{\Cat{C}}^2$, of the form
    \[
      \kappa(e,e')\colon
      \Hom_\cat{C}(e,e')\times\hom_\phi(e)
      \to \hom_\phi(e').
    \]
    We denote by $f\cdot x$ the image under this map of a pair $(f,x)$, where $f\colon e\to e'$ is a morphism in $\cat{C}$ and $x\in\hom_\phi(e)$.
    (We sometimes suppose that the family $\hom_\phi$ takes disjoint values between them, disjoint from the values taken by $\Hom_\cat{C}$ and disjoint from $\cat{C}$).

  \item
    If $e\in\set{\Cat{C}}$ and $x\in\hom_\phi(e)$, then $\id_e\cdot x=x$.

  \item
    If $f\colon e\to e'$, $g\colon e'\to e''$, and $x\in\hom_\phi(e)$, then $g\cdot(f\cdot x)=(g\cdot f)\cdot x$.
\end{enumerate}
This ``local'' definition is visibly equivalent to that of a presheaf of sets.

If $\phi'=(\cat{C}',\hom_{\phi'},\kappa')$ is another category of operators, then a morphism $\varphi\colon\phi\to\phi'$ is defined by a functor $F\colon\cat{C}\to\cat{C}'$ and a family of maps $\hom_\varphi$, indexed by $\set{\cat{C}}$, of the form
\[
  \hom_\varphi(e)\colon
  \hom_\phi(e)
  \to \hom_{\phi'}(\varphi(e))
\]
and such that $\varphi(f\cdot x)=\varphi(f)\cdot\varphi(x)$, where we write $\varphi(e)$, $\varphi(f)$, and $\varphi(x)$ instead of $F(e)$, $F(f)$, and $\hom_\varphi(e)(x)$ (respectively).

The ``global'' definition of a category of operators coincides with that of a \emph{co-étale functor} (called a functor of hypermorphisms in the terminology of \cite{Eh} --- see, for example, the ``sketch'' of this structure in \cite{Bu}).
We say that a functor $\overline{\phi}\colon\overline{\cat{C}}\to\cat{C}$ is co-étale if, for every $\overline{e}\in\set{\overline{\cat{C}}}$ and every morphism in $\cat{C}$ of the form $f\colon\overline{\phi}(\overline{e})\to e'$, there exists a unique morphism in $\cat{C}$ of the form $\overline{f}\colon\overline{e}\to\overline{e}'$ such that $\overline{\phi}(\overline{f})=f$.
More generally, suppose that $\cat{E}$ is a category that admits finite fibre products;
then we say that an $\cat{E}$-functor $(f_0,f)\colon(\overline{b},\overline{a},\overline{i},\overline{k})\to(b,a,i,k)$ is \emph{co-étale} if $(f,\overline{a})$ is a fibre product of $(a,f_0)$ (compare with the generalisations of this notion in \cref{sec:iv}).
We denote by $\Op{\cat{E}}$ the full subcategory of $\Cat{Cat}(\cat{E})^\mathbf{2}$ whose objects are the co-étale $\cat{E}$-functors.

\oldpage{287}
Suppose that $\cat{E}$ admits finite projective limits and finite inductive limits, and is endowed with a canonical choice of these limits;
in particular, we have an initial object and a terminal object, which we denote by $0$ and $1$ (respectively).
We denote the finite sums in $\cat{E}$ additively.
Then $(\cat{E},0,+)$ is a monoidal category in an evident way, in which we have the following monad:
\[
  0 \xrightarrow{\varepsilon}
  I \xleftarrow{\delta}
  I + 1
\]
where $\varepsilon$ is uniquely determined and $\delta$ is the ``codiagonal'' of the sum of $(1,1)$.
This determines --- following a procedure that generalises to the data of an arbitrary monad --- a triple on $\cat{E}$, which we denote by
\[
  \TT_{+1}
  = (T_{+1},I,K)
\]
...
\[
  ...
  \end{aligned}
\]
up to isomorphism, for all $e\in\set{\cat{E}}$.

\begin{itenv}{Proposition~III.4.24}
  If the finite sums in $\cat{E}$ are universal, then the categories $\Op{\cat{E}}$ and $\Cat{Cat}(\TT_{+1})$ are equivalent.
\end{itenv}

\begin{proof}
  We define an \emph{$\cat{E}$-graph of operators} to be the data of a triple $(b,a,f_0)$, where $(b,a)$ is an $\cat{E}$-graph on $e$ and $f_0\colon\overline{e}\to e$ is a morphism in $\cat{E}$ with target $e$.
  We will first show that the category $\Gr{\TT_{+1}}$ is equivalent to the category whose objects are the $\cat{E}$-graphs of operators and whose morphisms are determined by triples
  \[
    (m_0,m,\overline{m})
    \colon (b,a,f_0)
    \to (b',a',f'_0)
  ...
  \oldpage{288}
  Now, if $(b',a')$ is a $\TT_{+1}$-graph, its data is equivalent to that of an $\cat{E}$-graph of operators $(b,a,f_0)$, where $b=b'\cdot I'$ and $f_0=b'\cdot j'$;
  in other words, $b'$ is the crochet of $(b,f_0)$ \unsure{with} the sum $(I',j')$.
  It thus follows that $\Gr{\TT_{+1}}$ is equivalent to the category of $\cat{E}$-graphs of operators defined above.

  Let $(b',a')$ be a $\TT_{+1}$-graph on $e$;
  we will construct the morphism $a$, $c$, $j$, $I'$, $j'$, $b$, and $f_0$ as above, and form the fibre products $(v'_1,v'_2)$, $(v_1,v_2)$, and $(f,\overline{a})$ of $(a',b'+1)$, $(a,b)$, and $(a,f_0)$ (respectively), with sources $\pi'_2$, $\pi_2$, and $\overline{\pi}$ (respectively).
  There exist morphisms $I''\colon\pi_2\to\pi'_2$ and $\overline{I}\colon\overline{\pi}\to\pi'_2$ characterised by the relations
  \[
    \begin{aligned}
      v'_1\cdot I''
      &= I'\cdot v_1
    \\v'_2\cdot I''
      &= I\pi'\cdot I'\cdot v_2
    \\v'_1\cdot\overline{I}
      &= I'\cdot f
    \\v'_2\cdot\overline{I}
  ...
  \[
    ...
    \textand
    a'\cdot k' = Ke\cdot(a'+1)\cdot v'_2;
  \]
  from this we extract the data of a ``multiplication'' $k\colon\pi_2\to\pi'$ on the $\cat{E}$-graph $(b,a)$, i.e a morphism $k$ such that
  \[
    b\cdot k = b\cdot v_1
    \textand
    a\cdot k = a\cdot v_2
  \]
  along with the data of a morphism $\overline{b}\colon\overline{\pi}\to\overline{e}$ such that $f_0\cdot\overline{b}=b\cdot f$.
  These two morphisms, $k$ and $\overline{b}$, arise as crochets of fibre products characterised by the relations
  \[
    \begin{aligned}
      I'\cdot k
      &= k'\cdot I''
  ...
  \]
  \oldpage{290}
  We can easily show that the data of the ``multiplication'' $k'$ is equivalent to that of the ``multiplication'' $k$ along with $\overline{b}$ such that
  \[
    (f_0,f)
    \colon (\overline{b},\overline{a})
    \to (b,a)
  \]
  is a morphism of $\cat{E}$-graphs.

  The rest of the proof follows this model;
  we limit ourselves to pointing out what else can be constructed from \unsure{just} the multiplication $k'$ on $(b',a')$.
  Let $(\overline{v}_1,\overline{v}_2)$ be the canonical fibre product of $(\overline{b},\overline{a})$ of source $\overline{\pi}_2$;
  we have a morphism $f_2$ characterised by
  \[
    v_1\cdot f_2 = f\cdot\overline{v}_1
    \textand
    v_2\cdot f_2 = f\cdot\overline{v}_2.
  \]
  The relations
  \[
    a\cdot k\cdot f_2
    = a\cdot v_2\cdot f_2
    = a\cdot f\cdot \overline{v}_2
    = f_0\cdot\overline{a}\cdot\overline{v}_2
  \]
  imply the existence of a crochet $\overline{k}$ characterised by
  \[
    f\cdot\overline{k} = k\cdot f_2
    \textand
    \overline{a}\cdot\overline{k} = \overline{a}\cdot\overline{v}_2.
    \qedhere
  \]
\end{proof}

We remark that $\TT_{+1}$ is cartesian but not strongly cartesian.



\subsection{Hypertopologies}
\label{sec:III.5}

We define \emph{hypertopologies} to be $\TT_\mathcal{U}$-categories, where $\TT_\mathcal{U}=(\mathcal{U},I,K)$ is the \emph{ultrafilter triple} on $\Cat{Set}$ (defined with different notation in \cite{Ma}).
For every set $E$, $\mathcal{U}E$ is the set of ultrafilters on $E$.
...
We can deduce trivial new examples by taking, for example, the product in $\Cat{Cat}(\TT_\mathcal{U})$ of a monoid and a topology.
We hope to show in a future article that we can endow $\Cat{Top}$ with the structure of a hypertopology that has an interpretation analogous to that of the 2-category structure on $\Cat{Cat}$.
...
\begin{itemize}
  \item the identity triple
  \item the path triple
  \item $\TT_{+1}$.
\end{itemize}
...
\oldpage{292}
Throughout this chapter, $\cat{E}$ denotes a category admitting finite fibre products and a terminal object.
...
\[
  \begin{tikzcd}
    ...
    &&& \bullet
      ...
      \ar[dd,"b"]
  \\&& \bullet
      \ar[ull,near start,"v_1"]
      \ar[dr,swap,near start,"v_2"]
  \\\bullet
    &&& \bullet
      \ar[lll,"f"]
  \end{tikzcd}
...
Let $\cat{R}$ be a subcategory of $\cat{E}$;
we say that $\cat{R}$ is \emph{stabilised} if it satisfies the three following conditions:
\begin{enumerate}
  \item[(i)] Every invertible morphism in $\cat{E}$ is a morphism in $\cat{R}$, which implies the relation $\set{\cat{R}}=\set{\cat{E}}$.

  \item[(ii)] If $f$ and $f'$ are morphisms in $\cat{R}$, and if $f''$ is a morphism in $\cat{E}$ such that $f'\cdot f''=f$ in $\cat{E}$, then $f''$ is a morphism in $\cat{R}$.

  \item[(iii)] If $(b',F,f,b)$ is a cartesian square in $\cat{E}$, and if $b'$ is a morphism in $\cat{R}$, then $b$ is also a morphism in $\cat{R}$.
\end{enumerate}

\begin{rmenv}{Examples}
  \begin{enumerate}
    \item[(1)] The morphisms of $\cat{R}$ are the invertible morphisms of $\cat{E}$.
    \item[(2)] The morphisms of $\cat{R}$ are the monomorphisms of $\cat{E}$.
    \item[(3)] The morphisms of $\cat{R}$ are the retractions of $\cat{E}$.
    \item[(4)] $\cat{R}=\cat{E}$.
  \end{enumerate}
\end{rmenv}

If $\cat{R}$ is a subcategory of $\cat{E}$ and $C=(b',F,f,b)$ is a square in $\cat{E}$, we say that $C$ is an \emph{$\cat{R}$-square} if $F$ and $f$ are morphisms in $\cat{R}$;
\oldpage{293}
we say that $C$ is a \emph{square-$\cat{R}$} is $b$ and $b'$ are morphisms in $\cat{R}$, and we say that $C$ is an \emph{$\cat{R}$-cartesian square} if its \unsure{indicator} is a morphism in $\cat{R}$.
Both $\cat{R}$-squares and squares-$\cat{R}$ always define subcategories of $\cat{E}^\mathbf{2}$;
the latter is a full subcategory.
If $\cat{R}$ satisfies property~(iii), it is immediate that the $\cat{R}$-cartesian squares define a subcategory of $\cat{E}^\mathbf{2}$.

\begin{itenv}{Lemma}
  If $\cat{R}$ is a stabilised subcategory of $\cat{E}$, and if we have a commutative diagram of the form
  \[
    \begin{tikzcd}[sep=large]
      \bullet
        \ar[d,swap,"b''"]
    ...
    \\\bullet
      &\bullet
        \ar[l,swap,"\overline{f}'"]
      &\bullet
        \ar[l,swap,"\overline{f}"]
    \end{tikzcd}
  \]
  such that the four following squares are $\cat{R}$-cartesian:
  \begin{enumerate}
    \item $(b'',F',f',b')$
    \item $(\overline{b}'',f',\overline{f}',\overline{b}')$
    \item $(\overline{b}',f,\overline{f},\overline{b})$
    \item $(\overline{b}''\cdot b'',F'\cdot F,\overline{f}'\cdot\overline{f},\overline{b}\cdot b)$
  \end{enumerate}
  then the fifth square $(b',F,f,b)$ is also $\cat{R}$-cartesian.
\end{itenv}

\begin{proof}
  First of all, note that properties~(ii) and (iii) imply the following property:
  \begin{enumerate}
    \item[(iv)] If $(C_1,C'_1,C_2,C'_2)$ is a cartesian square in $\cat{E}^\mathbf{2}$, and if $C_1$ and $C_2$ are squares-$\cat{R}$, then $C'_1$, $C'_2$, and $C_1\cdot C'_1$ are squares $\cat{R}$.
  \end{enumerate}

  This is an easy exercise on \unsure{indicators}.
  The lemma is then proven by using the properties~(i), (ii), (iii), and (iv).
\end{proof}

Let $\Gamma$ be the category consisting of three objects, denoted $0$, $1$, and $1'$, and two morphisms, denoted
\[
  \beta\colon0\to1
  \textand
  \alpha\colon0\to1'.
...
\[
  ...
  a_0\cdot c = a
\]
where $(b_0,a_0)$ is the canonical product of $(e,\overline{e})$, if $b\colon\pi\to e$ and $a\colon\pi\to\overline{e}$.
A morphism in $\cat{E}^\Gamma$ can be identified with a pair $(C,\overline{C})$ of squares in $\cat{E}$ such that, if
\[
  C = (b',F,f,b)
  \textand
  \overline{C} = (a',\overline{F},\overline{f},a),
\]
then $F=\overline{F}$.
We define the \emph{source \unsure{indicator}} and \emph{target \unsure{indicator}} of $(C,\overline{C})$ to be the \unsure{indicators} $\overline{j}$ and $j$, respectively, of the squares $\overline{C}$ and $C$.
...
\begin{proof}
  In the ``source'' and ``target'' cases, this is a consequence of the fact that the squares in $\cat{E}$ whose \unsure{indicator} is in $\cat{R}$ define a stabilised subcategory of $\cat{E}^\mathbf{2}$.
  In the ``central'' case, this is an immediate consequence of the lemma, since every cartesian square is $\cat{R}$-cartesian, by~(i).
\end{proof}

\begin{rmenv}{Remark}
  The morphisms in the category $\cat{E}^\Gamma$ can be identified with the morphisms in the pseudo-category $\Sp{\cat{E^\mathbf{2}}}$.
  \oldpage{295}
  The latter is, in reality, a ``double'' structure, which is a novel example of multicategory (\cref{sec:III.3}) which is not a Lambek multicategory.
\end{rmenv}
...
let $\hat{\cat{S}}$ be a subcategory of $\cat{E}^\Gamma$;
we thus obtain a subcategory $\Gr{\TT}/\hat{\cat{S}}$ of $\Gr{\TT}$ and a subcategory $\Cat{Cat}(\TT)/\hat{\cat{S}}$ of $\Cat{Cat}(\TT)$ by taking the (not necessarily canonical) fibre products of functors:
\[
  \begin{tikzcd}
    \Cat{Cat}(\TT)
      \ar[d]
  & \Cat{Cat}(\TT)/\hat{\cat{S}}
      \ar[l]
      \ar[d]
  \\\Gr{\TT}
      \ar[d]
...
The study of subcategories of $\Cat{Cat}(\TT)$ thus leads us to describe various subcategory of $\cat{E}^\Gamma$.
The nomenclature that follows is not at all systematic;
it corresponds to various examples that we have seen.
In the rest of this section, $\cat{R}$ denotes a stabilised subcategory of $\cat{E}$.

\begin{itemize}
  \item $\hat{\cat{S}}_1$ is the full subcategory of $\cat{E}^\Gamma$ whose objects are the spans $(b,a)$ whose \unsure{indicator} is in $\cat{R}$.
  \item $\hat{\cat{S}}_2$ is the full subcategory of $\cat{E}^\Gamma$ whose objects are the spans $(b,a)$ such that $a$ is a morphism in $\cat{R}$.
\end{itemize}

\begin{itenv}{Proposition~IV.2.26}
  $\hat{\cat{S}}_1$ and $\hat{\cat{S}}_2$ are full subcategories of $\cat{E}^\Gamma$, stable under finite projective limits.
\end{itenv}

\begin{proof}
  $\cat{E}^\Gamma$ is a category of presheaves: its limits can be calculated term by term.
  If $1$ is the canonical terminal object of $\cat{E}$, then $(\id_1,\id_1)$ is the terminal object of $\cat{E}^\Gamma$ and, by~(i), it belongs to both $\hat{\cat{S}}_1$ and $\hat{\cat{S}}_2$.
  It thus suffices to prove that $\hat{\cat{S}}_1$ and $\hat{\cat{S}}_2$ are stable under finite fibre products.
  In both cases, this reduces to showing that the full subcategory of squares-$\cat{R}$ is stable under finite fibre products (in the case of $\hat{\cat{S}}_2$, we use the fact that projective limits commute with themselves).
\oldpage{296}
  This is precisely property~(iv).
\end{proof}

It does not appear to be sufficient to suppose that $\cat{R}$ is stabilised in order to obtain stability under non-finite projective limits.

\begin{itemize}
  \item
    $\hat{\cat{S}}_3$ is the subcategory of $\cat{E}^\Gamma$ defined by the morphisms consisting of a pair $(C,\overline{C})$, where $C$ is an $\cat{R}$-square.
    This is evidently a stabilised subcategory of $\cat{E}^\Gamma$.

  \item
    $\hat{\cat{S}}_4$, $\hat{\cat{S}}_5$, and $\hat{\cat{S}}_6$ are the subcategories of $\cat{E}^\Gamma$ defined, respectively, by the morphisms whose central, target, or source \unsure{indicator} is a morphism in $\cat{R}$.
    The morphisms of $\hat{\cat{S}}_5$ and $\hat{\cat{S}}_6$ are called \emph{$\cat{R}$-étale} and \emph{$\cat{R}$-co-étale} (respectively).

    We have already shown that $\hat{\cat{S}}_4$, $\hat{\cat{S}}_5$, and $\hat{\cat{S}}_6$ are stabilised subcategories of $\cat{E}^\Gamma$.
\end{itemize}
...
\begin{itenv}{Proposition~IV.2.27}
  ...
  for $\hat{\cat{S}}=\hat{\cat{S}}_3$, $\hat{\cat{S}}_4$, $\hat{\cat{S}}_5$, and $\hat{\cat{S}}_6$, it is a stabilised subcategory of $\Gr{\TT}$.
  We have analogous results when replacing $\Gr{\TT}$ by $\Cat{Cat}(\TT)$.
\end{itenv}
...
\begin{proof}
  ...
  \oldpage{297}
  \[
    \big( L(C), L(D), L(C'), L(D') \big)
  \]
  is an $\hat{\cat{S}}$-cartesian square, which proves (iii) for $\Gr{\TT}/\hat{\cat{S}}$.
  The rest of the proposition is a direct consequence of \unsure{Proposition~IV.1.25}.
\end{proof}

Let $\hat{\cat{S}}_p$ and $\hat{\cat{S}}_q$ be subcategories of $\cat{E}^\Gamma$, where $1\leqslant p<q\leqslant 6$;
we denote by $\hat{\cat{S}}_{pq}$ the subcategory of $\cat{E}^\Gamma$ whose set of objects is the set $\set{\hat{\cat{S}}_p}\cap\set{\hat{\cat{S}}_q}$, and whose morphisms are the morphisms common to both $\hat{\cat{S}}_p$ and $\hat{\cat{S}}_q$.
We thus obtain categories $\Cat{Cat}(\TT)/\hat{\cat{S}}$ by taking $\hat{\cat{S}}=\hat{\cat{S}}_{pq}$.

\begin{rmenv}{Examples}
  Unless otherwise stated, we continue to use the general notation of the previous section.
  The proposed equalities of categories are up to equivalence;
  they become strict equalities if we replace condition~(i) by the weaker condition:
  \begin{enumerate}
    \item[(i')] $\set{\cat{E}}=\set{\cat{R}}$.
  \end{enumerate}
...
Recall that a \emph{monad in $\Cat{D}$} is a triple $(\theta,i,k)$, where $\theta$ is a morphism in $\cat{D}$, and $i\colon\id_1\to\theta$ and $k\colon\theta\circ\theta\to\theta$ are 2-morphisms in $\cat{D}$ that make the following diagram commute:
\[
  \begin{tikzcd}[row sep=large]
    (\theta\circ\theta)\circ\theta
      \ar[rr,"s"]
      \ar[d,swap,"k\circ\theta"]
    & & \theta\circ(\theta\circ\theta)
      \ar[d,"\theta\circ k"]
  \\\theta\circ\theta
      \ar[r,"k"]
  ...
  \\1\circ\theta
    ...
    & \theta
      ...
      \ar[r,"r"]
    & \theta\circ 1
      \ar[u,swap,"\theta\circ i"]
  \end{tikzcd}
\]

We say that $(\mu,m)\colon(\theta,i,k)\to(\theta',i',k')$ is a \emph{morphism of monads in $\cat{D}$} if $(\theta,i,k)$ and $(\theta',i',k')$ are monads in $\cat{D}$, $\mu\colon1\to1'$ is a morphism in $\cat{D}$, and $\mu\colon\mu\circ\theta\to\theta'\circ\mu$ is a 2-morphism that makes the following diagram (where $s^{-1}$ is the inverse isomorphism of $s$) commute:
\[
  \begin{tikzcd}[row sep=large]
    \theta'\circ(\theta'\circ\mu)
      \ar[d,swap,"s^{-1}"]
    & \theta'\circ(\mu\circ\theta)
      \ar[l,swap,"\theta'\circ m"]
    && (\theta'\circ\mu)\circ\theta
      \ar[ll,swap,"s"]
    & (\mu\circ\theta)\circ\theta
      \ar[l,swap,"m\circ\theta"]
      \ar[d,"s"]
  \\(\theta'\circ\theta')\circ\mu
      \ar[d,swap,"k'\circ\mu"]
    &&&& \mu\circ(\theta\circ\theta)
...
\label{proposition:iv.3.28}
  To every morphism of monads
  \[
    (\mu,m)\colon
    (\theta, i, k)
    \to (\theta', i', k')
  \]
  we can associated a bimodule $(\sigma,h',h)\colon(\theta,i,k)\to(\theta',i',k')$, where
  \[
    \begin{aligned}
      \sigma
      &= \theta'\circ\mu
    \\h'
      &= (k'\circ\mu)\cdot s^{-1}
    \\h
      &= (k'\circ\mu)\cdot s^{-1}\cdot(\theta'\circ m)\cdot s.
    \end{aligned}
  \]
\end{itenv}

\begin{proof}
  We need to verify the commutativity of three diagrams, which is easy.
  For example, the commutativity of the diagram on the left below follows from that of the the diagram on the right:
  \[
    \begin{tikzcd}[sep=huge]
      \theta'\circ(\theta'\circ(\theta'\circ\mu))
        \ar[r,"{(\theta'\circ h')\cdot(\theta'\circ s^{-1})}"]
    & \theta'\circ(\theta'\circ\mu)
        \ar[dd,"{(k'\circ\mu)\cdot s^{-1}}"]
    \\(\theta'\circ\theta')\circ(\theta'\circ\mu)
        \ar[u,"s"]
        \ar[d,swap,"{(k'\circ(\theta'\circ\mu))}"]
    \\\theta'\circ(\theta'\circ\mu)
        \ar[r,swap,"{(k'\circ\mu)\cdot s^{-1}}"]
    & \theta'\circ\mu
    \end{tikzcd}
    \qquad
    \begin{tikzcd}[sep=huge]
...
\oldpage{300}
If $(\sigma,h',h)$ and $(\overline{\sigma},\overline{h}',\overline{h})\colon(\theta,i,k)\to(\theta',i',k')$ are bimodules between the same monads, we say that $g\colon(\sigma,h',h)\to(\overline{\sigma},\overline{h}',\overline{h})$ is a \emph{linear map} if $g\colon\sigma\to\overline{\sigma}$ is a 2-morphism in $\cat{D}$ that makes the following diagram commute:
\[
  \begin{tikzcd}[sep=large]
    \theta'\circ\sigma
      \ar[r,"h'"]
      \ar[d,swap,"\theta'\circ g"]
  & \sigma
      \ar[d,"g"]
  & \sigma\circ\theta
      \ar[l,swap,"h"]
      \ar[d,"g\circ\theta"]
  \\\theta'\circ\overline{\sigma}
      \ar[r,swap,"\overline{h}'"]
  & \overline{\sigma}
  & \overline{\sigma}\circ\theta
      \ar[l,"h"]
  \end{tikzcd}
\]
...
\begin{rmenv}{Remarks}
  \begin{enumerate}
    ...
    \item
      ...
      we could also define a bicategory $\Cat{Bim}(\cat{D})$ as having the monads in $\cat{D}$ as its objects, and bimodules as its morphisms with composition given by the tensor product defined above.
      The existence of this product is linked to the existence of cokernels in the categories $\Hom_\cat{D}(1,1')$ and the commutativity of the functors $\kappa(e,e',e'')$ with the cokernels.
      We could finally extend $\Cat{Bim}(\cat{D})$ to a multicategory, but we save these questions for a future work.
  \end{enumerate}
\end{rmenv}
...
\begin{itenv}{Proposition~IV.3.29}
\label{proposition:iv.3.29}
  Every $\TT$-functor $(f,\overline{f})\colon(\theta,i,k)\to(\theta',i',k')$ determines a morphism between monads $(\mu,m)\colon(\theta,i,k)\to(\theta',i',k')$, where $\mu=(Ie'\cdot f,\id_e)$ and $m$ is the crochet characterised by the relations
  \[
    p_1\cdot m = f
    \textand
    p_2\cdot m = b
  \]
  where we write $\theta'=(a',b')$ and $\theta=(a,b)$, and $e$ and $e'$ are the targets of $b$ and $b'$ (respectively), and $(p_1,p_2)$ is the fibre product of $(b',f)$.
\end{itenv}

\begin{proof}
  We can easily show that
  \[
    \theta'\circ\mu = (a'\cdot p_1, p_2)
    \textand
    \mu\circ\theta = (Tf\cdot a, b).
  \]

  To better display the essential ideas of the proof, we will suppose that $\Sp{\TT}$ is a 2-category, which is equivalent to supposing that all the fibre products in question are canonical;
  the proof in the general case is not more difficult, apart from when it comes to notation.
  We will first describe the elements of the following diagram in $\Sp{\TT}$:
  \[
  ...
  \begin{itemize}
    \item
      $\mu\circ i$ is equal to $i\colon(Ie'\cdot f,\id_e)\to(Tf\cdot a,b)$.

    \item
      $i'\circ\mu$ is equal to $i''\colon(Ie'\cdot f,\id_e)\to(a'\cdot p_1,p_2)$, where $i''$ is the crochet characterised by the equations
      \[
        p_1\cdot i'' = i'\cdot f
        \textand
        p_2\cdot i'' = \id_e.
      \]
      and whose existence follows from the equality $b'\cdot i'\cdot f=f\cdot\id_e$.

    \item
      $m\colon(Tf\cdot a,b)\to(a'\cdot p_1,p_2)$ is a 2-morphism in $\Sp{\TT}$, and the commutativity of the lower part of the above diagram is expressed by the equation $m\cdot i=i''$, which follows from the equalities
      \[
        p_1\cdot m\cdot i = p_1\cdot i''
        \textand
        p_2\cdot m\cdot i = p_2\cdot i''.
      \]

    \item
      \oldpage{302}
      $\mu\circ k$ is equal to
      \[
        k\colon(Ke'\cdot TTf\cdot Ta\cdot v_2,b\cdot v_1)\to(Tf\cdot a,b)
      \]
      where $(v_1,v_2)$ is the fibre product of $(a,Tb)$.

    \item
      $k'\circ\mu$ is equal to
      \[
        k''\colon(Ke'\cdot Ta'\cdot v'_2\cdot q_1,p_2\cdot q_2)\to(a'\cdot p_1,p_2)
      \]
      where $(v'_1,v'_2)$ is the fibre product of $(a',Tb')$, and $(q_1,q_2)$ the fibre product of $(v'_1,p_1)$, and where $k''$ is the crochet characterised by
      \[
        p_1\cdot k'' = k'\cdot q_1
        \textand
        p_2\cdot k'' = p_2\cdot q_2
      \]
      and whose existence follows from the equality $b'\cdot k'\cdot q_1=f\cdot p_2\cdot q_2$.

    \item
      $\theta'\circ m$ is equal to
      \[
        m'\colon (Ke'\cdot Ta'\cdot Tp_1\cdot p'_1, b\cdot p'_2)
        \to (Ke'\cdot Ta'\cdot v'_2\cdot q_1, p_2\cdot q_2)
      \]
      where $(p'_1,p'_2)$ is the fibre product of $(Tp_2,a)$, and where $m'$ is the crochet characterised by the equations
      \[
        v'_2\cdot q_1\cdot m' = Tp_1\cdot p'_1
        \textand
        q_2\cdot m' = m\cdot p'_2
      \]
      and whose existence follows from the equality $Tb'\cdot Tp_1\cdot p'_1=a'\cdot p_1\cdot m\cdot p'_2$.

    \item
      $m\circ\theta$ is equal to
      \[
        m''\colon (Ke'\cdot TTf\cdot Ta\cdot v_2, b\cdot v_1)
        \to (Ke'\cdot Ta'\cdot Tp_1\cdot p'_1, b\cdot p'_2)
      \]
      where $m''$ is the crochet characterised by the equations
      \[
        p'_1\cdot m'' = Tm\cdot v_2
        \textand
        p'_2\cdot m'' = v_1
      \]
      and whose existence follows from the equality $Tp_2\cdot Tm\cdot v_2=a\cdot v_1$.
  \end{itemize}

  The commutativity of the upper part of the above diagram is expressed by the equation $m\cdot k=k''\cdot m'\cdot m''$, which follows from the equalities
  \[
...
For example, if $\TT$ is the identity triple (\cref{sec:III.2}) on $\cat{E}$, then a morphism between monads in $\Sp{\cat{E}}$ coincides with a span of $\cat{E}$-functors $(F',F)$, where $F'$ is a co-étale functor (\cref{sec:iv.2}) (that that Lawvere called ``partial functors'' in a recent conference).

The bimodules in $\Sp{\cat{E}}$ are precisely the Bénabou profunctors;
we define $\TT$-profunctors as the bimodules in $\Sp{\TT}$.
\oldpage{303}
By \cref{proposition:iv.3.28} and \cref{proposition:iv.3.29}, a $\TT$-functor determines a $\TT$-functor (in the above sense), which suggests defining a \emph{$\TT$-natural transformation} to be a linear map between two $\TT$-profunctors associated in the aforementioned way to two $\TT$-functors.
We can also easily show that a linear map in $\Sp{\cat{E}}$ between $\cat{E}
$-functors is an $\cat{E}$-natural transformation (we can even more easily see this in the case where $\cat{E}=\Cat{Set}$).
We will later search for a construction of a bicategory $\Cat{Procat}(\TT)=\Cat{Bim}(\Sp{\TT})$.

\begin{rmenv}{Remarks}
  \begin{enumerate}
    \item
      The hypothesis that $\TT$ be cartesian is not necessary in order to define $\TT$-profunctors, but it seems indispensable to apply \cref{proposition:iv.3.28} and \cref{proposition:iv.3.29}.
      If $\cat{D}$ is only a pseudo-category, we cannot easily define a composition in the ``squares'' of $\cat{D}$, and even in particular examples, such as that where $\TT=\TT_\mathcal{U}$ (\cref{sec:III.5}), everything that mentions the notion of a $\TT$-natural transformation seems to be difficult.

    \item
      We can equally, starting from a $\TT$-functor, obtain a morphism in the opposite direction to that which we construct in \cref{proposition:iv.3.29};
      it is likely that these two morphisms are ``adjoint'' to one another, as in the case of ordinary functors.
  \end{enumerate}
\end{rmenv}
...
As in \cref{sec:II.4}, we denote a pseudo-monoidal category as a triple $\overline{\cat{D}}=(\cat{D},1,\circ)$.
Let $\Mon{\overline{\cat{D}}}$ be the category of monads in $\overline{\cat{D}}$ (\cref{sec:II.3}); its objects are triples $(\theta,i,k)$, where $i\colon1\to\theta$ and $k\colon\theta\circ\theta\to\theta$ are morphisms in $\cat{D}$ such that
\[
  \begin{gathered}
    k\cdot(i\circ\theta)\cdot l(\theta)
    = \theta
    = k\cdot(\theta\circ i)\cdot r(\theta)
  \\k\cdot(k\circ\theta)
    = k\cdot(\theta\circ k)\cdot s(\theta,\theta,\theta).
  \end{gathered}
  \tag{1}
\]
and $m\colon(\theta,i,k)\to(\theta',i',k')$ is a morphism in $\Mon{\overline{\cat{D}}}$ if $m\colon\theta\to\theta'$ is a morphism in $\cat{D}$ such that
\[
  m\cdot i = i'
  \textand
  m\cdot k = k'\cdot(m\circ m).
  \tag{2}
\]
We have an evident forgetful functor $\Mon{\overline{\cat{D}}}\to\cat{D}$ that sends the monad $(\theta,i,k)$ to the object $\theta$.
We will assume a rather strong hypothesis~(D) on $\overline{\cat{D}}$ which will ensure that this forgetful functor is tripleable whenever $\overline{\cat{D}}$ is monoidal and which, furthermore, will allow us to construct free monads by exactly imitating the construction of a free monoid generated by a set.

\begin{itenv}{Lemma~1}
\label{lemma:1}
  If the forgetful functor $\Mon{\overline{\cat{D}}}\to\cat{D}$ admits an adjoint, then it is tripleable.
\end{itenv}

\begin{proof}
  Let $\TT=(T,I,K)$ be the triple on $\cat{D}$ induced by the forgetful functor $U\colon\Mon{\overline{\cat{D}}}\to\cat{D}$ and its adjoint $F\colon\cat{D}\to\Mon{\overline{\cat{D}}}$, and let
  \[
    E\colon \Mon{\overline{\cat{D}}}
    \to \Alg{\TT}
  \]
  be the associated Eilenberg--Moore functor;
  we will construct an inverse $E'$ of $E$.

  Let $b\colon T\theta\to\theta$ be a $\TT$-algebra on $\theta$ and set
  \oldpage{305}
  \[
    F\theta = (\hat{\theta},\hat{i},\hat{k})
    \textand
    F\hat{\theta} = (\hat{\hat{\theta}},\hat{\hat{i}},\hat{\hat{k}}).
  \]
  Note that $Tb\colon F\hat{\theta}\to F\theta$ is a morphism in $\Mon{\overline{\cat{D}}}$, since $T=UF$.
  We will show that $(\theta,i,k)$ is a monad, where
  \[
    i = b\cdot \hat{i}
    \textand
    k = b\cdot \hat{k}\cdot(I\theta\circ I\theta).
  \]
  Since the principle is always the same, we will restrict ourselves to showing the last equation in~(1), by setting
  \[
    \tilde{I} = (I\theta\circ I\theta)\circ I\theta
    \textand
    \tilde{I}' = I\theta\circ(I\theta\circ I\theta).
  \]
  We obtain
  \[
    \begin{aligned}
      k\cdot(k\circ\theta)
      &= b\cdot\hat{k}\cdot(I\theta\circ I\theta)\cdot(b\circ b)\cdot(\hat{k}\circ\theta)\cdot\tilde{I}
    \\&= b\cdot\hat{k}\cdot(Tb\circ Tb)\cdot(IT\theta\circ IT\theta)\cdot(\hat{k}\circ\theta)\cdot\tilde{I}
    \\&= b\cdot Tb\cdot \hat{\hat{k}}\cdot(IT\theta\circ IT\theta)\cdot(\hat{k}\circ\theta)\cdot\tilde{I}
    \\&= b\cdot K\theta\cdot\hat{\hat{k}}\cdot(IT\theta\circ IT\theta)\cdot(\hat{k}\circ\theta)\cdot\tilde{I}
    \\&= b\cdot\hat{k}\cdot(K\theta\circ K\theta)\cdot(IT\theta\circ IT\theta)\cdot(\hat{k}\circ\theta)\cdot\tilde{I}
    \\&= b\cdot\hat{k}\cdot(\hat{k}\circ\theta)\cdot\tilde{I}
    \\&= b\cdot\hat{k}\cdot(\theta\circ\hat{k})\cdot s(\theta,\theta,\theta)\cdot\tilde{I}
    \\&= b\cdot\hat{k}\cdot(\theta\circ\hat{k})\cdot\tilde{I}'\cdot s(\theta,\theta,\theta)
    \\&= k\cdot(\theta\circ k)\cdot s(\theta,\theta,\theta).
    \end{aligned}
  \]

  We similarly define $\set{E'}$ and, by passing from objects to morphisms, an analogous argument allows us to define the functor $E'$.
  We can easily verify that $E'$ is an inverse of $E$, which essentially reduces to verifying, above, the two relations
  \[
    i = b\cdot\hat{i}
    \textand
    b\cdot\hat{k} = k\cdot(b\circ b)
  \]
  where the latter can be shown by composing both sides with the monomorphism $I\theta\circ I\theta$.
\end{proof}

\begin{rmenv}{Remark}
  This lemma is a particular case of a more general fact that holds for all ``algebraic'' structures, under the condition of choosing good forgetful functors.
  We will later see (\hyperref[proposition:a.2]{Proposition~A.2}) another application, in a slightly less particular case.
\end{rmenv}

For all $\theta\in\set{\cat{D}}$, we can define functors $Y_\theta$ and $Y'_\theta\colon\cat{D}\to\cat{D}$ which, to each morphism $m\colon\theta_1\to\theta_2$ in $\cat{D}$, associate (respectively) the morphisms
\[
  \theta\circ m\colon\theta\circ\theta_1\to\theta\circ\theta_2
  \textand
  m\circ\theta\colon \theta_1\circ\theta\to\theta_2\circ\theta.
\]

\oldpage{306}
We say that $\overline{\cat{D}}$ is \emph{countably distributive} if it satisfies the following axiom:

\begin{itenv}{Axiom~(D)}
\label{axiom:d}
  For every $\theta\in\set{\cat{D}}$, the functors $Y_\theta$ and $Y'_\theta$ commute with sums, and $\cat{D}$ admits finite and countable sums.
\end{itenv}

\begin{itenv}{Lemma~2}
\label{lemma:2}
  If $\overline{\cat{D}}$ is a countably distributive pseudo-category, and if $\{j_u\colon\theta_u\to\hat{\theta}\}_{u\in N}$ and $\{j'_v\colon\theta'_v\to\hat{\theta}'\}_{v\in N'}$ form systems of canonical injections of sums in $\cat{D}$, where $N$ and $N'$ are sets of at most countable cardinality, then
  \[
    \{j_u\circ j'_v\colon
    \theta_u\circ\theta'_v
    \to \hat{\theta}\circ\hat{\theta}'\}_{(u,v)\in N\times N'}
  \]
  forms a system of canonical injections of a sum.
  This claim extends to the composition of a finite sequence of sums in $\cat{D}$.
\end{itenv}

\begin{proof}
  This is an immediate consequence of axiom~(D), since
  \[
    j_u\circ j'_v
    = (j_u\circ\hat{\theta}')\cdot(\theta_u\circ j'_v)
  \]
  and since sums in a category are associative (infinite associativity).
\end{proof}

Let $\overline{\cat{D}}$ be a monoidal category.
We will make full use of the results of ``coherence theory'', long since developed by Mac-Lane.
In particular, $r(1)=s(1)$.
The lemma below will not be proven.

Let $\theta\in\set{\cat{D}}$;
we set
\begin{itemize}
  \item
    $\bigcirc^0\theta = 1$, $\bigcirc^1\theta=\theta$, and, inductively, $\bigcirc^{u+1}\theta=\theta\circ(\bigcirc^u\theta)$ for all $u\geqslant1$;
  \item
    $s_{u,0}\theta = r(\bigcirc^u\theta)^{-1}$ and $s_{0,v}\theta = l(\bigcirc^v\theta)^{-1}$ for all $u,v\geqslant0$; in particular $s_{0,0}=r(1)^{-1}=l(1)^{-1}$;
  \item
    $s_{1,v}\theta = \theta\circ(\bigcirc^v\theta)$ for $v\geqslant1$, and, inductively, for $u,v\geqslant1$,
    \[
      s_{u+1,v}\theta
      = (\theta\circ s_{u,v}\theta) \cdot s(\bigcirc^v\theta, \bigcirc^u\theta, \theta).
    \]
\end{itemize}

\begin{itenv}{Lemma~3}
\label{lemma:3}
  If $\overline{\cat{D}}$ is a countably distributive monoidal category, then we have the formula
  \[
    s_{u+v,w}\theta \cdot \big( (s_{u,v}\theta)\circ(\bigcirc^w\theta) \big)
    = s_{u,v+w}\theta \cdot \big( (\bigcirc^u\theta)\circ(s_{v,w}\theta) \big) \cdot s(\bigcirc^w\theta, \bigcirc^v\theta, \bigcirc^u\theta)
  \]
  for all $u,v,w\geqslant0$.
\end{itenv}

\oldpage{307}
Let $(\theta,i,k)$ be a monad in $\overline{\cat{D}}$;
we set
\begin{itemize}
  \item $k_0 = i$
  \item $k_1 = \theta$
  \item $k_2 = k$
  \item inductively, for $u\geqslant1$, $k_{u+1} = k\cdot(\theta\circ k_u)$.
\end{itemize}
...
\begin{proof}
  A monad is a bifunctor of the form $\mathbf{1}\to\overline{\cat{D}}$;
  the lemma is thus a result of the ``coherence theory'', but we will give a direct proof.

  For $u=1$, this is the definition of $k_{1+v}$ as $k\cdot(\theta\circ k_v)$ for $v\geqslant1$, since $s_{1,v}\theta$ is an identity.
  The equality remains true for $v=0$, since in this case it can be written as $r(\theta)^{-1}=k\cdot(\theta\circ i)$.

  Now suppose that $v\geqslant1$ is fixed, and suppose that the formula has been proven for some integer $u\geqslant1$;
  we will show that it is then true for the integer $u+1$.
  \[
    \begin{aligned}
      k_{1+u+v}\cdot s_{1+u,v}\theta
      &= k \cdot (\theta\circ k_{u+v}) \cdot (\theta\circ s_{u,v}\theta) \cdot s(\bigcirc^v\theta,\bigcirc^u\theta,\theta)
    \\&= k \cdot (\theta\circ k) \cdot \big(\theta\circ(k_u\circ k_v)\big) \cdot s(\bigcirc^v\theta,\bigcirc^u\theta,\theta)
    \\&= k \cdot (\theta\circ k) \cdot s(\theta,\theta,\theta) \cdot \big((\theta\circ k_u)\circ k_v\big)
    \\&= k \cdot (k\circ\theta) \cdot \big((\theta\circ k_u)\circ k_v\big)
    \\&= k\cdot (k_{u+1}\circ k_v).
    \end{aligned}
  \]

  The formula is thus proven in the three cases $u\geqslant1$, $v\geqslant0$, and $u=1$ and $v=0$.
  For $v=0$ and arbitrary $u\geqslant0$, we can reduce to proving that
  \[
    k_u = k\cdot (k_u\circ i) \cdot r(\bigcirc^u\theta).
  \]
  For $u=1$, this is true, by the definition of a monad.
  Now we suppose that we have proven this for some $u\geqslant1$, and prove that it remains true for $u+1$.
  We have that
  \[
    \begin{aligned}
      k_{1+u}
      &= k \cdot (\theta\circ k_u)
    \\&= k \cdot (\theta\circ k) \cdot \big(\theta\circ(k_u\circ i)\big) \cdot \big(\theta\circ r(\bigcirc^u\theta)\big)
    \\&= k \cdot (\theta\circ k) \cdot \big(\theta\circ(k_u\circ i)\big) \cdot s(1,\bigcirc^u\theta,\theta) \cdot r(\bigcirc^{1+u}\theta)
    \\&= k \cdot (\theta\circ k) \cdot s(\theta,\theta,\theta) \cdot \big((\theta\circ k_u)\circ i\big) \cdot r(\bigcirc^{1+u}\theta)
    \\&= k \cdot (k\circ\theta) \cdot \big((\theta\circ k_u)\circ i\big) \cdot r(\bigcirc^{1+u}\theta)
    \\&= k \cdot (k_{1+u}\circ i) \cdot r(\bigcirc^{1+u}\theta).
    \end{aligned}
  \]

  \oldpage{308}
  Note that we have used the formula from \hyperref[lemma:3]{Lemma~3} in the particular case where $u=1$, $v=u$, and $w=0$ (using the notation from the lemma), which can be written as
  \[
    r(\bigcirc^{1+u}\theta)^{-1}
    = \big(\theta\circ r(\bigcirc^u\theta)^{-1}\big) \cdot s(1,\bigcirc^u\theta,\theta).
  \]
  The formula is thus proven for $u\geqslant1$ and $v\geqslant0$.
  The case $u=v=0$ is then easy to prove.
\end{proof}

\begin{itenv}{Proposition~A.1}
\label{proposition:a.1}
  If $\overline{\cat{D}}$ is a countably distribute monoidal category, then the forgetful functor $\Mon{\overline{\cat{D}}}\to\cat{D}$ is tripleable.\footnote{This result also appears in the article ``Cohomology in tensored categories'' by M.~Barr, \emph{Proceedings of the Conference of Categorical Algebra, La Jolla}, Springer (1965).}
\end{itenv}

\begin{proof}
  Let $\hat{\theta}$ be a sum of the family $(\bigcirc^u\theta\mid u\in\mathbb{N})$ for an object $\theta$ of $\cat{D}$, and let $j_u\colon\bigcirc^u\theta\to\hat{\theta}$ (for $u\in\mathbb{N}$) be a system of canonical injections of this sum.
  By \hyperref[lemma:2]{Lemma~2}, there exists a crochet (of a sum indexed by $\mathbb{N}\times\mathbb{N}$) characterised by the relations
  \[
    \hat{k}\cdot(j_u\circ j_v)
    = j_{u+v}\cdot s_{u,v}\theta
    \qquad(\text{for }u,v\geqslant0).
    \tag{3}
  \]
  We will show that $(\theta,\hat{i},\hat{k})$, where $\hat{i}=j_0$, is a monad.
  We have
  \[
    \begin{aligned}
      \hat{k} \cdot (\hat{k}\circ\hat{\theta}) \cdot \big( (j_u\circ j_v)\circ j_w \big)
      &\overset{\text{(3)}}{=} \hat{k} \cdot (j_{u+v}\circ j_w) \cdot \big( s_{u,v}\theta\circ(\bigcirc^w\theta) \big)
    \\&= j_{u+v+w} \cdot s_{u+v,w}\theta \cdot \big( s_{u,v}\theta\circ(\bigcirc^w\theta) \big)
    \\&\overset{\text{(Lemma~3)}}{=} j_{u+v+w} \cdot s_{u,v+w}\theta \cdot \big( (\bigcirc^u\theta)\circ s_{v,w}\theta \big) \cdot s(\bigcirc^w\theta,\bigcirc^v\theta,\bigcirc^u\theta)
    \\&= \hat{k} \cdot (\hat{\theta}\circ\hat{k}) \cdot s(\hat{\theta},\hat{\theta},\hat{\theta}) \cdot \big( (j_u\circ j_v)\circ j_w \big).
    \end{aligned}
  \]
  Similarly, for ``identity'', we have
  \[
    \begin{aligned}
      \hat{k} \cdot (\hat{\theta}\circ\hat{i}) \cdot r(\hat{\theta}) \cdot j_u
      &= \hat{k} \cdot (\hat{\theta}\circ\hat{i}) \cdot (j_u\circ1) \cdot r(\bigcirc^u\theta)
    \\&= \hat{k} \cdot (j_u\circ j_0) \cdot s_{u,0}\theta^{-1}
    \\&= j_u.
    \end{aligned}
  \]
  ...
  Let $m$ be the crochet of the sum $m\colon\hat{\theta}\to\theta'$ characterised by the relations
  \[
    m\cdot j_u
    = k'_u\cdot \bigcirc^u j'
    \qquad(\text{for } u\geqslant0)
    \tag{4}
  \]
  \oldpage{309}
  where $k'_u$ was defined in Lemma~4, with respect to the monad $(\theta',i',k')$.
  We now show that $m\colon(\hat{\theta},\hat{i},\hat{k})\to(\theta',i',k')$ is a morphism of monads.
  Firstly, applying (4) with $u=0$ gives the first relation $m\cdot\hat{i}=i'$ of (2).
  We have
  \[
    \begin{aligned}
      m\cdot\hat{k}\cdot(j_u\circ j_v)
      &= m\cdot j_{u+v}\cdot s_{u,v}\theta
    \\&\overset{\text{(4)}}{=} k'_{u+v}\cdot\bigcirc^{u+v}j'\cdot s_{u,v}\theta
    \\&= k'_{u+v}\cdot s_{u,v}\big( (\bigcirc^u j')\circ(\bigcirc^v j') \big)
    \\&\overset{\text{Lemma~4}}{=} k'\cdot(k'_u\circ k'_v)\cdot\big( (\bigcirc^u j')\circ(\bigcirc^v j') \big)
    \\&\overset{\text{(4)}}{=} k'\cdot(m\circ m)\cdot(j_u\circ j_v)
    \end{aligned}
  \]
  and, by simplifying by $j_u\circ j_v$ we have shown the relations in~(2).

  The relation~(4) for $u=1$ becomes $m\cdot j_1=j'$.
  We will show that the morphism $m$ satisfying this relation and defining a morphism of monads is unique.
  It suffices, for example, to show that these conditions imply that $m$ satisfies the relations in~(4).
  For this, we first prove the two following formulas:
  \[
    \left.
    \begin{aligned}
      k'_u\cdot\bigcirc^u m
      &= m\cdot\hat{k}_u
    \\j_u
      &= \hat{k}_u\cdot\bigcirc^u j_1
    \end{aligned}
    \right\}
    \qquad(\text{for }u\geqslant0).
    \tag{5}
  \]
  The first of these is evident for $u=0$, $1$, and $2$;
  suppose that it is true for some $u\geqslant2$;
  then we have
  \[
    \begin{aligned}
      k'_{1+u}\cdot\bigcirc^{1+u}m
      &= k'\cdot(\theta'\circ k'_u)\cdot\bigcirc^{1+u}m
    \\&= k'\cdot(m\circ m)\cdot(\theta\circ\hat{k}_u)
    \\&= m\cdot\hat{k}\cdot(\theta\circ\hat{k}_u)
    \\&= m\cdot\hat{k}_{1+u}
    \end{aligned}
  \]
  which proves the first relation of~(5) for all $u\geqslant0$.
  The second is evident for $u=0$, $1$, and $2$;
  suppose that it is true for some $u\geqslant2$;
  then we have
  \[
    \begin{aligned}
      j_{1+u}
      &= \hat{k}\cdot(j_1\circ j_u)
    \\&= \hat{k}\cdot(\theta\circ\hat{k}_u)\cdot\bigcirc^{1+u}j_1
    \\&= \hat{k}_{1+u}\cdot\bigcirc^{1+u}j_1
    \end{aligned}
  \]
  which finishes the proof of~(5).

  We can now prove~(4) by using the formulas in~(5):
  \[
    \begin{aligned}
      m\cdot j_u
      &= m\cdot\hat{k}_u\cdot\bigcirc^u j_1
    \\&= k'_u\cdot\bigcirc^u m\cdot\bigcirc^u j_1
    \\&= k'_u\cdot\bigcirc^u j'.
    \end{aligned}
  \]
  This finishes the proof that $(\hat{\theta},\hat{i},\hat{k})$ is the free monad generated by $\theta$.

  The functor $\Mon{\overline{\cat{D}}}\to\cat{D}$ thus admits an adjoint, and is tripleable by Lemma~1.
\end{proof}

\begin{rmenv}{Remarks}
  \begin{enumerate}
    \item
      The author of this work does not know if we can adapt such a construction to a countably distributive pseudo-monoidal category $\overline{\cat{D}}$.
      \oldpage{310}
      However, we can construct free ``non-unital monads'' (pairs $(\theta,k)$ where $k\colon\theta\circ\theta\to\theta$ satisfies the second relation in~(1)), by using Lemma~3 for $u,v,w\geqslant1$ and by replacing, in the proof, $\hat{\theta}$ by the sum of the $\bigcirc^n\theta$ for $n\geqslant1$.

    \item
      This result, used in \cref{sec:III.1}, shows in particular that \emph{the existence of a free $\cat{E}$-category generated by an $\cat{E}$-graph in a category $\cat{E}$ is ensured if $\cat{E}$ admits (finite) fibre products and countable sums, and if these sums are universal and commute with the fibre products.}
    ...
    \item
      ...
      For example, if we take $\overline{\cat{D}}=\Gr{\TT_0}/e$, where $\TT_0=(T_0,I_0,K_0)$ is the obvious triple on $\cat{E}$ such that, for all $e'\in\set{\cat{E}}$, $T_0e'$ is equal to a final object of $\cat{E}$, then the adjunction morphism in \hyperref[proposition:a.1]{Proposition~A.1} will be a canonical injection $j_1\colon\theta\to\theta+1$.
  \end{enumerate}
\end{rmenv}



\subsection{Generalisation to certain algebraic structures}
\label{sec:a.2}

When the monoidal category $\overline{\cat{D}}=(\cat{D},1,\times)$ is \emph{cartesian}, that is, when $\times\colon\cat{D}\times\cat{D}\to\cat{D}$ is the adjoint of the ``diagonal'' functor $\cat{D}\to\cat{D}\times\cat{D}$, the notion of monad in $\overline{\cat{D}}$ coincides with that of \emph{monoid} in $\cat{D}$, or a $\cat{D}$-monoid.
Examples of monads that are not monoids are numerous (rings, triples, $\TT$-categories, \ldots).
We can ask ourselves the following questions:

\begin{enumerate}
  \item
    Which are the algebraic structures that, like that of monoids, can be defined more generally in non-cartesian monoidal categories (algebraic structures in the sense of Birkhoff--Lawvere)?
  \item
    \oldpage{311}
    Is the result of \hyperref[proposition:a.1]{Proposition~A.1} still true for such structures?
\end{enumerate}
...
\begin{itenv}{(T)}
  The set $\set{\Delta}$ endowed with the restriction of the first law $+\colon\Delta\times\Delta\to\Delta$ is identical (or isomorphic) to the set $\mathbb{N}$ of positive or zero integers with the usual addition.
  Every morphism $\varepsilon\colon n\to n'$ in $\Delta$ can be uniquely written in the form
  \[
    \varepsilon = \varepsilon_1 + \varepsilon_2 + \ldots + \varepsilon_n
  \]
  where $(\varepsilon_1,\varepsilon_2,\ldots,\varepsilon_{n'})\in\Delta_0^{n'}$.
\end{itenv}

We further say that $\Delta$ is \emph{countable} if $\Delta_0$ is finite or countable (see \hyperref[sec:a.3]{§A.3} for an interpretation of theories in terms of multicategories).

If $\overline{\cat{D}}=(\cat{D},1,\circ)$ and $\overline{\Delta}=(\Delta,0,+)$ are strict monoidal categories, we denote by $\Str{\overline{\Delta}}{\overline{\cat{D}}}$ the category whose objects are the functors $F\colon\Delta\to\cat{D}$ that commute with the first laws, or, equivalently, the bifunctors $\overline{\Delta}\to\overline{\cat{D}}$;
we call such a functor a \emph{$\overline{\Delta}$-structure}.
We say that $m\colon F\to F'$ is a \emph{morphism in $\Str{\overline{\Delta}}{\overline{\cat{D}}}$} if $F$ and $F'$ are $\overline{\Delta}$-structures and $m\colon F1\to F'1$ is a morphism in $\cat{D}$ such that
\[
  (\bigcirc^u m \mid u\in\mathbb{N})\colon F\to F'
\]
is a natural transformation.
We have a forgetful functor $\Str{\overline{\Delta}}{\overline{\cat{D}}}\to\cat{D}$ that sends a $\overline{\Delta}$-structure $F$ to the object $F1$ of $\cat{D}$.

\begin{itenv}{Proposition~A.2}
\label{proposition:a.2}
  If $\overline{\cat{D}}=(\cat{D},1,\circ)$ is a countably distributive strict monoidal category, and if $\overline{\Delta}=(\Delta,0,+)$ is a countable theory, then the forgetful functor $\Str{\overline{\Delta}}{\overline{\cat{D}}}\to\cat{D}$ is tripleable.
\end{itenv}
...
\begin{proof}
  ...
  \begin{enumerate}
      \item
        \emph{The $\overline{\Delta}$-structure $F_\theta\colon\Delta\to\cat{D}$ associated to an object $\theta\in\set{\cat{D}}$.}

        Let $\hat{\theta}$ be a sum in $\cat{D}$ of the family of objects $(\bigcirc^n\theta\mid\varepsilon\in\Delta_0)$, where $n$ denotes the source of $\varepsilon$ and $\Delta_0$ the set of morphisms in $\Delta$ with target $1$.
        Let
        \[
          j_\varepsilon\colon
          \bigcirc^n\theta \to \hat{\theta}
          \qquad(\text{for }\varepsilon\in\Delta_0)
        \]
        be a canonical system of injections for this sum.
        Let $F_\theta\varepsilon\colon\bigcirc^n\hat{\theta}\to\hat{\theta}$ be the (sum) crochet characterised by the relations
        \[
          F_\theta\varepsilon\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n})
          = j_{\varepsilon\cdot(\nu_1+\ldots+\nu_n)}
          \tag{1}
        \]
        for all $(\nu_1,\ldots,\nu_n)\in\Delta_0^n$.
        If $\varepsilon\colon n\to n'$ is an arbitrary morphism of $\Delta_0$, where now $n'$ can be different from $1$, then we have a unique decomposition $\varepsilon=\varepsilon_1+\ldots+\varepsilon_{n'}$, where $(\varepsilon_1,\ldots,\varepsilon_{n'})\in\Delta_0^{n'}$, which allows us to unambiguously define
        \[
          F_\theta\varepsilon
          = F_\theta\varepsilon_1 \circ \ldots \circ F_\theta\varepsilon_{n'}.
        \]
        We then have the relation
        \[
          F_\theta(\varepsilon+\varepsilon')
          = F_\theta\varepsilon \circ F_\theta\varepsilon'
          \tag{2}
        \]
        for every pair $(\varepsilon,\varepsilon')$ of morphisms in $\Delta$.

        Now we will show that, if $\varepsilon\colon n\to n'$ and $\tau\colon n'\to n''$ are ``consecutive'' morphisms in $\Delta$, then we have
        \[
          F_\theta(\tau\cdot\varepsilon)
          = F_\theta\tau\cdot F_\theta\varepsilon.
          \tag{3}
        \]
        We have decompositions
        \[
          \begin{aligned}
            \varepsilon &= \varepsilon_1+\ldots+\varepsilon_{n'}
          \\\tau &= \tau_1+\ldots+\tau_{n''}
          \end{aligned}
        \]
        where
        \[
          \begin{aligned}
            \varepsilon_i\colon
            &p_i \to 1
            \qquad(\text{for }1\leqslant i\leqslant n')
          \\\tau_{i'}\colon
            &p'_{i'} \to 1
            \qquad(\text{for }1\leqslant i'\leqslant n'')
          \end{aligned}
        ...
        \[
          ...
          \end{aligned}
        \]
        Since the latter of these two sums is ``finer'' than the former, we can group the terms of this second sum and write
        \[
          (p_1+\ldots+p_{p'_1}) + (p_{p'_1+1}+\ldots+p_{p'_1+p'_2}) + \ldots
          = n.
        \]
        \oldpage{313}
        We have
        \[
          \tau\cdot\varepsilon
          = \tau_1\cdot\sigma_1 + \ldots + \tau_{n''}\cdot\sigma_{n''}
        \]
        where
        \[
          \begin{aligned}
            \sigma_1
            &= \varepsilon_1 + \ldots + \varepsilon_{p'_1}
          \\\sigma_2
            &= \varepsilon_{p'_1+1} + \ldots + \varepsilon_{p'_1+p'_2}
          \\&\ldots.
          \end{aligned}
        \]
        This formula gives the (unique) decomposition of $\tau\cdot\varepsilon$, thus
        \[
          F_\theta(\tau\cdot\varepsilon)
          = F_\theta(\tau_1\cdot\sigma_1)\circ F_\theta(\tau_2\cdot\sigma_2) \circ\ldots\circ F_\theta(\tau_{n''}\cdot\sigma_{n''}).
        \]
        Suppose that we have proven~(3) in the case $n''=1$;
        then
        \[
          \begin{aligned}
            F_\theta(\tau\cdot\varepsilon)
            &= (F_\theta\tau_1\cdot F_\theta\sigma_1)\circ\ldots\circ(F_\theta\tau_{n''}\cdot F_\theta\sigma_{n''})
          \\&\overset{\circ\text{ functorial}}{=} (F_\theta\tau_1\circ\ldots\circ F_\theta\tau_{n''})\cdot(F_\theta\sigma_1\circ\ldots\circ F_\theta\sigma_{n''})
          \\&\overset{\text{(2) iterated}}{=} F_\theta(\tau_1+\ldots+\tau_{n''})\cdot F_\theta(\sigma_1+\ldots+\sigma_{n''})
          \\&= F_\theta\tau\cdot F_\theta\varepsilon
          \end{aligned}
        \]
        since $\sigma_1+\ldots+\sigma_{n''}=\varepsilon$.
        We now show that formula~(3) is true for $n''=1$.
        Let $(\nu_1,\ldots,\nu_n)\in\Delta_0^n$;
        we have
        \[
          \begin{aligned}
            F_\theta(\tau\cdot\varepsilon)\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n})
            &\overset{\text{by~(1)}}{=} j_{\tau\cdot\varepsilon\cdot(\nu_1+\ldots+\nu_n)}
          \\&= j_{\tau\cdot(\varepsilon_1+\ldots+\varepsilon_{n'})\cdot(\nu_1+\ldots+\nu_n)};
          \end{aligned}
        \]
        since, by regrouping the terms, we have $\nu_1+\ldots+\nu_n=\mu_1+\ldots+\mu_{n'}$, where
        \[
          \begin{aligned}
            \mu_1 &= \nu_1+\ldots+\nu_{p_1}
          \\\mu_2 &= \nu_{p_1+1}+\ldots+\nu_{p_1+p_2}
        ...
        \[
          \begin{aligned}
            j_{\tau\cdot(\varepsilon_1+\ldots+\varepsilon_{n'})\cdot(\nu_1+\ldots+\nu_n)}
            &= j_{\tau\cdot(\varepsilon_1\cdot\mu_1+\ldots+\varepsilon_{n'}\cdot\mu_{n'})}
          \\&= F_\theta\tau\cdot(j_{\varepsilon_1\cdot\mu_1}\circ\ldots\circ j_{\varepsilon_{n'}\cdot\mu_{n'}})
          \\&= F_\theta\tau\cdot\Big( \big(F_\theta\varepsilon_1\cdot(j_{\nu_1}\circ\ldots j_{\nu_{p_1}})\big) \circ \big(F_\theta\varepsilon_2\cdot(j_{\nu_{p_1+1}}\circ\ldots)\big) \circ \ldots \Big)
          \\&= F_\theta\tau\cdot(F_\theta\varepsilon_1\circ F_\theta\varepsilon_2\circ\ldots\circ F_\theta\varepsilon_{n'})\cdot(j_{\nu_1}\circ j_{\nu_2}\circ\ldots\circ j_{\nu_n})
          \\&= F_\theta\tau\cdot F_\theta\varepsilon\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n})
          \end{aligned}
        \]
        and, since the $j_{\nu_1}\circ\ldots j_{\nu_n}$ run over a system of canonical injections of the sum $\bigcirc^n\hat{\theta}$ (\hyperref[lemma:2]{Lemma~2}), we can simply the extremal terms of this sequence of equalities and obtain~(3).
        In summary, $F_\theta\colon\overline{\Delta}\to\cat{D}$ is a $\overline{\Delta}$-structure.

      \item
        \emph{$F_\theta$ is the free $\overline{\Delta}$-structure generated by $\theta$ and admitting $I\colon\theta\to\hat{\theta}$ as an adjunction morphism, by setting $I=j_1$.}

        \oldpage{314}
        In other words, we will show that, for every $\overline{\Delta}$-structure $F'$ and every morphism $I'\colon\theta\to\theta'$ in $\cat{D}$ such that $\theta'=F'1$, there exists a unique morphism $m\colon\theta\to\hat{\theta}$ such that
        \[
          m\cdot I = I'
        \]
        and such that
        \[
          (\bigcirc^n m\mid\tau\in\Delta_0)\colon F_\theta\to F'
        \]
        is a natural transformation, denoting by $n$ the source of $\tau$, which implies that, for all $\varepsilon\colon n\to n'$, we have
        \[
          (\bigcirc^{n'}m)\cdot F_\theta\varepsilon
          = F'\varepsilon\cdot(\bigcirc^n m).
          \tag{4}
        \]

        Let $m\colon\hat{\theta}\to\theta'$ be the (sum) crochet characterised by the relations
        \[
          m\cdot j_\tau
          = F'\tau\cdot(\bigcirc^n I')
          \tag{5}
        \]
        for every morphism $\tau\colon n\to 1$.
        We will show relation~(4).
        We write
        \[
          \varepsilon
          = \varepsilon_1 + \ldots + \varepsilon_{n'}
        \]
        where $\varepsilon_{i'}\colon p_{i'}\to 1$ (for $1\leqslant i'\leqslant n'$), and let
        \[
          (\nu_1,\ldots,\nu_n)
          \in\Delta_0^n
        \]
        where $\nu_i\colon q_i\to 1$ (for $1\leqslant i\leqslant n$).
        We have
        \[
          \begin{aligned}
            &(\bigcirc^{n'}m)\cdot F_\theta\varepsilon\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n})
          \\&\overset{\text{regrouping}}{=} (\bigcirc^{n'}m)\cdot \Big( \big(F_\theta\varepsilon_1\cdot(j_{\nu_1}\circ\ldots j_{\nu_{p_1}})\big) \circ \big(F_\theta\varepsilon_2\cdot(j_{\nu_{p_1+1}}\circ\ldots)\big) \circ\ldots \Big)
          \\&= \big(m\cdot F_\theta\varepsilon_1\cdot(j_{\nu_1}\circ\ldots j_{\nu_{p_1}})\big) \circ \big(m\cdot F_\theta\varepsilon_2\cdot(j_{\nu_{p_1+1}}\circ\ldots)\big) \circ \ldots
          \end{aligned}
        \]
        and, if we suppose~(4) to be true for $n'=1$, then this latter expression is equal to
        \[
          \begin{aligned}
            &\big( F'\varepsilon_1\cdot(\bigcirc^{p_1}m) \cdot (j_{\nu_1}\circ\ldots\circ j_{\nu_{p_1}}) \big) \circ \big( F'\varepsilon_2\cdot(\bigcirc^{p_2}m)\cdot(j_{\nu_{p_1+1}}\circ\ldots) \big) \circ\ldots
          \\&= (F'\varepsilon_1\circ F'\varepsilon_2\circ\ldots)\cdot(\bigcirc^{p_1+\ldots+p_n}m)\cdot(j_{\nu_1}\circ j_{\nu_2}\circ\ldots)
          \\&= F'\varepsilon\cdot(\bigcirc^n m)\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n}).
          \end{aligned}
        \]
        Then, simplifying by $j_{\nu_1}\circ\ldots\circ j_{\nu_n}$, we obtain~(4).
        We now prove~(4) for $n'=1$.
        We have
        \[
          \begin{aligned}
            m\cdot F_\theta\varepsilon\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n})
            &= m\cdot j_{\varepsilon\cdot(\nu_1+\ldots+\nu_n)}
          \\&\overset{\text{(5)}}{=} F'\big(\varepsilon\cdot(\nu_1+\ldots+\nu_n)\big)\cdot(\bigcirc^{q_1+\ldots+q_n}I')
          \\&= F'\varepsilon\cdot(F\nu_1\circ\ldots\circ F\nu_n)\cdot\big((\bigcirc^{q_1}I')\circ\ldots\circ(\bigcirc^{q_n}I')\big)
          \\&= F'\varepsilon\Big( \big(F\nu_1\cdot(\bigcirc^{q_1}I')\big)\circ\ldots\circ\big(F\nu_n\cdot(\bigcirc^{q_n}I')\big) \Big)
          \\&\overset{\text{(5)}}{=} F'\varepsilon\cdot\big( (m\cdot j_{\nu_1})\circ\ldots\circ(m\cdot j_{\nu_n}) \big)
          \\&= F'\varepsilon\cdot(\bigcirc^n m)\cdot(j_{\nu_1}\circ\ldots\circ j_{\nu_n})
          \end{aligned}
        \]
        \oldpage{315}
        which, after simplifying by $j_{\nu_1}\circ\ldots j_{\nu_n}$, finishes the proof of the existence of the morphism $m\colon F_\theta\to F'$ in $\Str{\overline{\Delta}}{\overline{\cat{D}}}$, since for $\tau=1$ in~(5) we obtain that $m\cdot I=I'$.

        This morphism is unique.
        Indeed, let $m'\colon F_\theta\to F'$ be such that we have $m'\cdot I=I'$.
        Applying~(1) in the particular case where $\nu_1=\nu_2=\ldots=\nu_n=1$, we obtain
        \[
          F_\theta\tau\cdot(\bigcirc^n I)
          = j_{\tau\cdot n}
          = j_\tau
        \]
        and thus
        \[
          \begin{aligned}
            m'\cdot j_\tau
            &= m'\cdot F_\theta\tau\cdot(\bigcirc^n I)
          \\&\overset{\text{by definition of }m'}{=} F'\tau\cdot(\bigcirc^n m')\cdot(\bigcirc^n I)
          \\&= F'\tau\cdot(\bigcirc^n I')
          \\&\overset{\text{(5)}}{=} m\cdot j_\tau.
          \end{aligned}
        \]
        Simplifying by $j_\tau$, we obtain that $m=m'$.

      \item
        Such a correspondence (which associates a $\overline{\Delta}$-structure $F_\theta$ to each object $\theta$) extends to an adjoint functor $\cat{D}\to\Str{\overline{\Delta}}{\overline{\cat{D}}}$ to the forgetful functor $\Str{\overline{\Delta}}{\overline{\cat{D}}}$.
        From this adjoint pair, we obtain a triple that we denote by $\TT_{\overline{\Delta}}(\overline{\cat{D}})=(T,I,K)$.
        The morphism that was denoted by $j_1$ will now be denoted by $I\theta$.
        We will construct a functor
        \[
          E'\colon \Alg{\TT_{\overline{\Delta}}(\overline{\cat{D}})} \to \Str{\overline{\Delta}}(\overline{\cat{D}}).
        \]
        Let $b\colon T\theta\to\theta$ be a $\TT_{\overline{\Delta}}(\overline{\cat{D}})$-algebra and set, for all $\varepsilon\colon n\to n'$,
        \[
          F\varepsilon
          = (\bigcirc^{n'} b)\cdot F_\theta\varepsilon\cdot(\bigcirc^n I\theta).
        \]
        We will show that this formula defines a $\overline{\Delta}$-structure $F$ on $\theta$.
        Firstly, $F$ commutes with the first laws;
        this is shown by demonstrating that, if $\varepsilon=\varepsilon_1+\ldots+\varepsilon_n$ is the unique decomposition of $\varepsilon$, then
        \[
          F\varepsilon
          = F\varepsilon_1\circ F\varepsilon_2\circ\ldots\circ F\varepsilon_{n'}.
        \]
        We now show that $F\colon\Delta\to\cat{D}$ is a functor.
        For any integer $n$, we have that $Fn=\bigcirc^n(F1)$.
        If $\varepsilon\colon n\to n'$ and $\varepsilon'\colon n'\to n''$ are ``consecutive'' morphisms in $\Delta$, then
        \oldpage{316}
        \[
          \begin{aligned}
            F\varepsilon'\cdot F\varepsilon
            &= (\bigcirc^{n''}b)\cdot F_\theta\varepsilon'\cdot(\bigcirc^{n'}Tb)\cdot(\bigcirc^{n'}IT\theta)\cdot F_\theta\varepsilon\cdot(\bigcirc^n I\theta)
          \\&= (\bigcirc^{n''}b)\cdot(\bigcirc^{n''}Tb)\cdot F_{T\theta}\varepsilon'\cdot(\bigcirc^{n'}IT\theta)\cdot F_\theta\varepsilon\cdot(\bigcirc^{n}I\theta)
          \\&= \big( \bigcirc^{n''}(b\cdot Tb) \big)\cdot F_{T\theta}\varepsilon'\cdot(\bigcirc^{n'}IT\theta)\cdot F_\theta\varepsilon\cdot(\bigcirc{n}I\theta)
          \\&= \big( \bigcirc^{n''}(b\cdot K\theta) \big)\cdot F_{T\theta}\varepsilon'\cdot(\bigcirc^{n'}IT\theta)\cdot F_\theta\varepsilon\cdot(\bigcirc^{n}I\theta)
          \\&= (\bigcirc^{n''}b)\cdot(\bigcirc^{n''}K\theta)\cdot F_{T\theta}\varepsilon'\cdot(\bigcirc^{n'}IT\theta)\cdot F_\theta\varepsilon\cdot(\bigcirc^n I\theta)
          \\&= (\bigcirc^{n''}b)\cdot F_\theta\varepsilon'\cdot(\bigcirc^{n'}K\theta)\cdot(\bigcirc^{n'}IT\theta)\cdot F_\theta\varepsilon\cdot(\bigcirc^n I\theta)
          \\&= (\bigcirc^{n''}b)\cdot F_\theta\varepsilon'\cdot F_\theta\varepsilon\cdot(\bigcirc^n I\theta)
          \\&= (\bigcirc^{n''}b)\cdot F_\theta(\varepsilon'\cdot\varepsilon)\cdot(\bigcirc^{n}I\theta)
          \\&= F(\varepsilon'\cdot\varepsilon).
          \end{aligned}
        \]
        Thus $F$ is a $\overline{\Delta}$-structure.
        Set $E'(b)=F$;
        we can easily extend $E'$ to morphisms by noting that a morphism in $\TT_{\overline{\Delta}}(\overline{\cat{D}})$ is a $\TT_{\overline{\Delta}}(\overline{\cat{D}})^\mathbf{2}$-algebra.

        We can easily show that $E'$ is the inverse of the Eilenberg--Moore functor by an analogous argument to that of \hyperref[lemma:1]{Lemma~1}.\qedhere
    \end{enumerate}
\end{proof}

\begin{itenv}{Proposition~A.3}
\label{proposition:a.3}
  To every pseudo-functor (\cref{sec:II.1})
  \[
    (F,u,c)\colon (\cat{D},1,\circ) \to (\cat{D}',1,\circ)
  \]
  between countably distributive strict monoidal categories, we can associate a morphism of triples $\TT_{\overline{\Delta}}(\overline{\cat{D}})\to\TT_{\overline{\Delta}}(\overline{\cat{D}}')$.
\end{itenv}

\begin{proof}
  If $\theta_1,\theta_2,\ldots,\theta_n,\ldots$ are objects in $\cat{D}$, set
  \[
    \begin{aligned}
      c_0(0)
      &= u
      \qquad(\text{where }0\text{ denotes the unique morphism in }\mathbf{1})
    \\c_1(\theta_1)
      &= \id_{F\theta_1}
    \\c_2(\theta_1,\theta_2)
      &= c(\theta_1,\theta_2)
    \\c_{n+1}(\theta_1,\theta_2,\ldots,\theta_n,\theta_{n+1})
      &= c_n(\theta_1,\ldots,\theta_n\circ\theta_{n+1})\cdot\big( F(\theta_1)\circ\ldots\circ F(\theta_{n-1})\circ c(\theta_n,\theta_{n+1}) \big)
    \end{aligned}
  \]
  for all integers $n\geqslant 3$.
  Analogously, set
  \[
    \begin{aligned}
      \bigcirc_0 0
    ...
    \\\bigcirc_1\theta_1
      &= \theta_1
    \\\bigcirc_2(\theta_1,\theta_2)
      &= \theta_1\circ\theta_2
    \\\bigcirc_{n+1}(\theta_1,\ldots,\theta_n,\theta_{n+1})
      &= \big( \bigcirc_n(\theta_1,\ldots,\theta_n) \big)\circ\theta_{n+1}.
    \end{aligned}
  \]
  \oldpage{317}
  We thus define, for all $n\geqslant0$, a functor $\bigcirc_n\colon\cat{D}^n\to\cat{D}$ and a natural transformation $c_n\colon\bigcirc_n F^n\to F\bigcirc_n$ called the ``$n$-th coherence''.
  We assume the follow formula from ``coherence theory'', for all $\theta\in\set{\cat{D}}$ and $(p_1,p_2,\ldots,p_n)\in\mathbb{N}^n$, where $n\in\mathbb{N}$,
  \[
    c_n(\bigcirc^{p_1}\theta,\ldots,\bigcirc^{p_n}\theta)\cdot(d_{p_1}\theta\circ\ldots\circ d_{p_n}\theta)
    = d_{p_1+\ldots+p_n}\theta
    \tag{5}
  \]
  where we set $d_n\theta=c_n(\theta,\theta,\ldots,\theta)$;
  we thus define a natural transformation $d_n\colon\bigcirc_n F\to F\bigcirc_n$.
  (We simply note that this formula can easily be deduced from \hyperref[lemma:4]{Lemma~4}, which corresponds to the case where $\cat{D}=1$).

  For all $\theta\in\set{\cat{D}}$, let $j_\varepsilon\theta\colon\bigcirc^n\theta\to T\theta$ be the morphism denoted by $j_\varepsilon$ in \hyperref[proposition:a.1]{Proposition~A.1}.
  We define natural transformations $j_\varepsilon\colon\bigcirc_n\to T$;
  let $d\colon TF\to FT$ be the natural transformation such that $d\theta\colon TF\theta\to FT\theta$ is, for all $\theta\in\set{\cat{D}}$, the (sum) crochet characterised by
  \[
    d\theta\cdot j_\varepsilon F\theta
    = Fj_\varepsilon\theta\cdot d_n\theta
    \tag{6}
  \]
  for all $\varepsilon\colon n\to 1$.
  We have that $d\cdot j_\varepsilon F=Fj_\varepsilon\cdot d_n$.

  We now show that $(F,d)\colon\TT_{\overline{\Delta}}(\overline{\cat{D}})\to\TT_{\overline{\Delta}}(\overline{\cat{D}}')$ is a morphism of triples.
  We have that $d\theta\cdot IF\theta = FI\theta$ by taking $\varepsilon=1$ in~(6), hence the first of the relations~(2) in \cref{sec:0.2}.
  For all
  \[
    (\varepsilon_1,\ldots,\varepsilon_n)\in\Delta_0^n
    \textand
    \tau\colon n\to 1
  \]
  where $\varepsilon_i\colon p_i\to 1$ (for $1\leqslant i\leqslant n$), we have, by the definition of $K$,
  \[
    \begin{aligned}
      &d\theta\cdot KF\theta\cdot j_\tau TF\theta\cdot(j_{\varepsilon_1}F\theta\circ\ldots\circ j_{\varepsilon_n}F\theta)
    \\&= d\theta\cdot j_{\tau\cdot(\varepsilon_1+\ldots+\varepsilon_n)}F\theta
    \\&\overset{\text{(6)}}{=} Fj_{\tau\cdot(\varepsilon_1+\ldots+\varepsilon_n)}\theta\cdot d_{p_1+\ldots+p_n}\theta
    \\&= F\big(K\theta\cdot j_\tau T\theta\cdot(j_{\varepsilon_1}\theta\circ\ldots\circ j_{\varepsilon_n}\theta)\big)\cdot d_{p_1+\ldots+p_n}\theta
    \\&= FK\theta\cdot Fj_\tau T\theta\cdot F(j_{\varepsilon_1}\theta\circ\ldots\circ j_{\varepsilon_n}\theta)\cdot d_{p_1+\ldots+p_n}\theta
    \\&\overset{\text{(5)}}{=} FK\theta\cdot Fj_\tau T\theta\cdot F(j_{\varepsilon_1}\theta\circ\ldots\circ j_{\varepsilon_n}\theta)\cdot c_n(\bigcirc^{p_1}\theta,\ldots,\bigcirc^{p_n}\theta)\cdot(d_{p_1}\theta\circ\ldots\circ d_{p_n}\theta)
    \\&= FK\theta\cdot Fj_\tau T\theta\cdot d_n T\theta\cdot(Fj_{\varepsilon_1}\theta\circ\ldots\circ Fj_{\varepsilon_n}\theta)\cdot(d_{p_1}\theta\circ\ldots\circ d_{p_n}\theta)
    \\&= FK\theta\cdot Fj_\tau T\theta\cdot d_n T\theta\cdot\bigcirc^n d\theta\cdot(j_{\varepsilon_1}F\theta\circ\ldots\circ j_{\varepsilon_n}F\theta)
    \\&\overset{\text{(6)}}{=} FK\theta\cdot dT\theta\cdot j_\tau FT\theta\cdot \bigcirc^n d\theta\cdot(j_{\varepsilon_1}F\theta\circ\ldots\circ j_{\varepsilon_n}F\theta)
    \\&= FK\theta\cdot dT\theta\cdot Td\theta\cdot j_\tau TF\theta\cdot(j_{\varepsilon_1}F\theta\circ\ldots\circ j_{\varepsilon_n}F\theta).
    \end{aligned}
  \]
  \oldpage{318}
  Simplifying by $j_{\varepsilon_1}F\theta\circ\ldots\circ j_{\varepsilon_n}F\theta$, and then by $j_\tau F\theta$, we obtain the second of the relations~(2), i.e.
  \[
    d\theta\cdot KF\theta
    = FK\theta\cdot tD\theta\cdot Td\theta.
    \qedhere
  \]
\end{proof}

Here is a case where $\TT_{\overline{\Delta}}(\overline{\cat{D}})$ is strongly cartesian (see \cref{sec:III.1}).
Suppose that $\overline{\cat{D}}$ is countably distributive and further satisfies the following conditions:
\begin{itemize}
  \item[(D')]
    $\cat{D}$ admits (finite) fibre products;
    the functors $Y_\theta$ and $Y'_\theta$ of axiom~(T) commute with the (finite) fibre products.

  \item[(D'')]
    The sums in $\cat{D}$ commute with the (finite) fibre products, and they are universal (and thus the canonical injections of sums are monomorphisms, by the lemma of \hyperref[proposition:iii.1.20]{Proposition~III.1.20}).
\end{itemize}

\begin{itenv}{Proposition~A.4}
\label{proposition:a.4}
  If these conditions are satisfied, then $\TT_{\overline{\Delta}}(\overline{\cat{D}})$ is strongly cartesian.
\end{itenv}
...
We note in passing the following fact: if $\cat{D}$ is an arbitrary category, then the ``target'' of an inductive limit in $\cat{D}^\mathbf{2}$ is always an inductive limit;
the ``source'' too, whenever $\cat{D}$ admits a final object.



\subsection{Theories and multicategories}
\label{sec:a.3}

We will show that there is an equivalence between the data of a strict monoidal theory (\hyperref[sec:a.2]{§A.2}) and that of a multicategory in the sense of \cite{La}.
\oldpage{319}
\hyperref[proposition:iii.2.21]{Proposition~III.2.21} can be expanded ($\Kl{\TT}$ is defined in \cref{sec:0.2}):

\begin{itenv}{Proposition~A.5}
\label{proposition:a.5}
  Let $\TT$ be a cartesian triple on the category $\cat{E}$, and let $\theta=(b,a,i,k)$ be a $\TT$-category on $e\in\set{\cat{E}}$;
  we have a $\Kl{\TT}$-category on $e\in\set{\Kl{\TT}}$ that projects onto the $\cat{E}$-category $(Tb,Ke\cdot Ta,Ti,Tk)$ from \hyperref[proposition:iii.2.21]{Proposition~III.2.21} by the forgetful functor $\Kl{\TT}\to\cat{E}$.
\end{itenv}

\begin{proof}
  $T$ commutes with finite fibre products, and thus sends every $\cat{E}$-category to an $\cat{E}$-category, and it is evident that
  \[
    (Ke,K\pi)\colon
    \big(TTb,T(Ke\cdot Ta),TTi,TTk\big)
    \to (Tb,Ke\cdot Ta,Ti,Tk)
  \]
...
In particular, if $\TT$ is the path triple $\NN$ from \cref{sec:III.3}, then we will see that this corresponds to axiom~(T) from \hyperref[sec:a.2]{§A.2}.

We have an evident isomorphism $\Cat{Set}\to\Cat{Gr}/1$ that, to each set $E$, associates the unique graph $\vec{E}$ on $1$ whose set of morphisms is $E$.
$\vec{N1}$ endowed with its free monoid structure is isomorphic to the set of positive or zero integers endowed with ordinary addition;
we can identify these two structures.
Then $0$ denotes both the object of $\vec{N1}$ and the identity morphism $\id_0$, which recovers the usual convention.

\begin{itenv}{Proposition~A.6}
\label{proposition:a.6}
  There is an equivalence between the data of a Lambek multicategory on $1$ and that of a strict monoidal theory.
  The unique $\NN$-algebra on $1$ corresponds to the ``theory of monads'' (which is also the \unsure{basic/rough} multicategory on $1$!).
\end{itenv}

\begin{proof}
  Let $\theta=(b,a,i,k)$ be a Lambek multicategory on $1$.
  Let $\vec{\Delta}_0$ be the morphism object (which is a graph on $1$ with $\Delta_0$ as its set of morphisms) of this multicategory.
  \hyperref[proposition:iii.2.21]{Proposition~III.2.21} shows us how to construct a $\Cat{Gr}/1$-category and thus, by the isomorphism $\Cat{Gr}/1\to\Cat{Set}$, a category whose objects are the positive or zero integers and whose morphisms are those of $N\vec{\Delta}_0$.
  \oldpage{320}
  \hyperref[proposition:a.5]{Proposition~A.5} shows us how we can complete this data to a monoidal category $\overline{\Delta}$, with the monoid structure on the objects and the arrows of $\Delta$ being, furthermore, free, which expresses axiom~(T).

  Conversely, if $\overline{\Delta}=(\Delta_0,1,+)$ is a strict monoidal theory, then the construction of a multicategory in the sense of \cite{La} is immediate, and the correspondence thus obtained is a bijection.
\end{proof}

This result suggests that there are many theories, and can serve as a guide to their construction.
We thus note that the notion of $\TT$-category is nothing more than one ``$\TT$-structure'' amongst the many.
The structure of a $\TT$-graph is another example.

\begin{rmenv}{Examples of strict monoidal theories}
  (We leave to the reader the task of defining composition in each case).

  \begin{enumerate}
    \item \emph{Pointed object.}
      $\Delta_0$ consists of two elements:
      \[
        \id_1\colon 1\to1
        \textand
        \iota\colon 0\to1.
    ...
    \item \emph{Modules.}
      $\Delta_0$ is a set of morphisms of the form $\lambda\colon1\to1$ and this set is endowed with the structure of a monoid, with unit $\id_1\colon1\to1$.
  \end{enumerate}
\end{rmenv}
