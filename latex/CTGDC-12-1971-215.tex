...
\begin{enumerate}
  \item $\set{\cat{E}}$ is a set; its elements are called \emph{objects of $\cat{E}$}.

  \item $\Hom_\cat{E}$ is a family of sets (that we suppose to be disjoint from one another, and disjoint from $\set{\cat{E}}$, to simplify constructions; but this is rarely the case in practice and, in theory, it is not indispensable).
  ...
  \item If $f\colon e\to e'$ is a morphism in $\cat{E}$, then
    \[
      f\cdot\id_e
      = f
      = \id_{e'}\cdot f.
  ...
  \item If $f\colon e\to e'$, $g\colon e'\to e''$, and $h\colon e''\to e'''$ are ``consecutive'' morphisms in $\cat{E}$, then
    \[
      h\cdot(g\cdot f)
      = (h\cdot g)\cdot f,
    \]
...
\begin{enumerate}
  \item[1\textquotesingle.] $\set{F}\colon\set{\cat{E}}\to\set{\cat{E}'}$ is a map.
  ...
  \item[3\textquotesingle.] $F(\id_e)=\id_{F(e)}$ for all $e\in\set{\cat{E}}$.

  \item[4\textquotesingle.] $F(g\cdot f)=F(g)\cdot F(f)$, if $f\colon e\to e'$ and $g\colon e'\to e''$ are consecutive morphisms in $\cat{E}$.
\end{enumerate}

A \emph{natural transformation $I\colon F\to F'$}, where $F\colon\cat{E}\to\cat{E}'$ and $F'\colon\cat{E}\to\cat{E}'$ are functors with the same source and target, is given by a family of morphisms in $\cat{E}'$ of the form $I(e)\colon F(e)\to F'(e)$, where the index $e$ runs over the set $\set{\cat{E}}$, such that for every morphism $f\colon e\to e'$ in $\cat{E}$ we have the relation $I(e')\cdot F(f)=F'(f)\cdot I(e)$.
\oldpage{221}
