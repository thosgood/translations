...
\begin{rmenv}{Remark}
  ...
  the object $e$ is denoted $\set{\theta}$ and called the \emph{object of objects}, and the object $\pi$ is called the \emph{object of morphisms}.
...
If $\theta=(b,a)$ and $\theta'=(b',a')$ are $\TT$-graphs, then a \emph{morphism} $f$ from $\theta$ to $\theta'$, also denoted $(f_0,f_1)\colon\theta\to\theta'$, is determined by a pair of morphisms $f_0\colon e\to e'$ and $f_1\colon\pi\to\pi'$ in $\cat{E}$ between the objects of objects and the objects of morphisms (respectively) that satisfies the following relations:
\[
  \begin{aligned}
    b'\cdot f_1
    &= f_0\cdot b
