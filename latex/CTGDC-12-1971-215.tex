...
\begin{proof}
  ...
  we will define an adjoint functor $F\colon\Gr{\TT}\to\Cat{Cat}(\TT)$ by using \cref{proposition:a.1} in the appendix.
  ...
  To show that the functor $\Cat{Cat}(\TT)/e\to\Gr{\TT}/e$ admits an adjoint, it thus suffices to show that $\overline{\Gr{\TT}/e}$ is countably distributive (\cref{proposition:a.1}).
  ...
  \[
    ...
    \to \Gr{\TT^\mathbf{2}}.
  \]
  The way in which the adjoint of \cref{proposition:a.1} is constructed leads us to define $\set{F^\mathbf{2}}$ as the extension of the maps $\set{\hat{F}_f}$, where $f$ runs over $\set{\cat{E}^\mathbf{2}}$, and where we denote by $\hat{F}_f$ the adjoint of the forgetful functor
  \[
...
\begin{proof}
  If $\TT'=(T',I',K')$ is the induced triple on $\Gr{\TT}$, then the proof of \cref{proposition:a.1} in the appendix shows that, for all $\theta\in\set{\Gr{\TT}}$, $T'\theta$ is a countable sum, $I'\theta$ is a canonical injection of this sum, and $K'\theta$ is a crochet of the sum $T'T'\theta$;
...
The essential result of this appendix is \cref{proposition:a.2}, but the only result used in the article is \cref{proposition:a.1}, which is used in the Lemma of \cref{proposition:III.1.19}.
...
\begin{rmenv}{Remarks}
  \begin{enumerate}
    ...
    \item
      If $\overline{\cat{D}}$ is not countably distributive, then the forgetful functor $\Mon{\overline{\cat{D}}}\to\cat{D}$ can admit an adjoint that is not of the form obtained in \cref{proposition:a.1}.
      For example, if we take $\overline{\cat{D}}=\Gr{\TT_0}/e$, where $\TT_0=(T_0,I_0,K_0)$ is the obvious triple on $\cat{E}$ such that, for all $e'\in\set{\cat{E}}$, $T_0e'$ is equal to a final object of $\cat{E}$, then the adjunction morphism in \cref{proposition:a.1} will be a canonical injection $j_1\colon\theta\to\theta+1$.
  \end{enumerate}
\end{rmenv}
