...
\begin{enumerate}
  \item $a\cdot t = \id_{Te}$
  \item $v_2\cdot d = I\pi\cdot t\cdot a$ and $k\cdot d = \id_\pi$.
\end{enumerate}
...
\begin{enumerate}
  \item $\Ord{\TT_\mathcal{U}}$ is isomorphism to the category of topologies, by \cite{Ba} and \unsure{Proposition~I.4.4} (we have given another proof, inspired by the work of \cite{Bu} and edited by D.~Tanré).

  \item Hypertopologies in finite sets are precisely those categories with a finite number of objects (which shows, in particular, that finite topologies are precisely finite preorders).
\end{enumerate}
