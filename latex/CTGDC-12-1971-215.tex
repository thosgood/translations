...
Let $\cat{A^\op}=(b^\op,a^\op)$ be a graph.
An $\NN$-graph $\cat{A}=(b,a)$ on $\cat{A}^\op$ is given by a diagram in $\Cat{Gr}$
\oldpage{277}
\[
  \begin{tikzcd}
    N\cat{A}^\op
...
\[
  \begin{gathered}
    b^\cdot = \set{b}
  \\a^\cdot = \set{a}
  \\\set{\cat{A}} = \set{\cat{A}^\op} = \set{\cat{A}^\cdot}
  \end{gathered}
  \qquad\qquad
...
The morphisms from $\cat{A}^\cdot$ and from $\cat{A}^\op$ are called the \emph{morphisms} and \emph{tangles} (or arrows, or promorphisms) respectively.
...
we define, by this correspondence, the morphism $\hat{i}\colon\cat{A}^\op\to\hat{\pi}$ in $\Cat{Gr}$.
\oldpage{279}
