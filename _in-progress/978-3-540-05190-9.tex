...
\label{I.2.2}
  ...
  The old school would have defined a (holomorphic) connection on $\scr{V}$ as the data, for every pair of points $(x,y)$ that are first order infinitesimal neighbours in $X$, of an isomorphism $\gamma_{y,x}\colon\scr{V}_x\to\scr{V}_y$ that depends holomorphically on $(x,y)$ and is such that $\gamma_{x,x}=\id$.
...
\label{I.2.10}
  ...
  Define $\Omega_X^p=\bigwedge^p\Omega_X^1$ and $\Omega_X^p(\scr{V})=\Omega_X^p\otimes_\cal{O}\scr{V}$ (the sheaf of \emph{exterior differential $p$-forms with values in $\scr{V}$}).
...
\oldpage{17}
  ...
  \begin{itemize}
    \item[] \textbf{Case 1:} \emph{$S=D^n$, $X=D^{n+1}=D^n\times D$, $f=\pr_1$, and $\scr{V}$ is free.}
...
\emph{[Translator] Theorem~1.23 and Proposition~1.24 have been removed from this edition, due to the following comment from the errata, written by P.~Deligne at Warwick University in April 1971:}
\begin{quote}
  \itshape
...
\label{II.2.12}
  Under the hypotheses of \hyperref[II.2.2]{(2.2)}, let $\scr{V}$ be a coherent analytic sheaf on $X^*$, and $\scr{V}_1$ and $\scr{V}_2$ extensions of $\scr{V}$ to a coherent analytic sheaf on $X$.
  Then the following conditions are equivalent.
  \begin{enumerate}[(i)]
    \item There exists an extension $\scr{V}'$ of $\scr{V}$ on $X$, as well as homomorphisms from $\scr{V}'$ to $\scr{V}_1$ and to $\scr{V}_2$.
    \item There exists an extension $\scr{V}''$ of $\scr{V}$ on $X$, as well as homomorphisms from $\scr{V}_1$ and from $\scr{V}_2$ to $\scr{V}''$.
...
\begin{proof}
  ...
  \[
    \scr{V}'' = (\scr{V}_1\oplus \scr{V}_2)/\scr{V}'
  \]
  where
  \[
    \scr{V}' = \scr{V}_1\cap \scr{V}_2.
  \]
  If (i) is locally true, then a global solution is given by the sum of the images of $\scr{V}_1$ and $\scr{V}_2$ in $j_*\scr{V}$, where $j$ is the inclusion of $X^*$ into $X$.
\end{proof}

\begin{rmenv}{2.13}
\label{II.2.13}
  We say that two extensions $\scr{V}_1$ and $\scr{V}_2$ of $\scr{V}$ are \emph{meromorphically equivalent} if the conditions of \hyperref[II.2.2]{(2.2)} are satisfied;
  we say that a coherent analytic sheaf on $X^*$ is \emph{meromorphic along $Y$} if it is locally endowed with an equivalence class of extensions of $\scr{V}$ to a coherent analytic sheaf on $X$.
  If there exists an extension of $\scr{V}$ on $X$ that is, locally on $Y$, meromorphically equivalent to the pre-given extensions, then this extension is unique up to meromorphic equivalence;
  we then say that $\scr{V}$ is \emph{effectively meromorphic along $Y$}.
\oldpage{66}
  ...
  Let $\scr{V}$ be a coherent analytic sheaf on $X^*$ that is meromorphic along $Y$;
  a \emph{section} $v\in\HH^0(X^*,\scr{V})$ is said to be \emph{meromorphic along $Y$} if, locally on $X$, it is defined by a section of one of the pre-given extensions of $\scr{V}$.
  The information of the sheaf $j_*^\mathrm{mero}\scr{V}$ on $X$ of sections of $\scr{V}$ which are meromorphic along $Y$ is equivalent to the information of the meromorphic structure along $Y$ of $\scr{V}$.
\end{rmenv}

\begin{rmenv}{2.14}
\label{II.2.14}
  Suppose that $X^*$ is reduced, and let $\scr{V}$ be a vector bundle on $X^*$ which is meromorphic along $Y$.
  We will define an equivalence class of ``norms'' on $\scr{V}$.
  The ``norms'' in question will be continuous families of norms on the $\scr{V}_x$ (for $x\in X^*$).
  If $v$ is a continuous section of $\scr{V}$, then $|v|$ is a positive function on $X^*$, which is zero at exactly the points where $v=0$.
...
\begin{rmenv}{2.15.1}
\label{II.2.15.1}
  If $\scr{V}=\cal{O}^n$, then we set $|v|=\sum|v_i|$.
\end{rmenv}

\begin{rmenv}{2.15.2}
\label{II.2.15.2}
  Let $x\in Y$, and let $\scr{V}_1$ be a pre-given extension of $\scr{V}$ on a neighbourhood of $x$.
  Then there exists an open neighbourhood $U$ of $x$, along with $\omega\colon\scr{V}_1\to\cal{O}^n$, such that $\omega$ is a monomorphism on $U\cap X^*$.
  We set
  \[
    |v|_\omega = |\omega(v)|
  \]
...
\begin{rmenv}{2.15.3}
\label{II.2.15.3}
  For $x$ and $\scr{V}_1$ as in \hyperref[II.2.15.1]{(2.15.1)}, there exists a neighbourhood $U$ of $x$, and an epimorphism $\eta\colon\cal{O}^n\to\scr{V}_1$ on $U$.
  We set
  \[
...
\label{II.2.15.4}
  ...
  \[
    \cal{O}^n \xrightarrow{\eta} \scr{V} \xrightarrow{\omega} \cal{O}^m
  \]
...
\begin{itenv}{Proposition 2.16}
\label{II.2.16}
  Under the hypotheses of \hyperref[II.2.14]{(2.14)}, there exists exactly one equivalence class \hyperref[II.2.14.1]{(2.14.1)} of norms on $\scr{V}$ which are locally equivalent (in the sense of \hyperref[II.2.14.1]{(2.14.1)}) to the norms \hyperref[II.2.15.2]{(2.15.2)} and \hyperref[II.2.15.3]{(2.15.3)}.
\end{itenv}
...
\label{II.2.17}
  Under the hypotheses of \hyperref[II.2.14]{(2.14)}, let $|v|$ be a moderate norm on $\scr{V}$, $\pi\colon\widetilde{X}^*\to X^*$ a covering of $X^*$, and $v$ a continuous section of $\pi^*\scr{V}$.
...
This is the case in the following well-known proposition, which shows that the knowledge of moderate norms on $\scr{V}$ is equivalent to the knowledge of the meromorphic structure of $\scr{V}$ along $Y$.

\begin{itenv}{Proposition 2.18}
\label{II.2.18}
  Under the hypotheses of \hyperref[II.2.14]{(2.14)}, for a holomorphic section $v$ of $\scr{V}$ over $X^*$ to be meromorphic along $Y$, it is necessary and sufficient for it to be of moderate growth along $Y$.
\end{itenv}

\begin{proof}
  The claim is local on $X$, and we can reduce, by \hyperref[II.2.16]{(2.16)} and \hyperref[II.2.15.2]{(2.15.2)} to the classical case where $\scr{V}=\cal{O}$.
\end{proof}
...
\label{II.2.19}
  ...
  It is clear that {\rm(b)}$\implies${\rm(a)}.
  We have
  \begin{enumerate}
...
\oldpage{69}
  ...
  Let $\scr{V}_2$ be a vector bundle on $X_2^*$ that is meromorphic along $Y_2$, and let $\scr{V}_1$ be its inverse image.
  The inverse images of the pre-given extensions of $\scr{V}_2$ define a meromorphic structure along $Y_1$ on $\scr{V}_1$, and we have
  \begin{enumerate}
    \item[{\rm(iii\textsubscript{a})}] The inverse image of a moderate norm on $\scr{V}_2$ is a moderate norm on $\scr{V}_1$.
    \item[{\rm(iii\textsubscript{b})}] Conversely, if {\rm(a)} is satisfied, then a norm on $\scr{V}_2$ is moderate if its inverse image is.
    \item[{\rm(iii\textsubscript{c})}] In particular, if {\rm(b)} is satisfied, then the moderate norms on $\scr{V}_1=\scr{V}_2$ adapted to $X_1$ or $X_2$ agree.
  \end{enumerate}
\end{itenv}

\begin{proof}
  We trivially have that {\rm(i\textsubscript{a})}~+~{\rm(i\textsubscript{b})}$\implies${\rm(i\textsubscript{c})}, {\rm(ii\textsubscript{a})}~+~{\rm(ii\textsubscript{b})}$\implies${\rm(ii\textsubscript{c})}, {\rm(iii\textsubscript{a})}~+~{\rm(iii\textsubscript{b})}$\implies${\rm(iii\textsubscript{c})}, and almost trivially that {\rm(i\textsubscript{a})}$\implies${\rm(i\textsubscript{b})} and {\rm(i\textsubscript{a})}~+~{\rm(iii\textsubscript{a})}$\implies${\rm(iii\textsubscript{b})}.

  If $Y_2$ is defined by equations $f_i=0$, then $Y_1$ is defined by the inverse image of the $f_i$, and {\rm(i\textsubscript{a})} follows from \hyperref[II.2.5]{Definition~2.5} (along with \hyperref[II.2.4.1]{(2.4.1)}).

  By \hyperref[II.2.15.2]{(2.15.2)} and \hyperref[II.2.16]{(2.16)}, we can reduce to proving {\rm(iii\textsubscript{a})} in the trivial case where $\scr{V}=\cal{O}$.
...
\label{II.2.23}
  ...
  Let $\pi\colon\widetilde{X}^*\to(X^*)^\an$ be a covering of $(X^*)^\an$, and $\scr{V}$ an
\oldpage{71}
  ...
  \begin{enumerate}[(i)]
    ...
    \item A \emph{moderate norm on $\scr{V}$} is defined to be a moderate norm on $\scr{V}^\an$ with respect to the meromorphic structure at infinity of $\scr{V}^\an$ \hyperref[II.2.16]{(2.16)}.
    \item A continuous section $v$ of $\pi^*\scr{V}$ is said to be of \emph{moderate growth} if it is of moderate growth along $Y$ \hyperref[II.2.17]{(2.17)}.
  \end{enumerate}
\end{rmenv}
...
\label{II.2.24}
  Let $X$ be a separated scheme that is reduced and of finite type over $\CC$, and let $\scr{V}$ be an algebraic vector bundle on $X$.
  Then a holomorphic section $v$ of $\scr{V}^\an$ is algebraic if and only if it is of moderate growth.
\end{itenv}
...
\label{II.3.7}
  ...
  If $\scr{V}$ is a vector bundle on $X$, then the morphism \hyperref[II.3.7.2]{(3.7.2)} extends by linearity to
  \[
  \label{II.3.7.3}
    \Res\colon \Omega_X^1\langle Y\rangle(\scr{V}) \to \cal{O}_{\widetilde{Y}}\otimes\scr{V}.
  \tag{3.7.3}
  \]
...
\oldpage{78}
  \[
  \label{II.3.7.4}
    \Res_{Y_i}\colon \Omega_X^1\langle Y\rangle(\scr{V}) \to \scr{V}|Y_i.
  \tag{3.7.4}
  \]
\end{rmenv}
...
\begin{rmenv}{3.8}
\label{II.3.8}
  Under the hypotheses of \hyperref[II.3.1]{(3.1)}, let $\scr{V}_0$ be a vector bundle on $X*$, endowed with an integrable connection $\nabla$.
  Suppose that $\scr{V}_0$ is given as the restriction to $X^*$ of a vector bundle $\scr{V}$ on $X$.
  Locally on $X$, the choice of a basis $e$ of $\scr{V}$ allows us to define the ``connection matrix''
  \[
  \label{II.3.8.1}
    \Gamma \in j_*\Omega_{X^*}^1(\shEnd(\scr{V})).
  \tag{3.8.1}
  \]
  A change of basis $e\mapsto e'$ modifies $\Gamma$ by addition of a section of $\Omega_X^1(\shEnd(\scr{V}))$ \hyperref[I.3.1.3]{(I.3.1.3)}.
  Thus the ``polar part of $\Gamma$''
  \[
  \label{II.3.8.2}
    \dot{\Gamma} \in j_*\Omega_{X^*}^1\big(\shEnd(\scr{V}_0))\big/\Omega_X^1(\shEnd(\scr{V})\big)
  \tag{3.8.2}
  \]
  depends only on $\scr{V}$ and on $\nabla$.
  We say that \emph{the connection $\nabla$ has at worst a logarithmic pole along $Y$} if, in every local basis of $\scr{V}$, the connection forms present at worst a logarithmic pole along $Y$.
  ...
  \[
  \label{II.3.8.3}
    \Res_{Y_i}(\Gamma) \in \shEnd(\scr{V}|Y_i)
  \tag{3.8.3}
  \]
  and it depends only on $\scr{V}$ and on $\nabla$.
  More globally, if $i\colon\widetilde{Y}\to X$ is the projection of the normalisation of $Y$ onto $X$, then the residue of the connection along $Y$ is an endomorphism of $i^*\scr{V}$
  \[
  \label{II.3.8.4}
    \Res_Y(\Gamma) \in \shEnd(i^*\scr{V}).
  \tag{3.8.4}
  \]
\end{rmenv}
...
\label{II.3.9}
  ...
  \begin{enumerate}[a)]
    ...
    \item The connection on $\scr{V}$ has at worst a logarithmic pole along $Y$.
  \end{enumerate}
...
\oldpage{79}
  ...
  If a vector field $v$ satisfies \hyperref[II.3.9.1]{(3.9.1)}, and if $g$ is a section of $\scr{V}$, then $\nabla_v(g)$ is again a regular section of $\scr{V}$.
...
\label{II.3.11}
  Let $\scr{V}$ be a vector bundle on $X=D^n$, let $Y=\{0\}\times D^{n-1}$, and $X^*=X\setminus Y$, and let $\Gamma$ be an integrable connection on $\scr{V}|X^*$ presenting a logarithmic pole along $Y$.
  Let $T$ be the monodromy transformation of $\scr{V}|X^*$ defined by the positive generator of $\pi_1(X^*)\cong\pi_1(D^*)\cong\ZZ$ (cf. \hyperref[II.1.15]{(1.15)}).
  Then the horizontal automorphism $T$ of $\scr{V}|X^*$ can be extended to an automorphism of $\scr{V}$, again denoted by $T$, and
  \[
...
\label{II.3.12}
  ...
  For a vector bundle $\scr{V}$ on $X$, we denote by $j_*^\mathrm{m}j^*\scr{V}$ the sheaf of sections of $\scr{V}$ over $X^*$ that are meromorphic along $Y$.
...
\oldpage{80}
  ...
  Let $\scr{V}$ be a vector bundle on $X$, and $\Gamma$ an integrable connection on $\scr{V}|X^*$ presenting a logarithmic pole along $Y$.
  We define a filtration $P$, again called the \emph{pole-order filtration}, of the complex $j_*^\mathrm{m}j^*\Omega_X^\bullet(\scr{V})=j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V})$ by
  \[
  \label{II.3.12.2}
    P^k\big(j_*^\mathrm{m}\Omega_{X^*}^p(\scr{V})\big)
    = P^{k-p}(j_*^\mathrm{m}\cal{O}_{X^*})\otimes\Omega_X^p\otimes\scr{V}.
  \tag{3.12.2}
  \]
  ...
  \begin{enumerate}[a)]
    ...
    \item $\Omega_X^\bullet\langle Y\rangle(\scr{V})$ is a sub-complex of $j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V})$.
  \end{enumerate}

  Furthermore,
  \begin{enumerate}
    \item[{\rm c)}] the operators $\dd$ are $\cal{O}_X$-linear on the complexes $\Gr_p^n(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}))$; and
    \item[{\rm d)}] the filtration $P$ induces the \emph{Hodge filtration} $F$ on $\Omega_X^\bullet\langle Y\rangle(\scr{V})$ by the stupid truncations $\sigma_{\geq p}$, whence we have a morphism of filtered complexes
    \[
    \label{II.3.12.3}
      \big(\Omega_X^\bullet\langle Y\rangle(\scr{V}), F\big)
      \to
      \big(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}), P\big).
    \tag{3.12.3}
    \]
  \end{enumerate}
\end{rmenv}
...
\oldpage{81}
      \[
      \label{II.3.13.1}
        \Gr_F\big(\Omega_X^\bullet\langle Y\rangle(\scr{V})\big)
        \to \Gr_P\big(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V})\big).
      \tag{3.13.1}
      \]
  \end{enumerate}
\end{itenv}
...
\begin{proof}
  It suffices to prove (ii), which also implies that, for each $p$, $\Gr_P^p(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}))[p]$ is a resolution of $\Omega_X^p\langle Y\rangle(\scr{V})$.

  \begin{itemize}
    \item[] \textbf{First reduction (Extensions).}
      If $\scr{V}$ is an extension of bundles with connections satisfying \hyperref[II.3.13]{(3.13)}, as in
      \[
        0 \to \scr{V}' \to \scr{V} \to \scr{V}'' \to 0
      \]
      then the lines of the diagram
      \[
        \begin{tikzcd}
          0 \rar
          & \Gr_F\Omega_X^\bullet\langle Y\rangle(\scr{V}') \rar \dar
          & \Gr_F\Omega_X^\bullet\langle Y\rangle(\scr{V}) \rar \dar
          & \Gr_F\Omega_X^\bullet\langle Y\rangle(\scr{V}'') \rar \dar
          & 0
        \\0 \rar
          & \Gr_Pj_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}') \rar
          & \Gr_Pj_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}) \rar
          & \Gr_Pj_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}'') \rar
          & 0
        \end{tikzcd}
      \]
      are exact.
      For \hyperref[II.3.13.1]{(3.13.1)} to be a quasi-isomorphism, it thus suffices to prove the analogous claim for $\scr{V}'$ and $\scr{V}''$.
    \item[] \textbf{Second reduction (Products).}
      Suppose that $(X,Y)$ is the product of $(X_1,Y_1)$ and $(X_2,Y_2)$, and that $\scr{V}$ is the external tensor product $\scr{V}=\scr{V}_1\boxtimes\scr{V}_2$ of bundles $\scr{V}_i$ with connections satisfying the hypotheses of \hyperref[II.3.13]{(3.13)} on $(X_i,Y_i)$.
      ...
      \[
      \label{II.3.13.2}
        \Gr_F\big(\Omega_{X_1}^\bullet\langle Y_1\rangle(\scr{V}_1)\big)
        \boxtimes \Gr_F\big(\Omega_{X_2}^\bullet\langle Y_2\rangle(\scr{V}_2)\big)
        \simto \Gr_F\big(\Omega_X^\bullet\langle Y\rangle(\scr{V})\big).
      \tag{3.13.2}
      \]
...
\oldpage{82}
      ...
      \[
      \label{II.3.13.3}
        \Gr_P\big(j_*^\mathrm{m}\Omega_{X_1^*}^\bullet(\scr{V}_1)\big)
        \boxtimes \Gr_P\big(j_*^\mathrm{m}\Omega_{X_2^*}^\bullet(\scr{V}_2)\big)
        \simto \Gr_P\big(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V})\big).
      \tag{3.13.3}
      \]
      The morphism \hyperref[II.3.13.1]{(3.13.1)} can be identified, via \hyperref[II.3.13.2]{(3.13.2)} and \hyperref[II.3.13.3]{(3.13.3)}, with the external tensor product of the analogous morphisms for $\scr{V}_1$ and $\scr{V}_2$.
      Since the complexes in question have $\cal{O}_X$-linear differentials \hyperref[II.3.12]{(3.12.d)}, to prove that \hyperref[II.3.13.3] is a quasi-isomorphism, it suffices to prove the analogous claim for $\scr{V}_1$ and $\scr{V}_2$.
    \item[] \textbf{Case of constant coefficients.}
      ...
      \begin{itemize}
        ...
        \item[(3.13.6)]\label{II.3.13.6}
          $\scr{V}$ is the constant vector bundle defined by a vector space $V$, and the connection is of the form
          \[
      ...
      The bundle $\scr{V}$ with connection is then the external tensor product of the bundles $(\cal{O},\gamma_i\dd z/z)$ on $D$.
      ...
      \begin{enumerate}[a)]
        \item $P^i(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}))=0$ for $i>-1$;
\oldpage{83}
        \item $P^{-1}(j_*^\mathrm{m}\Omega_{X^*}^p(\scr{V}))$ is equal to $0$ if $p=0$, and to $\Omega_X^1\langle Y\rangle(\scr{V})$ if $p=1$;
        \item $\Gr_P^0(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}))$ is the complex
          \[
            \frac{1}{z}\cal{O}
            \xrightarrow{\partial_z+\gamma} \frac{1}{z^2}\cal{O}\bigg/\frac{1}{z}\cal{O}
          \]
          and if $\gamma-1\neq0$ then $\Coker(\dd)=0$ and $\Ker(\dd)=\scr{V}=\Omega_X^0\langle Y\rangle(\scr{V})$; and
        \item $\Gr_P^{-n}(j_*^\mathrm{m}\Omega_{X^*}^\bullet(\scr{V}))$, for $n>0$, is the complex
          \[
    ...
    \item[] \textbf{General case.}
      ...
      For $0<|t|\leq1$, let $\scr{V}_t$ be the bundle with connection given by the inverse image of $\scr{V}$ under the homothety $H_t$ with ratio $t$.
      As $t\to0$, the $\scr{V}_t$ ``tend'' to the constant vector bundle $\scr{V}_0$ defined by the fibre $V_0$ of $\scr{V}$ at $0$, endowed with a connection satisfying \hyperref[II.3.13.4]{(3.13.4)}, \hyperref[II.3.13.5]{(3.13.5)}, and \hyperref[II.3.13.6]{(3.13.6)}.
      ...
      \]
      Then $H_t=H\circ i_t$.
      The inverse image of the connection $\nabla$ on $\scr{V}$ is $\nabla_1$ on $H^*\scr{V}|H^{-1}(X^*)$.
      The corresponding relative connection (relative to $\pr_2$) extends to $H^*(\scr{V})|X^*\times D$.
      If we set $\scr{V}_t=i_t^*H^*\scr{V}$, then, for $t\neq0$, we have an isomorphism of bundles with connections
      \[
      \label{II.3.13.7}
        \scr{V}_t \cong H_t^*(\scr{V}).
      \tag{3.13.7}
      \]
...
\oldpage{84}
      \[
      \label{II.3.13.8}
        \scr{V}_0 = H_0^*(\scr{V}) = \cal{O}_X\otimes_{\CC}V_0
      \tag{3.13.8}
      \]
      and the connection on $\scr{V}_0|X^*$ satisfies \hyperref[II.3.13.4]{(3.13.4)}, \hyperref[II.3.13.5]{(3.13.5)}, and \hyperref[II.3.13.6]{(3.13.6)}.
      ...
      \label{II.3.13.9}
        \varphi\colon
        \big(
          \Omega_{X\times D/D}^\bullet\langle Y\times D\rangle(H^*\scr{V}),F
        \big)
        \to
        \big(
          j_*^\mathrm{m}\Omega_{X^*\times D}^\bullet(H^*\scr{V}),P
        \big).
      \tag{3.13.9}
      \]
      ...
      It thus follows that $i_t^*\Gr^p(\varphi)$ (the arrow \hyperref[II.3.13.1]{(3.13.1)} for $\scr{V}_t$) is a quasi-isomorphism near to $0$, for $t$ small enough.
      Since the $\scr{V}_t$ are isomorphic to one another, close to $0$, for $t\neq0$ \hyperref[II.3.13.7]{(3.13.7)}, $i_1^*\Gr^p(\varphi)$ is a quasi-isomorphism near to $0$, which proves \hyperref[II.3.13].
  \end{itemize}
\end{proof}

\begin{itenv}{Corollary 3.14}
\label{II.3.14}
  Let $X$ be a smooth scheme over $\CC$, $Y$ a normal crossing divisor in $X$, $j$ the inclusion of $X^*=X\setminus Y$ into $X$, $\scr{V}$ a vector bundle on $X$, and $\Gamma$ an integrable connection on $\scr{V}|X^*$ that presents a logarithmic pole along $Y$.
  ...
  \begin{enumerate}[(i)]
    \item the homomorphism of complexes
      \[
        i\colon \Omega_X^\bullet\langle Y\rangle(\scr{V}) \to j_*\Omega_{X^*}^\bullet(\scr{V})
      \]
...
\begin{proof}
  ...
  \[
    G^i = \Gr^i_P\big(j_*\Omega_{X^*}^\bullet(\scr{V})\big/\Omega_X^\bullet\langle Y\rangle(\scr{V})\big)
  \]
  are acyclic.
...
\label{II.3.15}
  ...
  \[
    \mathbb{H}^k\big(X,\Omega_X^\bullet\langle Y\rangle(\scr{V})\big)
    \simto \mathbb{H}^k\big(X^*,\Omega_{X^*}^\bullet(\scr{V})\big).
  \]
\end{itenv}
...
\begin{proof}
  ...
  \[
    \RR^kj_*\Omega_{X^*}^p(\scr{V})=0
    \quad\mbox{for $k>0$}
  \]
  and so
  \[
    \mathbb{H}^\bullet\big(X,j_*\Omega_{X^*}^\bullet(\scr{V})\big)
    \simto \mathbb{H}^\bullet\big(X^*,\Omega_{X^*}^\bullet(\scr{V})\big).
  \]
  Also, by \hyperref[II.3.14]{(3.14.i)}, we have
  \[
    \mathbb{H}^\bullet\big(X,\Omega_X^\bullet\langle Y\rangle(\scr{V})\big)
    \simto \mathbb{H}^\bullet\big(X,j_*\Omega_{X^*}^\bullet(\scr{V})\big)
  \]
...
\emph{[Translator] The proof of \hyperref[II.4.1]{Theorem~4.1} has been replaced with the proof given in the errata, written by P.~Deligne at Warwick University in April 1971. The errata also cites the following:}
\begin{quote}
  N.~Katz.
...
\label{II.4.1}
  ...
  Let $\scr{V}$ be a vector bundle on $X^*$ that is meromorphic along $Y$, and let $\nabla$ be a connection on $\scr{V}$.
  Then the following conditions are equivalent:
  \begin{enumerate}[(i)]
...
\oldpage{86}
    ...
    \item the (multiform) horizontal sections of $\scr{V}$ are of moderate growth along $Y$.
  \end{enumerate}
  ...
  \begin{enumerate}
    \item[(iv)] for all $y\in Y$, there exists an open neighbourhood $U$ of $y$, and a basis $e\colon\cal{O}^d\to\scr{V}$ of $\scr{V}$ on $U\setminus Y$ that is meromorphic along $Y$, such that the connection matrix (which is a matrix of differential forms) presents at worst a logarithmic pole along $Y$.
  \end{enumerate}
\end{itenv}
...
\begin{proof}
  ...
  \begin{enumerate}[a)]
    \item the meromorphic structure of $\scr{V}$ along $Y$ is defined by a coherent extension $\widetilde{\scr{V}}$ of the vector bundle $\scr{V}$ on $X$; and
  ...
  We can think of $\scr{V}$ as a vector bundle with integrable connection on $X_1^*=\pi^{-1}(X^*)$, and $\widetilde{\scr{V}}$ as its canonical extension on $X_1$.
  The vector bundle $\scr{V}_0=\pi_*\widetilde{\scr{V}}$ is a coherent extension of $\scr{V}$ on $X$.
  We will show that conditions~(i), (ii), and (iii) are all equivalent to the following:
  \begin{enumerate}
    \item[\textit{(v)}] \textit{The extensions $\widetilde{\scr{V}}$ and $\scr{V}_0$ of $\scr{V}$ are meromorphically equivalent.}
  \end{enumerate}
\end{proof}
...
\label{III.1.2}
  A section of an \emph{algebraic} vector bundle $\scr{V}$ on $X$ is said to be \emph{in the Nilsson class} if it is a multiform holomorphic section of finite determination that is of moderate growth at infinity \hyperref[II.2.23]{(II.2.23.iv)}.

  If $\scr{V}=\cal{O}$, then we speak of \emph{functions in the Nilsson class}.
\end{rmenv}
...
\label{III.1.4}
  Let $\scr{V}$ be an algebraic vector bundle on $X$.
  The ``evaluation at $w$'' function, that sends each (algebraic) $f\in\Hom(\scr{W},\scr{V})$ to the section $f(w)$ of $\scr{V}^\an$ gives a bijection between $\Hom(\scr{W},\scr{V})$ and the set of sections of $\scr{V}$ in the Nilsson class that are of monodromy subordinate to $(W_0,w_0)$.
\end{itenv}

\begin{rmenv}{1.5}
\label{III.1.5proof}
  Here we prove \hyperref[III.1.3]{(1.3)} and \hyperref[III.1.4]{(1.4)}.
  We have already seen (in \hyperref[I.6.11]{(I.6.11)}) that the function $E_w\colon f\mapsto f(w)$ identifies $\Hom(\scr{W}^\an,\scr{V}^\an)$ with the set of multiform holomorphic sections of $\scr{V}^\an$ of finite determination with monodromy subordinate to $(W_0,w_0)$.
  ...
  Trivially, \hyperref[III.1.3]{(1.3.i)}$\implies$\hyperref[III.1.3]{(1.3.ii)} and \hyperref[III.1.3]{(1.3.i)}$\implies$\hyperref[III.1.3]{(1.3.iii)}.
  ...
  For $f\colon\scr{W}^\an\to\scr{V}^\an$, the $f(e_i)$ are determinations of $f(w)$.
  From this, and from the equation $f=fee^{-1}$, we deduce that
\oldpage{124}
  \begin{enumerate}[a)]
    \item if $f(w)$ satisfies \hyperref[III.1.3]{(1.3.ii)}, then the restriction of $f\in\HH^0(\shHom(\scr{W},\scr{V})^\an)$ to any curve is of moderate growth; and
...
\label{III.1.5}
  Under the hypotheses of \hyperref[III.1.4]{(1.4)}, if $X$ is affine, with coordinate ring $A$, and if $\scr{V}$ is of rank~$m$, then the set of sections of $\scr{V}$ in the Nilsson class that are of monodromy subordinate to $(W_0,w_0)$ is a \emph{projective} $A$-module of rank~$dm$.
\end{itenv}
...
\label{III.2.1}
  ...
  If $\scr{V}$ is a local system of $\CC$-vector spaces on $S$, then the local monodromy group at $t$ acts on $V|(D\setminus\{t\})$.
  If $\scr{V}$ is the complexification of a local system of $\ZZ$-modules of finite type, then the characteristic polynomial of the monodromy transformation has integer coefficients.
