...
\label{I.1.3}
  Under the hypotheses of \cref{I.1.2}, with $X$ connected, the functor $\scr{F}\mapsto\scr{F}_{x_0}$ is an equivalence between the category of locally constant sheaves on $X$ and the category of sets endowed with an action by the group $\pi_1(X,x_0)$.
\end{proposition}

\oldpage{4}
\begin{corollary}{1.4}
\label{I.1.4}
  Under the hypotheses of \cref{I.1.2}, with $X$ connected, the functor $\scr{F}\mapsto\scr{F}_{x_0}$ is an equivalence between the category of complex local systems on $X$ and the category of complex finite-dimensional representations of $\pi_1(X,x_0)$.
\end{corollary}
...
\label{I.1.7}
  If $\scr{F}$ is a locally constant sheaf on $X$, then there exists exactly one action (said to be \emph{canonical}) of $\Pi_1(X)$ on $\scr{F}$ that, at each $x_0\in X$, induces the action from \cref{I.1.2} of $\pi_1(X,x_0)$ on $\scr{F}$.
\end{proposition}
...
\oldpage{7}
  ...
  In other words, a connection \cref{I.2.2.4}, allowing us to compare two neighbouring fibres of $\scr{V}$, also allows us to define the differential $\nabla s$ of a section of $\scr{V}$.
...
\label{I.2.4}
  ...
  \]
  that satisfies the Leibniz identity (\cref{I.2.3.5}) for local sections $f$ of $\scr{O}$ and $s$ of $\scr{V}$.
...
\label{I.2.7}
  ...
  \begin{env}{2.7.4}
  \label{I.2.7.4}
    We define a connection on the dual $\scr{V}^\vee$ of $\scr{V}$ via \cref{I.2.7.3} and the defining isomorphism $\scr{V}^\vee = \shHom(\scr{V},\scr{O})$.
    We have
    \[
  ...
  For \cref{I.2.7.2}, for example, one must verify that, firstly, the given formula defines a $\mathbb{C}$-bilinear map from $(\scr{V}_1\otimes\scr{V}_2)$, which means that the right-hand side $\II(v_1,v_2)$ is $\mathbb{C}$-bilinear and such that $\II(fv_1,v_2)=\II(v_1,fv_2)$;
...
\label{I.2.9}
  ...
  If $f$ is a homomorphism between bundles $\scr{V}_1$ and $\scr{V}_2$ endowed with connections, then it is equivalent to say either that $f$ is horizontal, or that $f$ is compatible with the connections \cref{I.2.8}.
\end{env}
...
\label{I.2.10}
  ...
  Let $\scr{V}_1$ and $\scr{V}_2$ be vector bundles endowed with connections, and let $\scr{V}$ be their tensor product \cref{I.2.7.2}.
...
\label{I.2.11}
  Under the hypotheses of \cref{I.2.10}, the \emph{curvature} $R$ of the given connection on $\scr{V}$ is the composite homomorphism
  \[
...
\oldpage{11}
  ...
  We endow $\shEnd(\scr{V})$ with the connection given in \cref{I.2.7.3}.
...
\label{I.2.19}
  Under the hypotheses of \cref{I.2.16}, the complex $\Omega_X^\bullet(\scr{V})$ is a resolution of the sheaf $\scr{V}$.
\end{proposition}
...
\label{I.2.22}
  ...
  We define, as in \cref{I.2.11,I.2.21}, the \emph{curvature} $R\in\Omega_{X/S}^2(\shEnd(\scr{V}))$ of a relative connection.
  A relative connection is said to be \emph{integrable} if $R=0$, in which case we have the \emph{relative de Rham complex with values in $\scr{V}$}, denoted by $\Omega_{X/S}^\bullet(\scr{V})$, and defined as in \cref{I.2.15,I.2.21}.
\end{env}
...
\oldpage{21}
  ...
  By \cref{I.2.25}, since $\HH^i(T,\scr{O}_S)=0$, we have that
  \[
...
\label{I.3.1}
  ...
  If $\nabla$ is the covariant derivative defined by another connection on $\scr{V}$, then we have seen \cref{I.2.6} that $\nabla$ can be written in the form
  \[
    \nabla = {}_0\!\nabla + \Gamma,
...
\oldpage{22}
  ...
  connections on $\scr{V}$ correspond, via \cref{I.3.1.2}, with $(n\times n)$-matrices of differential forms on $X$.
  If $\omega_e$ is the matrix of the connection $\nabla$ in the basis $e$, and if $f\colon\CC^n\to\simto\scr{V}$ is another basis of $\scr{V}$, with matrix $A\in\GL_n(\scr{O})$ (where $A=e^{-1}f$), then \cref{I.3.1.2}
  \[
    \begin{aligned}
      \nabla v
...
\oldpage{24}
  ...
  For every local section $s$ of $\scr{L}$, $p_2^*s$ defines a local section $\DD^n(s)$ of $P^n(\scr{L})$ \cref{I.4.2.2}.
...
\label{I.4.5}
  \emph{(Cauchy).}
  Let $X$ and $\scr{L}$ be as in \cref{I.4.2}, and let $E$ be an $n^\mathrm{th}$-order differential equation on $\scr{L}$.
  Then
  \begin{enumerate}[(i)]
...
In particular, it follows from \cref{I.4.5}~(ii) and from \cref{I.2.17} that $E$ defines a canonical connection on $P^{n-1}(\scr{L})$, whose horizontal sections are the images under $\DD$ of solutions of $E$.
...
\label{I.4.6}
  ...
  \begin{enumerate}[a)]
    ...
    \item a surjective homomorphism $\lambda\colon\scr{V}\to\scr{L}$ (by $i=0$ in \cref{I.4.2.3}).
  \end{enumerate}
...
\begin{lemma}{4.8}
\label{I.4.8}
  With the hypotheses and notation of \cref{I.4.6}, $\lambda$ is a cyclic section of $\shHom(\scr{V},\scr{L})$.
\end{lemma}
...
\begin{proof}
  ...
  For every $\scr{O}$-module $\scr{M}$, we have \cref{I.2.17}
  \[
...
\oldpage{28}
  ...
  \]
  commutes;
  if $\lambda$ is cyclic, then this, along with \cref{I.4.9.2}, implies that $\gamma^n(v)$ is locally a direct factor, of codimension~$1$ in $P^n(\scr{L})$, and admits $\omega^{\otimes n}\otimes\scr{L} \simeq \Ker(\pi_{n-1,n})$ as a complement.
  ...
  By \cref{I.4.9.1}, if $v$ is a horizontal section of $\scr{V}$, then $E\DD^n\lambda v = E\gamma^n v = 0$, and so $\lambda v$ is a solution of $E$.
  We endow $P^{n-1}(\scr{L})$ with the connection \cref{I.4.6} defined by $E$.
  ...
  \[
    ...
    \end{tikzcd}
  \]
  commutes, whence we have an isomorphism between $(\scr{V},\scr{L},\lambda)$ and the triple given by \cref{I.4.6} applied to $(\scr{L},E)$.
  The functor
  \[
...
\begin{env}{4.10}
\label{I.4.10}
  We now summarise the relations between two systems $(\scr{V},\scr{L},\lambda)$ and $(\scr{L},E)$ that correspond under \cref{I.4.6} and \cref{I.4.9}.
...
\begin{env}{5.3}
\label{I.5.3}
  By \cref{I.5.2}, there exists exactly (up to isomorphism) one pair $(u,P)$ consisting of a projective line $P$ on $S$ (with structure group $\mathrm{PGL}_2(\scr{O}_S)$) and an $S$-immersion $u$ of $X_2$ into $P$.
...
\label{I.5.11}
  ...
  We have seen, in \cref{I.4.5}, that $E$ defines a connection on the bundle $P^1(\scr{L})$ of first-order jets of sections of $\scr{L}$, and we obtain from it a connection on
  \[
  ...
  we have \cref{I.4.5} that $\scr{O}\otimes V\simto P^1(\scr{L})$, and the linear form $\lambda\colon P^1(\scr{L})\to\scr{L}$ defines a section $\lambda_0$ of the projective bundle associated to the vector bundle $P^1(\scr{L})$.
...
\begin{proof}
  ...
  \]
  If we identify $P^1(\scr{L})$ with $\scr{O}^2$, then the matrix of the connection \cref{I.5.10} defined by $E$ on $\wedge^2 P^1(\scr{L})\sim\scr{O}$ is then $-a(x)\tr M$, where $M$ is the matrix in \cref{I.4.8.1}.
...
\label{I.6.5}
  Let $X$ and $x_0$ be as in \cref{I.5.1}, and let $\scr{F}$ be a sheaf of $\CC$-vector spaces on $X$ that satisfies the principle of analytic continuation.
  ...
  \begin{enumerate}[(i)]
    ...
    \item the subsheaf of $\scr{F}$ of $\CC$-vector spaces generated by the determinations of $s$ is a complex local system \cref{I.1.1}.
  \end{enumerate}
\end{proposition}
...
\label{I.6.7}
  Under the hypotheses of \cref{I.6.5}, a multiform section $s$ of $\scr{F}$ is said to be a \emph{finite determination} if it satisfies either of the equivalent conditions of \cref{I.6.5}.
\end{definition}

\begin{env}{6.8}
\label{I.6.8}
  Under the hypotheses of \cref{I.6.5}, let $s$ be a multiform section of finite determination of $\scr{F}$.
...
\label{I.6.9}
  ...
  The multiform section $s$ of $\scr{F}$ is said to be of \emph{monodromy subordinate to $(w_0,v_0)$} if it is the finite determination, and if, with the notation of \cref{I.6.8}, there exists a homomorphism of $\pi_1(X,x_0)$-representations of $W_0$ in $V_{x_0}$ that sends $w_0$ to $v_0$.
...
\oldpage{44}
  ...
  In particular, $\dd$ is continuous and extends to $\dd\colon\scr{O}^\wedge\to\Omega^\wedge$, and the triple $(\scr{O}^\wedge,\dd,\Omega^\wedge)$ again satisfies \cref{II.1.4.1}.
\end{env}
...
We can easily show, using \cref{II.1.6}, that the triple $(\scr{O}',\dd,\Omega')$ again satisfies \cref{II.1.4.1}.
...
\begin{env}{1.8}
\label{II.1.8}
  Under the hypotheses of \cref{II.1.4}, let $V$ be a finite-dimensional vector space over $K$, and $V_0$ a lattice in $V$, i.e. a free sub-$\scr{O}$-module of $V$ such that $KV_0=V$.
...
\label{II.1.9}
  \emph{[N. Katz].}
  Under the hypotheses of \cref{II.1.4}, and with the notation of \cref{II.1.8}, let $\nabla$ be a connection \cref{II.1.2} on a vector space $V$ of dimension~$n$ over $K$.
...
Conditions~a) and b) of \cref{II.1.9} are more manageable in a different form:

\begin{lemma}{1.9.1}
\label{II.1.9.1}
  Let $V_0$, $\tau$, and $e$ be as in \cref{II.1.9}.
  ...
  \tag{1.9.2}
  \]
  The bound~a) is equivalent to the same bound \cref{II.1.9.2} for $r=0$.
\end{lemma}

\begin{proof}
  Going from \cref{II.1.9} to \cref{II.1.9.2} is clear, as is the converse for $r=0$.
  So suppose that \cref{II.1.9.2} holds true for $r>0$ and some value $C_0$ for the constant.
  We have
  \[
  \label{II.1.9.a}
...
\oldpage{46}
  ...
  The inequalities \hyperref[II.1.9.a]{(a)} and \hyperref[II.1.9.b]{(b)} then imply the inequality of the form \cref{II.1.9}:
  \[
...
\begin{lemma}{1.9.3}
\label{II.1.9.3}
  Let $(V_0,\tau_0,e_0)$ and $(V_1,\tau_1,e_1)$ be two systems as in \cref{II.1.9}.
  Then
  \[
...
\begin{proof}
  It suffices to establish an inequality like \cref{II.1.9.3} when we change only one of the data $V_0$, $\tau_0$, or $e$.
  The case where we change only the reference lattice $V_0$ follows from \cref{II.1.8.1}.

  We will systematically use the fact that, for $f\in K$, we have, by \cref{II.1.4.3},
  \[
  \label{II.1.9.4}
    v(\partial_{\tau_i}f) \geq v(f)
  \tag{1.9.4}
  \]
  (using the notation of \cref{II.1.2.3}).

  If $e$ and $f$ are two bases, then $e=fa$ for some $a\in\GL_n(K)$, whence
  \[
    \nabla_\tau^i(e) = \sum_j\binom{i}{j}\nabla_\tau^j(f)\cdot\nabla_\tau^{i-j}a,
  \]
  and, by \cref{II.1.9.4},
  \[
  ...
  \]
  whence the inequality \cref{II.1.9.3} for a change of basis.

\oldpage{47}
  If $\tau$ and $\omega$ are vectors as in \cref{II.1.9}, then $\sigma=f\tau$, with $f$ invertible, whence
  \[
    \nabla_\sigma = f\nabla_\tau
...
By \cref{II.1.9.1} and \cref{II.1.9.3} it suffices, to prove \cref{II.1.9}, to prove an upper bound of the form \cref{II.1.9.2} for \emph{one} choice of $(V_0,\tau,e)$.

\begin{lemma}{1.9.6}
\label{II.1.9.6}
  Under the hypotheses of \cref{II.1.9}, let $e\colon K^n\to V$ be a basis of $V$, $t$ a uniformiser, $\omega$ a differential form presenting a simple pole (i.e. a basis of $t^{-1}\Omega$), set $\tau=\omega^{-1}\in\Omega$, and let $\Gamma=(\Gamma_j^i)$ be the connection matrix in the bases $e$ and $\omega$.
  ...
  Then the upper bound \cref{II.1.9.2} is satisfied for $r=\sup\{s,0\}$.
\end{lemma}
...
\oldpage{48}
  ...
  By the definition of $\Omega'$ \cref{II.1.6}, $\omega'$ presents a simple pole.
  ...
  This satisfies \cref{II.1.9.2} on $\scr{O}'$ (for suitable bases), and \cref{II.1.9.6} then follows from \cref{II.1.9.3}.
\end{proof}

Theorem~\cref{II.1.9} follows from the following proposition and from \cref{I.1.3}.

\begin{proposition}{1.10}
\label{II.1.10}
  Under the hypotheses of \cref{II.1.9}, let $X$ be a basis of $\Omega^\vee$, $t$ a uniformiser, set $\tau=tX$, and let $v$ be a cyclic vector \cref{II.1.2} of $V$.
  Set
\oldpage{49}
  \[
    \begin{aligned}
    ...
    \end{aligned}
  \]
  Then the bound \cref{II.1.9.2} holds true for
  \[
    \begin{aligned}
      r
...
\begin{proof}
  ...
  It remains only to apply \cref{II.1.9.6}.
...
\label{II.1.11}
  Under the hypotheses of \cref{II.1.9}, we say that the connection $\nabla$ is \emph{regular} if condition~a) of \cref{II.1.9} is satisfied.
\end{definition}

\begin{theorem}{1.12}
\label{II.1.12}
  (N. Katz).
  Under the hypotheses of \cref{II.1.9}, we have the following:
  \begin{enumerate}[(i)]
...
\begin{proof}
  Claim~(iii), already utilised in \cref{II.1.12}, follows for example from the calculation in \cref{II.1.10} and from the fact that the inverse image of a differential form presenting a simple pole again presents a simple pole.

\oldpage{52}
  Claim~(ii) follows immediately from criterion~(i) in \cref{II.1.12}.
  ...
  After an eventual extension of scalars, we choose bases $e'$ and $e''$ of $V'$ and $V''$ (respectively) satisfying (i) or (ii) in \cref{II.1.12}.
  Lift $e''$ to a family of vectors $e''_0$ of $V$.
  For large enough $N$, the basis $e'\cup t^{-N}e''_0$ of $V$ will satisfy (i) in \cref{II.1.12} if $e'$ and $e''$ satisfy (i) in \cref{II.1.12}, and will satisfy (ii) in \cref{II.1.12} in the contrary case.
\end{proof}
...
\label{II.1.14}
  ...
  Such a connection defines a connection \cref{II.1.2} on $V_K$ (cf. \cref{II.1.7.2}).
  We say that a connection $\nabla$ on $V$ is \emph{regular} at $p$ if it is meromorphic at $0$ and if the induced connection on $V_K$ is regular in the sense of \cref{II.1.11}, i.e. if there exists a basis of $V$ close to $p$ in which the matrix of the connection presents at most a simple pole at $p$ \cref{II.1.12}.
\end{env}
...
\oldpage{53}
  ...
  Using the dictionary \cref{I.2}, every vector bundle $V$ with connection is thus endowed with an action of the local fundamental group $\ZZ$.
...
\begin{theorem}{1.17}
\label{II.1.17}
  Under the hypotheses of \cref{II.1.16}, the monodromy transformation $T$ extends to an automorphism of $V$ whose fibre at $0$ is given by
  \[
