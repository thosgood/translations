...
\theoremstyle{definition}
  ...
  \newtheorem{innerreminder}{Reminder}
  \newenvironment{reminder}[1]
    {\renewcommand\theinnerreminder{#1}\innerreminder}
    {\endinnerreminder}
...
\label{I.1.3}
  Under the hypotheses of \hyperref[I.1.2]{(I.1.2)}, with $X$ connected, the functor $\scr{F}\mapsto\scr{F}_{x_0}$ is an equivalence between the category of locally constant sheaves on $X$ and the category of sets endowed with an action by the group $\pi_1(X,x_0)$.
\end{proposition}

\oldpage{4}
\begin{corollary}{1.4}
\label{I.1.4}
  Under the hypotheses of \hyperref[I.1.2]{(I.1.2)}, with $X$ connected, the functor $\scr{F}\mapsto\scr{F}_{x_0}$ is an equivalence between the category of complex local systems on $X$ and the category of complex finite-dimensional representations of $\pi_1(X,x_0)$.
\end{corollary}

\begin{env}{1.5}
\label{I.1.5}
  Under the hypotheses of \hyperref[I.1.2]{(I.1.2)}, if $a\colon[0,1]\to X$ is a path, and $b$ a loop based at $a(0)$, then $aba^{-1}=a(b)$ is a path based at $a(1)$.
...
\label{I.1.6}
  Under the hypotheses of \hyperref[I.1.5]{(I.1.5)}, there exists, up to unique isomorphism, exactly one locally constant sheaf of groups $\Pi_1(X)$ on $X$ (\emph{the fundamental groupoid}), endowed, for all $x_0\in X$, with an isomorphism
  \[
  \label{I.1.6.1}
    \Pi_1(X)_{x_0} \simeq \pi_1(X,x_0)
  \tag{1.6.1}
  \]
  and such that, for every path $a\colon[0,1]\to X$, the isomorphism in \hyperref[I.1.5]{(I.1.5)} between $\pi_1(X,a(0))$ and $\pi_1(X,a(1))$ can be identified, via \hyperref[I.1.6.1]{(I.1.6.1)}, with the isomorphism in \hyperref[I.1.2]{(I.1.2)} between $\Pi_1(X)_{a(0)}$ and $\Pi_1(X)_{a(1)}$.
\end{proposition}

If $X$ is connected, with base point $x_0$, then the sheaf $\Pi_1(X)$ corresponds, via the equivalence in \hyperref[I.1.3]{(I.1.3)}, to the group $\pi_1(X,x_0)$ endowed with its action over itself by inner automorphisms.

\begin{proposition}{1.7}
\label{I.1.7}
  If $\scr{F}$ is a locally constant sheaf on $X$, then there exists exactly one action (said to be \emph{canonical}) of $\Pi_1(X)$ on $\scr{F}$ that, at each $x_0\in X$, induces the action from \hyperref[I.1.2]{(I.1.2)} of $\pi_1(X,x_0)$ on $\scr{F}$.
\end{proposition}
...
\begin{env}{2.1}
  ...
  We define a (holomorphic) \emph{vector bundle} on $X$ to be a locally free sheaf of modules that is of finite type over the structure sheaf $\scr{O}$ of $X$.
  If $\scr{V}$ is a vector bundle on $X$, and $x$ a point of $X$, then we denote by $\scr{V}_{(x)}$ the free $\scr{O}_{(x)}$-module of finite type of germs of sections of $\scr{V}$.
  If $\mathfrak{m}_x$ is the maximal ideal of $\scr{O}_{(x)}$, then we define the \emph{fibre at $x$ of the vector bundle $\scr{V}$} to be the \todo of finite rank
  \[
  \label{I.2.1.1}
    \scr{V}_x = \scr{V}_{(x)} \otimes_{\scr{O}_{(x)}} \scr{O}_{(x)}/\mathfrak{m}_x.
  \tag{2.1.1}
  \]
  ...
  \[
  \label{I.2.1.2}
    f^*\scr{V} \simeq \scr{O}_X \otimes_{f^*\scr{O}_Y} f^\sbullet\scr{V}
  \tag{2.1.2}
  \]
...
\label{I.2.2}
  ...
  Suitably interpreted, this ``definition'' coincides with the currently fashionable definition \hyperref[I.2.2.4]{(I.2.2.4)} given below (which we not be use in the rest of the section).
...
\oldpage{6}
  ...
  \begin{env}{2.2.2}
  \label{I.2.2.2}
    If $Y$ is a subspace of $X$, then the \emph{$n$\textsuperscript{th} infinitesimal neighbourhood} of $Y$ in $X$ is the subspace of $X$ defined locally by the $(n+1)$th power of the ideal of $\scr{O}_X$ that defines $Y$.
  \end{env}
...
\label{I.2.3}
  ...
  \[
    j^1\colon \scr{V} \to P^1(\scr{V}) \simeq \scr{O}_{X_1}\otimes_{\scr{O}_X}\scr{V}.
  \]

  A connection (\hyperref[I.2.2.4]{(I.2.2.4)}) can be understood as a homomorphism (which is automatically an isomorphism)
  \[
  \label{I.2.3.1}
  ...
  \]
\oldpage{7}
  a connection can also be understood as a ($\scr{O}$-linear) homomorphism
  \[
  \label{I.2.3.2}
  ...
  In other words, a connection \hyperref[I.2.2.4]{(I.2.2.4)}, allowing us to compare two neighbouring fibres of $\scr{V}$, also allows us to define the differential $\nabla s$ of a section of $\scr{V}$.

  Conversely, equation~\hyperref[I.2.3.4]{(I.2.3.4)} allows us to define $\DD$, and thus $\gamma$, from the covariant derivative $\nabla$.
  ...
  Definition~\hyperref[I.2.2.4]{(I.2.2.4)} is thus equivalent to the following definition, due to J.L.~Koszul.
\end{env}
...
\label{I.2.4}
  ...
  \[
    \nabla\colon \scr{V} \to \Omega_X^1(\scr{V}) = \Omega_X^1\otimes_{\scr{O}}\scr{V}
  \]
  that satisfies the Leibniz identity (\hyperref[I.2.3.5]{(I.2.3.5)}) for local sections $f$ of $\scr{O}$ and $s$ of $\scr{V}$.
...
\label{I.2.7}
  ...
  This is evident with \hyperref[I.2.2.4]{(I.2.2.4)}.
  ...
  The canonical connection on $\scr{O}$ is the connection for which $\nabla f=\dd f$.
  
  Let $\scr{V}$ be a vector bundle endowed with a connection.
  \begin{env}{2.7.4}
  \label{I.2.7.4}
    We define a connection on the dual $\scr{V}^\vee$ of $\scr{V}$ via \hyperref[I.2.7.3]{(I.2.7.3)} and the defining isomorphism $\scr{V}^\vee = \shHom(\scr{V},\scr{O})$.
    We have
    \[
  ...
  For \hyperref[I.2.7.2]{(I.2.7.2)}, for example, one must verify that, firstly, the given formula defines a $\mathbb{C}$-bilinear map from $(\scr{V}_1\otimes\scr{V}_2)$, which means that the right-hand side $\II(v_1,v_2)$ is $\mathbb{C}$-bilinear and such that $\II(fv_1,v_2)=\II(v_1,fv_2)$;
  secondly, one must also verify identity~\hyperref[I.2.3.5]{(I.2.3.5)}.
\end{env}

\begin{env}{2.8}
\label{I.2.8}
  An $\scr{O}$-homomorphism $f$ between vector bundles $\scr{V}_1$ and $\scr{V}_2$ endowed with connections
\oldpage{9}
  is said to be \emph{compatible with the connections} if
  \[
    {}_2\!\nabla\cdot f = f\cdot{}_1\!\nabla.
  \]
  By \hyperref[I.2.7.3]{(I.2.7.3)}, this reduces to saying that $\nabla f=0$, if $f$ is thought of as a section of $\shHom(\scr{V}_1,\scr{V}_2)$.
  For example, by \hyperref[I.2.7.3]{(I.2.7.3)}, the canonical map
  \[
...
\label{I.2.9}
  ...
  If $f$ is a homomorphism between bundles $\scr{V}_1$ and $\scr{V}_2$ endowed with connections, then it is equivalent to say either that $f$ is horizontal, or that $f$ is compatible with the connections \hyperref[I.2.8]{(I.2.8)}.
\end{env}

\begin{env}{2.10}
\label{I.2.10}
  Let $\scr{V}$ be a holomorphic vector bundle on $X$.
  Define $\Omega_X^p=\wedge^p\Omega_X^1$ and $\Omega_X^p(\scr{V})=\Omega_X^p\otimes_\scr{O}\scr{V}$ (the sheaf of \emph{exterior differential $p$-forms with values in $\scr{V}$}).
  ...
  To prove that the right-hand side $\II(\alpha,v)$ of \hyperref[I.2.10.2]{(I.2.10.2)} defines a homomorphism \hyperref[I.2.10.1]{(I.2.10.1)}, it suffices to show that $\II(\alpha,v)$ is $\CC$-bilinear and satisfies
  \[
  ...
  Let $\scr{V}_1$ and $\scr{V}_2$ be vector bundles endowed with connections, and let $\scr{V}$ be their tensor product \hyperref[I.2.7.2]{(I.2.7.2)}.
...
\oldpage{10}
  ...
  Let $\scr{V}$ be a vector bundle endowed with a connection.
  If we apply the above formula to $\scr{O}$ and $\scr{V}$, then, for any local section $\alpha$ (resp. $\nu$) of $\Omega^p$ (resp. $\Omega^q(\scr{V})$), we have that
  \[
  \label{I.2.10.4}
    \nabla(\alpha\wedge\nu) = \dd\alpha\wedge\nu + (-1)^p\alpha\wedge\nabla\nu.
  \tag{2.10.4}
...
\label{I.2.11}
  Under the hypotheses of \hyperref[I.2.10]{(I.2.10)}, the \emph{curvature} $R$ of the given connection on $\scr{V}$ is the composite homomorphism
  \[
...
\begin{env}{2.12}
\label{I.2.12}
  Taking $q=0$ in \hyperref[I.2.10.4]{(I.2.10.4)} gives
  \[
  \label{I.2.12.1}
...
\oldpage{11}
  ...
  We endow $\shEnd(\scr{V})$ with the connection given in \hyperref[I.2.7.3]{(I.2.7.3)}.
  The equation $\nabla(\nabla\nabla)=(\nabla\nabla)\nabla$ can be written as $\nabla(R\wedge\nu) = R\wedge\nabla\nu$.
  By \hyperref[I.2.7.3]{(I.2.7.3)}, we have that $\nabla R\wedge\nu = \nabla(R\wedge\nu) - R\wedge\nabla\nu$, so that
  \[
  \label{I.2.12.3}
    \nabla R=0
...
\label{I.2.13}
  ...
  \[
    ...
    \end{aligned}
  \]
  From this formula, and from \hyperref[I.2.10.2]{(I.2.10.2)}, we see that, for any local section $\nu$ of $\Omega_X^p(\scr{V})$, and holomorphic vector fields $X_0,\ldots,X_p$,
  \[
  \label{I.2.13.1}
    \begin{aligned}
...
\label{I.2.14}
  A connection is said to be \emph{integrable} if its curvature is zero, i.e. \hyperref[I.2.13.2]{(I.2.13.2)} if the following holds identically:
  \[
    \nabla_{[X,Y]} = [\nabla_X,\nabla_Y].
  \]
\end{definition}
...
\oldpage{12}
The results \hyperref[I.2.16]{(I.2.16)} to \hyperref[I.2.19]{(I.2.19)} that follow will be proven in a more general setting in \hyperref[I.2.23]{(I.2.23)}.

\begin{proposition}{2.16}
\label{I.2.16}
  Let $V$ be a local complex system on a complex-analytic variety $X$ \hyperref[0.6]{(0.6)}, and let $\scr{V}=\scr{O}\otimes_\CC V$.
  \begin{enumerate}[(i)]
    ...
    \item For any local section $f$ (resp. $v$) of $\scr{O}$ (resp. $V$),
      \[
      \label{I.2.16.1}
        \nabla(fv) = \dd f\cdot v.
      \tag{2.16.1}
...
\begin{proof}
  If $\nabla$ satisfies (i), then \hyperref[I.2.16.1]{(I.2.16.1)} is a particular case of \hyperref[I.2.3.5]{(I.2.3.5)}.
  Conversely, the right-hand side $\II(f,v)$ of \hyperref[I.2.16.1]{(I.2.16.1)} is $\CC$-bilinear, and thus extends uniquely to a $\CC$-linear map $\nabla\colon\scr{V}\to\Omega^1(\scr{V})$, which we can show defines a connection.
...
\label{I.2.17}
  ...
  \begin{enumerate}[a)]
    \item the complex local system $V$ is sent to $\scr{V}=\scr{O}\otimes V$ endowed with its canonical connection;
...
to the unit complex local system $\underline{\CC}$ corresponds the bundle $\scr{O}$ endowed with the connection $\nabla$ such that $\nabla f=\dd f$.

Definition~\hyperref[I.2.10.2]{(I.2.10.2)} implies the following:
\begin{proposition}{2.18}
\label{I.2.18}
  If $V$ is a complex local system on $X$, and if $\scr{V}=\scr{O}\otimes_\CC V$,
\oldpage{13}
  then the system of isomorphisms
  \[
    \Omega_X^p\otimes_\CC V
    \simeq
    \Omega_X^p\otimes_\scr{O}\scr{O}\otimes_\CC V
    \simeq
    \Omega_X^p\otimes_\scr{O}\scr{V}
  \]
...
\label{I.2.19}
  Under the hypotheses of \hyperref[I.2.16]{(I.2.16)}, the complex $\Omega_X^\bullet(\scr{V})$ is a resolution of the sheaf $\scr{V}$.
\end{proposition}
...
\label{I.2.20}
  ...
  \begin{env}{2.20.2}
  \label{I.2.20.2}
    Theorem~\hyperref[I.2.17]{(I.2.17)} makes essential use of the non-singularity of $X$;
    it is thus unimportant to note that this hypothesis has not been used in an essential way before \hyperref[I.2.17]{(I.2.17)}
  \end{env}

  \begin{env}{2.20.3}
  \label{I.2.20.3}
    The definition \hyperref[I.2.4]{(I.2.4)} of a connection and the definition \hyperref[I.2.11]{(I.2.11)} of an integrable connection are formal enough that we can transport them to the category of schemes, or in relative settings:
  \end{env}
\end{env}
...
\label{I.2.21}
  \begin{enumerate}[(i)]
    \item Let $f\colon X\to S$ be a smooth morphism of schemes, and $\scr{V}$ a quasi-coherent sheaf on $X$.
      A \emph{relative connection} on $\scr{V}$ is an $f^*\scr{O}_S$-linear sheaf morphism
      \[
        \nabla\colon \scr{V} \to \Omega_{X/S}^1(\scr{V})
      \]
      (called the \emph{covariant derivative} defined by the connection) that identically satisfies, for any local section $f$ (resp. $v$) of $\scr{O}_X$ (resp. $\scr{V}$),
      \[
        \nabla(fv) = f\cdot\nabla v + \dd f\cdot v.
      \]
    \item Given $\scr{V}$ endowed with a relative connection, there exists exactly one system of $f^*\scr{O}_S$-homomorphisms of sheaves
      \[
        \nabla^{(p)}\colon \Omega_{X/S}^p(\scr{V}) \to \Omega_{X/S}^{p+1}(\scr{V})
      \]
      that satisfies \hyperref[I.2.10.4]{(I.2.10.4)} and is such that $\nabla^{(0)}=\nabla$.
\oldpage{14}
    \item The \emph{curvature} of a connection is defined by
      ...
      \]
      The curvature satisfies the Ricci identity \hyperref[I.2.12.2]{(I.2.12.2)} and the Bianchi identity \hyperref[I.2.12.3]{(I.2.12.3)}.
...
\label{I.2.22}
  ...
  A \emph{local relative system} on $X$ is a sheaf of $f^*\scr{O}_S$-modules that is locally isomorphic to the sheaf-theoretic inverse image of a coherent analytic sheaf on $S$.
  If $\scr{V}$ is a coherent analytic sheaf on $X$, then a \emph{relative connection} on $\scr{V}$ is an $f^*\scr{O}_S$-linear homomorphism
  \[
    \nabla\colon \scr{V} \to \Omega_{X/S}^1(\scr{V})
  \]
  that identically satisfies, for any local section $f$ (resp. $v$) of $\scr{O}$ (resp. $\scr{V}$),
  \[
    \nabla(fv) = f\cdot\nabla v + \dd f\cdot v.
  \]
  ...
  We define, as in \hyperref[I.2.11,I.2.21]{(I.2.11,I.2.21)}, the \emph{curvature} $R\in\Omega_{X/S}^2(\shEnd(\scr{V}))$ of a relative connection.
  A relative connection is said to be \emph{integrable} if $R=0$, in which case we have the \emph{relative de Rham complex with values in $\scr{V}$}, denoted by $\Omega_{X/S}^\bullet(\scr{V})$, and defined as in \hyperref[I.2.15,I.2.21]{(I.2.15,I.2.21)}.
\end{env}

The ``absolute'' statements \hyperref[I.2.17,I.2.18,I.2.19]{(I.2.17,I.2.18,I.2.19)} have ``relative'' (i.e. ``with parameters'') analogues:

\begin{theorem}{2.23}
\label{I.2.23}
  Under the hypotheses of \hyperref[I.2.22]{(I.2.22)}, we have the following.
  \begin{enumerate}[(i)]
    \item For every relative local system $V$ on $X$, there exists a coherent analytic sheaf \mbox{$\scr{V}=\scr{O}_X\otimes_{f^*\scr{O}_S}V$}, and exactly one relative connection, said to be canonical, such that a local section $v$ of $\scr{V}$ is horizontal (i.e. such that $\nabla v=0$) if and only if $v$ is a section of $V$;
      this connection is integrable.
    \item Given a relative local system $V$ on $X$, the de Rham complex defined
\oldpage{15}
      by $\scr{V}=\scr{O}_X\otimes_{f^*\scr{O}_S}V$, endowed with its canonical connection, is a resolution of the sheaf $V$.
    \item The following functors are quasi-inverse to one another, and thus give an equivalence between the category of relative local systems on $X$ and the category of coherent analytic sheaves on $X$ endowed with a relative integrable connection:
      \begin{enumerate}[a)]
        \item the relative local system $V$ is sent to $\scr{V}=\scr{O}_X\otimes_{f^*\scr{O}_S}V$ endowed with its canonical connection;
...
\begin{proof}
  ...
  The canonical relative connection necessarily satisfies, for any local section $f$ (resp $v_0$) of $\scr{O}_X$ (resp. $V$),
  \[
  \label{I.2.23.1}
    \nabla(fv_0) = \dd f\cdot v_0.
  \tag{2.23.1}
  \]
  The right-hand side $\II(f,v_0)$ of this equation is biadditive in $f$ and $v_0$, and satisfies, for any local section $g$ of $f^*\scr{O}_S$, the identity
  \[
    \II(fg,v_0) = \II(f,gv_0)
  \]
  (using the fact that $\dd g=0$ in $\Omega_{X/S}^1$).
  We thus deduce the existence and uniqueness of a relative connection $\nabla$ that satisfies \hyperref[I.2.23.1]{(I.2.23.1)}.
...
\begin{env}{2.23.2}
\label{I.2.23.2}
  We first of all consider the particular case of (ii) where $S=D^n$, $X=D^n\times D^m$, $f=\pr_2$, and the relative local system $V$ is the inverse image of $\scr{O}_S$.
  The complex of global sections
  \[
    0 \to \Gamma(f^\sbullet\scr{O}_S) \to \Gamma(\scr{O}_X) \to \Gamma(\Omega_{X/S}^1) \to \ldots
  \]
  is acyclic, since it admits the homotopy operator $H$ defined below.
\oldpage{16}
  \begin{enumerate}[a)]
    \item $H\colon \Gamma(\scr{O}_X) \to \Gamma(f^\sbullet\scr{O}_S) = \Gamma(S,\scr{O}_S)$ is the inverse image under the zero section of $f$;
  ...
  This remains true if we replace $D^{m+n}$ by a smaller polycylinder, and so $\Omega_{X/S}^\bullet$ is a resolution of $f^\sbullet\scr{O}_S$.
\end{env}
...
\label{I.2.23.3}
  ...
  Applying the syzygy theorem, and possibly shrinking $X$ and $S$, we can further suppose that the direct image of $V_0$ on $D^n$, which we again denote by $V_0$, admits a finite resolution $\scr{L}$ by free coherent $\scr{O}_{D^n}$-modules.
  To prove (ii), we are allowed to replace $V_0$ by its direct image on $D^n$, and to suppose that $D^n=S$, which we now do.

  If $\Sigma_0$ is a short exact sequence of coherent $\scr{O}_S$-modules
  \[
  ...
  \[
    \Omega_{X/S}^\bullet(\Sigma)\colon 0 \to \Omega_{X/S}^p\otimes_{f^\sbullet\scr{O}_S}f^\sbullet V'_0 \to \Omega_{X/S}^p\otimes_{f^\sbullet\scr{O}_S}f^\sbullet V_0 \to \Omega_{X/S}^p\otimes_{f^\sbullet\scr{O}_S}f^\sbullet V''_0 \to 0.
  \]
  This sequence is exact since $\Omega_{X/S}^p$ is flat over $f^\sbullet\scr{O}_S$, since it is locally free over $\scr{O}_X$ which is itself flat over $f^\sbullet\scr{O}_S$.
...
\oldpage{17}
  ...
  \begin{enumerate}[\bf {Case}~1:]
    ...
    \item \emph{$S=D^n$, $X=D^{n+1}=D^n\times D$, and $f=\pr_1$.}
      ...
      There thus exist (by Case~1) relative local systems $V_0$ and $V_1$ such that $(\scr{V}_i,\nabla_i)\simeq\scr{O}_X\otimes_{f^\sbullet\scr{O}_S}V_i$.
      We then have that
      \[
        (\scr{V},\nabla) \simeq \scr{O}_X\otimes_{f^\sbullet\scr{O}_S}(V_0/dV_1).
      \]
\oldpage{18}
    ...
    \item[\bf General case.]
      ...
      \[
        \scr{V} \simeq \scr{O}_X\otimes_{p^\sbullet\scr{O}_{S\times D^{n-1}}}p^\sbullet V_1.
      \]
      ...
      \[
        \alpha\colon \scr{V}\simeq\scr{O}_X\otimes_{f^\sbullet\scr{O}_S}V
      \]
      ...
      If $1\leq i<n$, since $R=0$, then, by an analogous statement to \hyperref[I.2.13.2]{(I.2.13.2)}, we have that
      \[
        \nabla_{x_n}\nabla_{x_i}v = \nabla_{x_i}\nabla_{x_n}v = 0.
      \]
...
\oldpage{19}
      of \hyperref[I.2.23]{(I.2.23)}.
  \end{enumerate}
\end{proof}

Some results in general topology (\hyperref[I.2.24]{(I.2.24)} to \hyperref[I.2.27]{(I.2.27)}) will be necessary to deduce \hyperref[I.2.28]{(I.2.28)} from \hyperref[I.2.23]{(I.2.23)}.
...
\begin{proof}
  ...
  We conclude by \hyperref[I.2.24]{(I.2.24)}.
\end{proof}
...
\begin{proof}
  ...
  Equation~\hyperref[I.2.26.1]{(I.2.26.1)} then follows from \hyperref[I.2.26.2]{(I.2.26.2)}.
\end{proof}
...
\oldpage{20}
  \[
  \label{I.2.27.1}
    V_\mathrm{rel} = f^\sbullet\scr{O}_S \otimes_\CC V
  \tag{2.27.1}
  \]
  is a relative local system.
  We denote by $\Omega_{X/S}^\bullet(V)$ the corresponding de Rham complex.
  By \hyperref[I.2.23]{(I.2.23)}, $\Omega_{X/S}^\bullet$ is a resolution of $V_\mathrm{rel}$.
  We thus have that
  \[
  \label{I.2.27.2}
  ...
  From \hyperref[I.2.27.1]{(I.2.27.1)}, we thus obtain an arrow
  \[
  \label{I.2.27.3}
    \scr{O}_S \otimes_\CC \RR^i f_*V \to \RR^i f_*(V_\mathrm{rel}),
  \tag{2.27.3}
  \]
  whence, by composition, an arrow
  \[
  \label{I.2.27.4}
    \scr{O}_S \otimes \RR^i f_*V \to \RR^i f_*(\Omega_{X/S}(V)).
  \tag{2.27.4}
  \]
\end{env}
...
\label{I.2.28}
  ...
  \begin{enumerate}[a)]
    ...
    \item the fibres of $f$ satisfy
      ...
      \]
      Then the arrow \hyperref[I.2.27.4]{(I.2.27.4)} is an isomorphism:
      \[
        \scr{O}_S \otimes_\CC \RR^i f_*V \simto \RR^i f_*(\Omega_{X/S}^\bullet(V)).
      \]
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  To show that \hyperref[I.2.27.4]{(I.2.27.4)} is an isomorphism, it suffices to construct a fundamental system $T$ of neighbourhoods of $s$ such that the arrows
  \[
  \label{I.2.28.1}
    \HH^0(T,\scr{O}_S) \otimes \HH^i(T\times Y,\pr_2^\sbullet V_0) \simto \HH^i(T\times Y,\pr_1^\sbullet\scr{O}_S \otimes \pr_2^\sbullet V_0)
  \tag{2.28.1}
  \]
  are isomorphisms.
  In fact, the fibre at $s$ of \hyperref[I.2.27.3]{(I.2.27.3)}, which is the inductive limit of the arrows \hyperref[I.2.28.1]{(I.2.28.1)}, will then be an isomorphism.

  We will prove \hyperref[I.2.28.1]{(I.2.28.1)} for a compact Stein neighbourhood $T$ of $s$, assumed to be contractible.
  The arrow in \hyperref[I.2.28.1]{(I.2.28.1)} can then be written as
\oldpage{21}
  \[
  \label{I.2.28.2}
    \HH^0(T,\scr{O}_S) \otimes \HH^i(Y,V_0) \simto \HH^i(T\times Y,\pr_1^\sbullet\scr{O}_S \otimes \pr_2^\sbullet V_0).
  \tag{2.28.2}
  \]

  We can calculate the right-hand side of \hyperref[I.2.28.2]{(I.2.28.2)} by using the Leray spectral sequence for $\pr_2\colon T\times Y\to Y$.
  By \hyperref[I.2.25]{(I.2.25)}, since $\HH^i(T,\scr{O}_S)=0$, we have that
  \[
    \HH^i(T\times Y,\pr_1^\sbullet\scr{O}_S \otimes \pr_2^\sbullet V_0) \simeq \HH^i(Y,\HH^0(T,\scr{O}_S) \otimes V_0),
  \]
  and we conclude by \hyperref[I.2.26]{(I.2.26)}.
\end{proof}

\begin{env}{2.29}
\label{I.2.29}
  Under the hypotheses of \hyperref[I.2.28]{(I.2.28)}, with $S$ smooth, we define the \emph{Gauss--Manin connection} on $\RR^i f_*\Omega_{X/S}^\bullet(V)$ as being the unique integrable connection that admits the local sections of $\RR^i f_*V$ as its horizontal sections \hyperref[I.2.17]{(I.2.17)}.
\end{env}
...
\label{I.3.1}
  ...
  If $\nabla$ is the covariant derivative defined by another connection on $\scr{V}$, then we have seen \hyperref[I.2.6]{(I.2.6)} that $\nabla$ can be written in the form
  \[
    \nabla = {}_0\!\nabla + \Gamma,
  ...
  If we suppose that we have chosen a basis of $V$, i.e. an isomorphism $e\colon\CC^n\to V_0$ with coordinates (identified with basis vectors) $e_\alpha\colon\CC\to V_0$, then $\Gamma$ can be written as a matrix $\omega_\beta^\alpha$ of differential forms (the \emph{matrix of forms of the connection}), and \hyperref[I.3.1.1]{(I.3.1.1)} can then be written as
  \[
  \label{I.3.1.2}
...
\oldpage{22}
  ...
  connections on $\scr{V}$ correspond, via \hyperref[I.3.1.2]{(I.3.1.2)}, with $(n\times n)$-matrices of differential forms on $X$.
  If $\omega_e$ is the matrix of the connection $\nabla$ in the basis $e$, and if $f\colon\CC^n\to\simto\scr{V}$ is another basis of $\scr{V}$, with matrix $A\in\GL_n(\scr{O})$ (where $A=e^{-1}f$), then \hyperref[I.3.1.2]{(I.3.1.2)}
  \[
    \begin{aligned}
      \nabla v
    ...
    \end{aligned}
  \]
  Comparing this with \hyperref[I.3.1.2]{(I.3.1.2)} in the basis $f$, we find that
  \[
  \label{I.3.1.3}
  ...
  Equation~\hyperref[I.3.1.2]{(I.3.1.2)} can the be written as
  \[
  \label{I.3.1.4}
...
\begin{env}{3.2}
\label{I.3.2}
  With the notation of \hyperref[I.3.1.2]{(I.3.1.2)}, and using Einstein summation notation, we have that
  \[
    \begin{aligned}
      \nabla\nabla v
...
\oldpage{23}
  ...
  \tag{3.2.2}
  \]
  Equation~\hyperref[I.3.2.1]{(I.3.2.1)} gives, in a system $(x^i)$ of local coordinates,
  \[
  \label{I.3.2.3}
    \begin{cases}
      R_{\beta i j}^\alpha
  ...
  The condition $R_{\beta i j}^\alpha = 0$ is the integrability condition of the system \hyperref[I.3.1.5]{(I.3.1.5)}, in the classical sense of the word;
  it can be obtained by eliminating $v^\alpha$ from the equations given by substituting \hyperref[I.3.1.5]{(I.3.1.5)} into the identity $\partial_i\partial_j v^\alpha = \partial_j\partial_i v^\alpha$.
\end{env}
...
\label{I.4.1}
  ...
  By \hyperref[I.3]{(I.3)}, this system can be described as the differential equation of horizontal sections of a rank-$n$ vector bundle endowed with a suitable connection, and this is what we aim to further explore.
\end{env}
...
\oldpage{24}
  ...
  \label{I.4.2.3}
    \begin{aligned}
      \Gr P^n(\scr{L}) &\simeq \Gr P^n(\scr{O})\otimes\scr{L}
  ...
  \[
    \begin{cases}
      \mbox{$A$ is $\scr{O}$-linear} & \mbox{for $n=0$ ;}
    \\\mbox{$[A,f]$ is of order $\leq m$ for every local section $f$ of $\scr{O}$} & \mbox{for $n=m+1$.}
    \end{cases}
  \]

  For every local section $s$ of $\scr{L}$, $p_2^*s$ defines a local section $\DD^n(s)$ of $P^n(\scr{L})$ \hyperref[I.4.2.2]{(I.4.2.2)}.
...
\label{I.4.3}
  \begin{enumerate}[(i)]
    \item A \emph{linear homogeneous $n^\mathrm{th}$-order differential equation} on $\scr{L}$ is an $\scr{O}_X$-homomorphism $E\colon P^n(\scr{L})\to\Omega^{\otimes n}\otimes\scr{L}$ that induces the identity on the submodule $\Omega^{\otimes n}\otimes\scr{L}$ of $P^n(\scr{L})$.
    \item A local section $s$ of $\scr{L}$ is a \emph{solution} of the differential equation $E$ if $E(\DD^n(s))=0$.
  \end{enumerate}
\end{definition}

In fact, I have cheated with this definition, in that I have only considered equations that can be put in the ``resolved'' form \hyperref[I.4.1.1]{(I.4.1.1)}.

\oldpage{25}
\begin{env}{4.4}
\label{I.4.4}
  Suppose that $\scr{L}=\scr{O}$, and let $x$ be a local coordinate on $X$.
  The choice of $x$ allows us to identify $p^k(\scr{O})$ with $\scr{O}^{[0,k]}$, with the arrow $\DD^k$ becoming
  \[
    \begin{aligned}
      \DD^k\colon \scr{O} &\to P^k(\scr{O}) \simeq \scr{O}^{[0,k]}
    \\f &\mapsto (\partial_x^i f)_{0\leq i\leq k}.
    \end{aligned}
  \]
  The choice of $x$ also allows us to identify $\Omega^1$ with $\scr{O}$, so that the $n^\mathrm{th}$-order differential equation can be identified with a morphism $E\in\Hom(\scr{O}^{[0,n]},\scr{O})$, and, as such, has coordinates $(b_i)_{0\leq i\leq n}$ with $b_n=1$.
  ...
  The existence and uniqueness theorem for solutions of the Cauchy problem in \hyperref[I.4.4.1]{(I.4.4.1)} implies the following.
\end{env}

\begin{theorem}{4.5}
\label{I.4.5}
  \emph{(Cauchy).}
  Let $X$ and $\scr{L}$ be as in \hyperref[I.4.2]{(I.4.2)}, and let $E$ be an $n^\mathrm{th}$-order differential equation on $\scr{L}$.
  Then
  \begin{enumerate}[(i)]
    ...
    \item the canonical arrow $\DD^{n-1}\colon\scr{L}^E \to P^{n-1}(\scr{L})$ induces an isomorphism
      \[
        \scr{O}\otimes_\CC\scr{L}^E \simto P^{n-1}(\scr{L}).
      \]
  \end{enumerate}
\end{theorem}

In particular, it follows from \hyperref[I.4.5]{(I.4.5)}~(ii) and from \hyperref[I.2.17]{(I.2.17)} that $E$ defines a canonical connection on $P^{n-1}(\scr{L})$, whose horizontal sections are the images under $\DD$ of solutions of $E$.
...
\label{I.4.6}
  ...
  \begin{enumerate}[a)]
    ...
    \item a surjective homomorphism $\lambda\colon\scr{V}\to\scr{L}$ (by $i=0$ in \hyperref[I.4.2.3]{(I.4.2.3)}).
  \end{enumerate}
  Furthermore, the solutions of $E$ are exactly the images under $\lambda$ of the horizontal sections of $\scr{V}$.
  This is just another way of expressing how to obtain \hyperref[I.4.1.2]{(I.4.1.2)} from \hyperref[I.4.1.1]{(I.4.1.1)}.
\end{env}
...
\label{I.4.7}
  ...
  If $\scr{L}$ is an invertible module, then we say that a section $v$ of $\scr{V}\otimes\scr{L}$ is cyclic if, for every every local isomorphism between $\scr{L}$ and $\scr{O}$, the corresponding section of $\scr{V}$ is cyclic.
  This applies, in particular, to sections $v$ of $\shHom(\scr{V},\scr{L})=\scr{V}^\vee\otimes\scr{L}$.
\end{env}

\begin{lemma}{4.8}
\label{I.4.8}
  With the hypotheses and notation of \hyperref[I.4.6]{(I.4.6)}, $\lambda$ is a cyclic section of $\shHom(\scr{V},\scr{L})$.
\end{lemma}

\begin{proof}
  The problem is local on $X$;
  we can reduce to the case where $\scr{L}=\scr{O}$ and where there exists a local coordinate $x$.

  We use the notation of \hyperref[I.4.4]{(I.4.4)}.
  A section $(f^i)$ of $P^{n-1}(\scr{O})\simeq\scr{O}^{[0,n-1]}$ is horizontal if and only if it satisfies
  \[
    \begin{cases}
      \partial_x f^i = f^{i+1}
...
\oldpage{27}
  ...
  \]
  which proves \hyperref[I.4.8]{(I.4.8)}.
\end{proof}

\begin{proposition}{4.9}
\label{I.4.9}
  The construction in \hyperref[I.4.6]{(I.4.6)} establishes an equivalence between the following categories, where we take morphisms to be isomorphisms:
  \begin{enumerate}[a)]
    \item the category of invertible sheaves on $X$ endowed with an $n^\mathrm{th}$-order differential equation \hyperref[I.4.3]{(I.4.3)} ; and
...
\begin{proof}
  We will construct a functor that is quasi-inverse to that in \hyperref[I.4.6]{(I.4.6)}.
  ...
  For every $\scr{O}$-module $\scr{M}$, we have \hyperref[I.2.17]{(I.2.17)}
  \[
    \Hom_\scr{O}(\scr{V},\scr{M}) \simto \Hom_\CC(V,\scr{M}).
  \]
  ...
  The problem is local on $X$.
  We can restrict to the case where $\scr{L}=\scr{O}$ and where we have a local coordinate $x$.
  With the notation of \hyperref[I.4.4]{(I.4.4)}, the morphism $\gamma^k$ then admits the morphisms $\partial_x^i\lambda = \nabla_x^i$ (for $0\leq i\leq k$) as coordinates.
  For $k=n-1$, these form a basis of $\Hom(\scr{V},\scr{O})$ if and only if $\gamma^{n-1}$ is an isomorphism.
...
\oldpage{28}
  ...
  \]
  commutes;
  if $\lambda$ is cyclic, then this, along with \hyperref[I.4.9.2]{(I.4.9.2)}, implies that $\gamma^n(v)$ is locally a direct factor, of codimension~$1$ in $P^n(\scr{L})$, and admits $\omega^{\otimes n}\otimes\scr{L} \simeq \Ker(\pi_{n-1,n})$ as a complement.
  ...
  By \hyperref[I.4.9.1]{(I.4.9.1)}, if $v$ is a horizontal section of $\scr{V}$, then $E\DD^n\lambda v = E\gamma^n v = 0$, and so $\lambda v$ is a solution of $E$.
  We endow $P^{n-1}(\scr{L})$ with the connection \hyperref[I.4.6]{(I.4.6)} defined by $E$.
  ...
  A particular case of \hyperref[I.4.9.3]{(I.4.9.3)} shows that the diagram
  \[
    \begin{tikzcd}[row sep=huge]
    ...
    \end{tikzcd}
  \]
  commutes, whence we have an isomorphism between $(\scr{V},\scr{L},\lambda)$ and the triple given by \hyperref[I.4.6]{(I.4.6)} applied to $(\scr{L},E)$.
  The functor
  \[
    (\scr{V},\scr{L},\lambda) \mapsto (\scr{L},E)
  \]
  is thus quasi-inverse to the functor in \hyperref[I.4.6]{(I.4.6)}.
\end{proof}

\begin{env}{4.10}
\label{I.4.10}
  We now summarise the relations between two systems $(\scr{V},\scr{L},\lambda)$ and $(\scr{L},E)$ that correspond under \hyperref[I.4.6]{(I.4.6)} and \hyperref[I.4.9]{(I.4.9)}.
...
In this section, we specialise the results of \hyperref[I.4]{(I.4)} to the case where $n=2$, and we express certain results given in R.C.~Gunning~\cite{11} in a more geometric form.

\begin{env}{5.1}
\label{I.5.1}
  Let $S$ be an analytic space, and let $q\colon X_2\to S$ be an analytic space over $S$ that is locally isomorphic to the finite analytic space over $S$ defined by the $\scr{O}_S$-algebra $\scr{O}_S[T]/(T^3)$.
...
\begin{lemma}{5.2}
\label{I.5.2}
  Under the hypotheses of \hyperref[I.5.1]{(I.5.1)}, let $u$ and $v$ be $S$-immersions of $X_2$ into $\PP_S^1$, i.e.
  \[
    \begin{tikzcd}
...
\begin{proof}
  The problem is local on $S$, which allows us to suppose that $X_2$ is defined by the $\scr{O}_S$-algebra $\scr{O}_S[T]/(T^3)$, and that $u(X_2)$ and $v(X_2)$ are contained inside the same affine line, say, $\AA_S^1$.
...
\begin{env}{5.3}
\label{I.5.3}
  By \hyperref[I.5.2]{(I.5.2)}, there exists exactly (up to isomorphism) one pair $(u,P)$ consisting of a projective line $P$ on $S$ (with structure group $\mathrm{PGL}_2(\scr{O}_S)$) and an $S$-immersion $u$ of $X_2$ into $P$.
  ...
  The morphism $q_1\colon X_2\to X$ is of the type considered in \hyperref[I.5.1]{(I.5.1)}.
\end{env}
...
in particular, the construction in \hyperref[I.5.5]{(I.5.5)} gives us a morphism $\gamma$ that fits into a commutative diagram:
\[
\label{I.5.6.1}
  \begin{tikzcd}[column sep=huge]
...
It is not difficult to show that such a morphism $\gamma$ is defined by a unique projective connection (a proof of this will be given in \hyperref[I.5.10]{(I.5.10)}), and so Definition~\hyperref[I.5.6]{(I.5.6)} is equivalent to the following.

\begin{definition}{5.6~bis}
\label{I.5.6bis}
  \emph{(Infinitesimal version).}
  A \emph{projective connection} on $X$ is a morphism $\gamma\colon X_3\hookrightarrow P_\tg$ that makes the diagram in \hyperref[I.5.6.1]{(I.5.6.1)} commute.
\end{definition}
...
\begin{env}{5.7}
\label{I.5.7}
  Set $\Omega^{\otimes n}=(\Omega_X^1)^{\otimes n}$ \hyperref[I.4.2]{(I.4.2)}.
...
\oldpage{32}
  ...
  Also, if $\Delta$ is the diagonal map, then, by \hyperref[I.5.4.2]{(I.5.4.2)}, we have that
  \[
...
\begin{proof}
  ...
  \tag{5.8.1}
  \]
  (modulo the identification of $\Omega^{\otimes3}$ with an ideal of $\scr{O}_{X_3}$).
\end{proof}
...
\oldpage{33}
  ...
  To prove this, we identify, using \hyperref[I.5.5]{(I.5.5)}, the projective double tangent bundle to $X$ or $Y$ with the constant projective bundle.
  ...
  \]
  and \hyperref[I.5.9.2]{(I.5.9.2)} then follows.

  Equation~\hyperref[I.5.9.2]{(I.5.9.2)} shows that $6\theta f$ is the classical \emph{Schwarz derivative} of $f$.
  ...
  To prove \hyperref[I.5.9.3]{(I.5.9.3)}, the simplest method is to note the following.
\oldpage{34}
  \begin{enumerate}[(i)]
    \item The right-hand side of \hyperref[I.5.9.3]{(I.5.9.3)} is invariant under a linear substitution of constant coefficients $L$ acting on $g$ and $h$: the numerator and denominator $\det(L)^2$.
    \item The right-hand side of \hyperref[I.5.9.3]{(I.5.9.3)} is invariant under the substitution
      \[
      ...
      With these variance properties agreeing with those of the left-hand side of \hyperref[I.5.9.3]{(I.5.9.3)}, it suffices to prove \hyperref[I.5.9.3]{(I.5.9.3)} in the particular case where $h=1$.
      The equation then reduces to \hyperref[I.5.9.2]{(I.5.9.2)}.
  \end{enumerate}
...
\label{I.5.10}
  ...
  It thus admits $\infty^3$ solutions, locally, and these solutions are permuted amongst themselves by the projective group (since the group transitively permutes the Cauchy data: \hyperref[I.5.2]{(I.5.2)}).
  The set of solutions is thus a projective connection (the local version, \hyperref[I.5.6]{(I.5.6)}).
  This construction is inverse to that which associates, to any projective connection in the sense of \hyperref[I.5.6]{(I.5.6)}, a projective connection in the sense of \hyperref[I.5.6bis]{(I.5.6bis)}.
\end{env}

\begin{env}{5.11}
\label{I.5.11}
  Let $X$ be a smooth curve, $\scr{L}$ an invertible sheaf on $X$, and $E$ a second-order ordinary differential equation on $\scr{L}$.
  We have seen, in \hyperref[I.4.5]{(I.4.5)}, that $E$ defines a connection on the bundle $P^1(\scr{L})$ of first-order jets of sections of $\scr{L}$, and we obtain from it a connection on
  \[
  ...
  we have \hyperref[I.4.5]{(I.4.5)} that $\scr{O}\otimes V\simto P^1(\scr{L})$, and the linear form $\lambda\colon P^1(\scr{L})\to\scr{L}$ defines a section $\lambda_0$ of the projective bundle associated to the vector bundle $P^1(\scr{L})$.
  ...
  by \hyperref[I.4.8]{(I.4.8)}, the differential of this map is everywhere non-zero, and so $\lambda_{0,\sigma}$ allows us to transport the canonical projective connection of $\PP^1(\CC)$ to $X$.
...
\label{I.5.12}
  ...
  The construction in \hyperref[I.5.10]{(I.5.10)} gives a bijection between
  \begin{enumerate}[a)]
...
\begin{proof}
  The problem is local on $X$;
  we can thus suppose that $X$ is an open subset of $\CC$, and that $\scr{L}=\scr{O}$.
  An equation $E$ can then be written as
  \[
    E\colon y'' + a(x)y' + b(x)y = 0.
  \]
  If we identify $P^1(\scr{L})$ with $\scr{O}^2$, then the matrix of the connection \hyperref[I.5.10]{(I.5.10)} defined by $E$ on $\wedge^2 P^1(\scr{L})\sim\scr{O}$ is then $-a(x)\tr M$, where $M$ is the matrix in \hyperref[I.4.8.1]{(I.4.8.1)}.
  ...
  Identifying $\Omega$ with $\scr{O}$ by means of the given local coordinate, we then have that
  \[
  \label{I.5.12.1}
  ...
  Equation~\hyperref[I.5.9.3]{(I.5.9.3)} gives (using the same notation for determinants as before)
  \[
    \begin{aligned}
      \theta(\varphi)
      &= \frac{
...
\label{I.6.5}
  Let $X$ and $x_0$ be as in \hyperref[I.5.1]{(I.5.1)}, and let $\scr{F}$ be a sheaf of $\CC$-vector spaces on $X$ that satisfies the principle of analytic continuation.
  ...
  \begin{enumerate}[(i)]
    ...
    \item the subsheaf of $\scr{F}$ of $\CC$-vector spaces generated by the determinations of $s$ is a complex local system \hyperref[I.1.1]{(I.1.1)}.
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  We conclude the proof of \hyperref[I.6.5]{(I.6.5)} by noting that, by the above, the largest open subset of $X$ over which the determinations of $s$ generate a local system is closed and contains $x_0$.
\end{proof}

\begin{definition}{6.7}
\label{I.6.7}
  Under the hypotheses of \hyperref[I.6.5]{(I.6.5)}, a multiform section $s$ of $\scr{F}$ is said to be a \emph{finite determination} if it satisfies either of the equivalent conditions of \hyperref[I.6.5]{(I.6.5)}.
\end{definition}

\begin{env}{6.8}
\label{I.6.8}
  Under the hypotheses of \hyperref[I.6.5]{(I.6.5)}, let $s$ be a multiform section of finite determination of $\scr{F}$.
...
\oldpage{39}
  ...
  The triple consisting of $V_{x_0}$, $v_0$, and the representation of $\pi_1(X,x_0)$ on $V_{x_0}$ defined by $V$ \hyperref[I.1.4]{(I.1.4)} is called the \emph{monodromy} of $s$.
...
\label{I.6.9}
  ...
  The multiform section $s$ of $\scr{F}$ is said to be of \emph{monodromy subordinate to $(w_0,v_0)$} if it is the finite determination, and if, with the notation of \hyperref[I.6.8]{(I.6.8)}, there exists a homomorphism of $\pi_1(X,x_0)$-representations of $W_0$ in $V_{x_0}$ that sends $w_0$ to $v_0$.
  Let $W$ be the local system defined by $W_0$, and let $w$ be the unique multiform section of $w$ of base determination $w_0$.
  It is clear that, under the hypotheses of \hyperref[I.6.5]{(I.6.5)}, we have
\end{env}
...
\label{I.6.11}
  ...
  Let $W_0$ be a finite-dimensional complex representation of $\pi_1(X,x_0)$ endowed with a cyclic vector $w_0$, and $W$ the corresponding local system on $X$, with $\scr{W}=\scr{O}\otimes_\CC W$ being
\oldpage{40}
...
\label{II.1.1}
  ...
  We classically say that $0$ is a \emph{regular singular point} of \hyperref[II.1.1.1]{(II.1.1.1)} if the functions $x^{n-i}a_i(x)$ are holomorphic at $0$.
  If this is true, then, after multiplying by $x^n$, we can write \hyperref[II.1.1.1]{(II.1.1.1)} in the form
  \[
  \label{II.1.1.2}
...
\begin{env}{1.4}
\label{II.1.4}
  Let $\scr{O}$ be a discrete valuation ring of \emph{equal characteristic~$0$}, with maximal ideal $\fk{m}$, residue field $k=\scr{O}/\fk{m}$, and field of fractions $K$.
  Suppose that $\scr{O}$ is endowed with a free rank-$1$ $\scr{O}$-module $\Omega$ along with a derivation $\dd\colon\scr{O}\to\Omega$ that satisfies
  \begin{env}{1.4.1}
  \label{II.1.4.1}
    \itshape
    There exists a uniformiser $t$ such that $\dd t$ generates $\Omega$.
  \end{env}
  (For less hyper-generality, see \hyperref[II.1.7]{(II.1.7)}).

  If $t_1$ is another uniformiser, then $t_1=at$ for some $a\in\scr{O}^*$, and, by hypothesis, $\dd a$ is a multiple of $\dd t$, i.e. $\dd a=\lambda\dd t$.
  We thus have that
  \[
    \dd t_1
    = a\dd t + \dd a\cdot t
  ...
  \[
    v\colon K^* \to \ZZ
  \]
  the valuation of $K$ defined by $\scr{O}$;
  ...
  If $f\in K^*$ with $f=at^n$ (for $a\in\scr{O}$), then
  \[
...
\oldpage{44}
  ...
  In particular, $\dd$ is continuous and extends to $\dd\colon\scr{O}^\wedge\to\Omega^\wedge$, and the triple $(\scr{O}^\wedge,\dd,\Omega^\wedge)$ again satisfies \hyperref[II.1.4.1]{(II.1.4.1)}.
\end{env}

\begin{lemma}{1.5}
\label{II.1.5}
  If $\scr{O}$ is complete, then the triple $(\scr{O},\dd,\Omega)$ is isomorphic to the triple $(k[[t]],\partial_t,k[[t]])$.
\end{lemma}
...
\begin{proof}
  ...
  Since $\scr{O}$ is complete, $\dd\colon\fk{m}\to\Omega$ is surjective, and $\Ker(\dd)\simto k$.
  This gives us a field of representatives that is annihilated by $\dd$, and the choice of a uniformiser $t$ gives the desired isomorphism $k[[t]]\simto\scr{O}$.
\end{proof}

\begin{env}{1.6}
\label{II.1.6}
  If an $\scr{O}$-algebra $\scr{O}'$ is a discrete valuation ring with a field of fractions $K'$ that is algebraic over $K$, then the derivation $\dd$ extends uniquely to $\dd\colon K'\to\Omega\otimes_{\scr{O}}K'$.
  Let $e$ be the ramification index of $\scr{O}'$ over $\scr{O}$, and let $t'$ be a uniformiser of $\scr{O}'$.
  We set
  \[
    \Omega' = 1/(t')^{e-1}\Omega\otimes_{\scr{O}}\scr{O}'.
  \]
\end{env}

We can easily show, using \hyperref[II.1.6]{(II.1.6)}, that the triple $(\scr{O}',\dd,\Omega')$ again satisfies \hyperref[II.1.4.1]{(II.1.4.1)}.
...
\begin{env}{1.7}
  ...
  \begin{enumerate}
    \item[(1.7.1)]\label{II.1.7.1}
      $\scr{O}=\scr{O}_{x,X}$ (the local ring for the Zariski topology), $\Omega=(\Omega_{X/\CC}^1)_x$, and $\dd=\mbox{the differential}$ ;
    \item[(1.7.2)]\label{II.1.7.2}
      $\scr{O}=\scr{O}_{x,X^\an}$ (the local ring of germs at $x$ of holomorphic functions), $\Omega=(\Omega_{X^\an/\CC}^1)_{(x)}$, and $\dd=\mbox{the differential}$ ; or
...
\begin{env}{1.8}
\label{II.1.8}
  Under the hypotheses of \hyperref[II.1.4]{(II.1.4)}, let $V$ be a finite-dimensional vector space over $K$, and $V_0$ a lattice in $V$, i.e. a free sub-$\scr{O}$-module of $V$ such that $KV_0=V$.
  For every homomorphism $e\colon\scr{O}^n\to V$, we define the \emph{valuation $v(e)$ of $e$} to be the largest integer $m$ such that $e(\scr{O}^n)\subset\fk{m}^m V_0$.
...
\label{II.1.9}
  \emph{[N. Katz].}
  Under the hypotheses of \hyperref[II.1.4]{(II.1.4)}, and with the notation of \hyperref[II.1.8]{(II.1.8)}, let $\nabla$ be a connection \hyperref[II.1.2]{(II.1.2)} on a vector space $V$ of dimension~$n$ over $K$.
...
Conditions~a) and b) of \hyperref[II.1.9]{(II.1.9)} are more manageable in a different form:

\begin{lemma}{1.9.1}
\label{II.1.9.1}
  Let $V_0$, $\tau$, and $e$ be as in \hyperref[II.1.9]{(II.1.9)}.
  ...
  \tag{1.9.2}
  \]
  The bound~a) is equivalent to the same bound \hyperref[II.1.9.2]{(II.1.9.2)} for $r=0$.
\end{lemma}

\begin{proof}
  Going from \hyperref[II.1.9]{(II.1.9)} to \hyperref[II.1.9.2]{(II.1.9.2)} is clear, as is the converse for $r=0$.
  So suppose that \hyperref[II.1.9.2]{(II.1.9.2)} holds true for $r>0$ and some value $C_0$ for the constant.
  We have
  \[
  \label{II.1.9.a}
...
\oldpage{46}
  ...
  The inequalities \hyperref[II.1.9.a]{(a)} and \hyperref[II.1.9.b]{(b)} then imply the inequality of the form \hyperref[II.1.9]{(II.1.9)}:
  \[
...
\begin{lemma}{1.9.3}
\label{II.1.9.3}
  Let $(V_0,\tau_0,e_0)$ and $(V_1,\tau_1,e_1)$ be two systems as in \hyperref[II.1.9]{(II.1.9)}.
  Then
  \[
...
\begin{proof}
  It suffices to establish an inequality like \hyperref[II.1.9.3]{(II.1.9.3)} when we change only one of the data $V_0$, $\tau_0$, or $e$.
  The case where we change only the reference lattice $V_0$ follows from \hyperref[II.1.8.1]{(II.1.8.1)}.

  We will systematically use the fact that, for $f\in K$, we have, by \hyperref[II.1.4.3]{(II.1.4.3)},
  \[
  \label{II.1.9.4}
    v(\partial_{\tau_i}f) \geq v(f)
  \tag{1.9.4}
  \]
  (using the notation of \hyperref[II.1.2.3]{(II.1.2.3)}).

  If $e$ and $f$ are two bases, then $e=fa$ for some $a\in\GL_n(K)$, whence
  \[
    \nabla_\tau^i(e) = \sum_j\binom{i}{j}\nabla_\tau^j(f)\cdot\nabla_\tau^{i-j}a,
  \]
  and, by \hyperref[II.1.9.4]{(II.1.9.4)},
  \[
  ...
  \]
  whence the inequality \hyperref[II.1.9.3]{(II.1.9.3)} for a change of basis.

\oldpage{47}
  If $\tau$ and $\omega$ are vectors as in \hyperref[II.1.9]{(II.1.9)}, then $\sigma=f\tau$, with $f$ invertible, whence
  \[
    \nabla_\sigma = f\nabla_\tau
    \qquad\mbox{($f\in\scr{O}^*$)}
  \]
  and we can show by induction that
  \[
    \nabla_\sigma^i = \sum_{j\leq i}\varphi_j\nabla_\tau^j
    \qquad\mbox{($\varphi_j\in\scr{O}$)}.
  \]
...
By \hyperref[II.1.9.1]{(II.1.9.1)} and \hyperref[II.1.9.3]{(II.1.9.3)} it suffices, to prove \hyperref[II.1.9]{(II.1.9)}, to prove an upper bound of the form \hyperref[II.1.9.2]{(II.1.9.2)} for \emph{one} choice of $(V_0,\tau,e)$.

\begin{lemma}{1.9.6}
\label{II.1.9.6}
  Under the hypotheses of \hyperref[II.1.9]{(II.1.9)}, let $e\colon K^n\to V$ be a basis of $V$, $t$ a uniformiser, $\omega$ a differential form presenting a simple pole (i.e. a basis of $t^{-1}\Omega$), set $\tau=\omega^{-1}\in\Omega$, and let $\Gamma=(\Gamma_j^i)$ be the connection matrix in the bases $e$ and $\omega$.
  ...
  Then the upper bound \hyperref[II.1.9.2]{(II.1.9.2)} is satisfied for $r=\sup\{s,0\}$.
\end{lemma}

\begin{proof}
  Let $N$ be an integer such that the $r_iN$ are integers, and set $\scr{O}'=\scr{O}(\sqrt[N]{t})$.
...
\oldpage{48}
  ...
  On $\scr{O}'$, let $\omega'$ be the basis of $\Omega\otimes K'$ given by the inverse image of $\omega$, let $\tau'$ be the corresponding basis of ${\Omega'}^\vee\otimes K'$, and let $e'=e\Lambda$ be a new basis of $V'=V\otimes K'$.
  In these bases, the connection matrix is
  \[
    \Gamma' = \Lambda^{-1}\Gamma\Lambda + \Lambda^{-1}\partial_{\tau'}\Lambda.
  \]

  The matrix $\Lambda^{-1}\partial_{\tau'}\Lambda$ has coefficients in $\scr{O}'$, and so either
  \begin{enumerate}[(a)]
    \item $s\leq0$, and $\Gamma'$ has coefficients in $\scr{O}'$ ; or
    \item $s>0$, $-v(\Gamma')=s$, and the ``most polar part'' $\gamma$ of $\Gamma'$ is not nilpotent, so that $-v({\Gamma'}^\ell)=\ell s$.
  \end{enumerate}

  By the definition of $\Omega'$ \hyperref[II.1.6]{(II.1.6)}, $\omega'$ presents a simple pole.
  ...
  This satisfies \hyperref[II.1.9.2]{(II.1.9.2)} on $\scr{O}'$ (for suitable bases), and \hyperref[II.1.9.6]{(II.1.9.6)} then follows from \hyperref[II.1.9.3]{(II.1.9.3)}.
\end{proof}

Theorem~\hyperref[II.1.9]{(II.1.9)} follows from the following proposition and from \hyperref[I.1.3]{(I.1.3)}.

\begin{proposition}{1.10}
\label{II.1.10}
  Under the hypotheses of \hyperref[II.1.9]{(II.1.9)}, let $X$ be a basis of $\Omega^\vee$, $t$ a uniformiser, set $\tau=tX$, and let $v$ be a cyclic vector \hyperref[II.1.2]{(II.1.2)} of $V$.
  Set
\oldpage{49}
  \[
    \begin{aligned}
    ...
    \end{aligned}
  \]
  Then the bound \hyperref[II.1.9.2]{(II.1.9.2)} holds true for
  \[
    \begin{aligned}
      r
...
\begin{proof}
  ...
  It remains only to apply \hyperref[II.1.9.6]{(II.1.9.6)}.
...
\label{II.1.11}
  Under the hypotheses of \hyperref[II.1.9]{(II.1.9)}, we say that the connection $\nabla$ is \emph{regular} if condition~a) of \hyperref[II.1.9]{(II.1.9)} is satisfied.
\end{definition}

\begin{theorem}{1.12}
\label{II.1.12}
  (N. Katz).
  Under the hypotheses of \hyperref[II.1.9]{(II.1.9)}, we have the following:
  \begin{enumerate}[(i)]
...
\oldpage{51}
    ...
    \item For the connection $\nabla$ to be irregular, and to satisfy a bound of the form \hyperref[II.1.9.2]{(II.1.9.2)} for $r=a/b>0$, it is necessary and sufficient for $V$ to admit a basis $e$ (after a change of rings from $\scr{O}$ to $\scr{O}'=\scr{O}(\sqrt[b]{t})$, and for the natural valuation, with values in $\ZZ$, of $\scr{O}'$) such that the matrix of the connection, in this basis, presents a pole of order $a+1$, and for the polar part of order $a+1$ of this matrix (which is in $\MM_n(k)$ and determined up to a scalar multiple) to be non-nilpotent.
  \end{enumerate}
\end{theorem}

\begin{proof}
  By extension of scalars, the number $r$ such that $\nabla$ satisfies \hyperref[II.1.9.2]{(II.1.9.2)} is multiplied by the ramification index.
  This leads us to the case where $b=1$.
  Conditions~(i) and (ii) are then sufficient, by \hyperref[II.1.9.6]{(II.1.9.6)}.
  Conversely, let $v$ be a cyclic vector \hyperref[II.1.3]{(II.1.3)}, $t$ a uniformiser, and $\tau\in\Omega^\vee$ of valuation~$1$.
  Then it follows from the proofs of \hyperref[II.1.9.6]{(II.1.9.6)} and of \hyperref[II.1.10]{(II.1.10)} that the basis $e_i=t^{ri}\nabla_\tau^i v$ ($0\leq i\leq\dim V$) satisfies (i) or (ii).
\end{proof}
...
\label{II.1.13}
  \begin{enumerate}[(i)]
    ...
    \item If the connections on $V_1$ and $V_2$ are regular, then the natural connections of
      ...
      \]
      are regular.
    \item If $\scr{O}'$ is a discrete valuation ring with field of fractions $k'$ that is algebraic over the field of fractions $K$ of $\scr{O}$, and if $V'=V\otimes_K K'$, then the connection on $V'$ is regular if and only if the connection on $V$ is regular.
  \end{enumerate}
\end{proposition}

\begin{proof}
  Claim~(iii), already utilised in \hyperref[II.1.12]{(II.1.12)}, follows for example from the calculation in \hyperref[II.1.10]{(II.1.10)} and from the fact that the inverse image of a differential form presenting a simple pole again presents a simple pole.

\oldpage{52}
  Claim~(ii) follows immediately from criterion~(i) in \hyperref[II.1.12]{(II.1.12)}.
  ...
  After an eventual extension of scalars, we choose bases $e'$ and $e''$ of $V'$ and $V''$ (respectively) satisfying (i) or (ii) in \hyperref[II.1.12]{(II.1.12)}.
  Lift $e''$ to a family of vectors $e''_0$ of $V$.
  For large enough $N$, the basis $e'\cup t^{-N}e''_0$ of $V$ will satisfy (i) in \hyperref[II.1.12]{(II.1.12)} if $e'$ and $e''$ satisfy (i) in \hyperref[II.1.12]{(II.1.12)}, and will satisfy (ii) in \hyperref[II.1.12]{(II.1.12)} in the contrary case.
\end{proof}
...
\label{II.1.14}
  ...
  Such a bundle defines a vector space $V_K$ over the field of fractions $K$ of the local ring $\scr{O}_{p,S}$.
  ...
  Such a connection defines a connection \hyperref[II.1.2]{(II.1.2)} on $V_K$ (cf. \hyperref[II.1.7.2]{(II.1.7.2)}).
  We say that a connection $\nabla$ on $V$ is \emph{regular} at $p$ if it is meromorphic at $0$ and if the induced connection on $V_K$ is regular in the sense of \hyperref[II.1.11]{(II.1.11)}, i.e. if there exists a basis of $V$ close to $p$ in which the matrix of the connection presents at most a simple pole at $p$ \hyperref[II.1.12]{(II.1.12)}.
\end{env}
...
\begin{theorem}{1.17}
\label{II.1.17}
  Under the hypotheses of \hyperref[II.1.16]{(II.1.16)}, the monodromy transformation $T$ extends to an automorphism of $V$ whose fibre at $0$ is given by
  \[
