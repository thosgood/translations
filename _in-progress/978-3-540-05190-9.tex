...
\oldpage{6}
  ...
  \begin{env}{2.2.2}
  \label{I.2.2.2}
    If $Y$ is a subspace of $X$, then the \emph{$n$\textsuperscript{th} infinitesimal neighbourhood} of $Y$ in $X$ is the subspace of $X$ defined locally by the $(n+1)$th power of the ideal of $\sh{O}_X$ that defines $Y$.
  \end{env}
...
\label{I.2.24}
  ...
  \[
    \varinjlim_{U\supset Y} \HH^\bullet(U,\sh{F}) \simto \HH^\bullet(Y,\sh{F}).
  \]
\end{reminder}
...
\label{I.2.28}
  ...
  \begin{enumerate}[a)]
    ...
    \item the fibres of $f$ satisfy
      ...
      \[
        \sh{O}_S \otimes_\CC \RR^i f_*V \simto \RR^i f_*(\Omega_{X/S}^\bullet(V)).
      \]
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  \[
  \label{I.2.28.1}
    \HH^0(T,\sh{O}_S) \otimes \HH^i(T\times Y,\pr_2^\sbullet V_0) \simto \HH^i(T\times Y,\pr_1^\sbullet\sh{O}_S \otimes \pr_2^\sbullet V_0)
  \tag{2.28.1}
  \]
  are isomorphisms.
...
\oldpage{21}
  \[
  \label{I.2.28.2}
    \HH^0(T,\sh{O}_S) \otimes \HH^i(Y,V_0) \simto \HH^i(T\times Y,\pr_1^\sbullet\sh{O}_S \otimes \pr_2^\sbullet V_0).
  \tag{2.28.2}
  \]
...
\label{I.3.1}
  ...
  The choice of a basis $e\colon\CC^n\simto\sh{V}$ of $\sh{V}$ allows us to think of $\sh{V}$ as being defined by a
\oldpage{22}
  ...
  If $\omega_e$ is the matrix of the connection $\nabla$ in the basis $e$, and if $f\colon\CC^n\to\simto\sh{V}$ is another basis of $\sh{V}$, with matrix $A\in\GL_n(\sh{O})$ (where $A=e^{-1}f$), then \cref{I.3.1.2}
  \[
    \begin{aligned}
      \nabla v
...
\label{I.4.5}
  ...
  \begin{enumerate}[(i)]
    ...
    \item the canonical arrow $\DD^{n-1}\colon\sh{L}^E \to P^{n-1}(\sh{L})$ induces an isomorphism
      \[
        \sh{O}\otimes_\CC\sh{L}^E \simto P^{n-1}(\sh{L}).
      \]
  \end{enumerate}
\end{theorem}
...
\begin{proof}
  ...
  \[
    \Hom_\sh{O}(\sh{V},\sh{M}) \simto \Hom_\CC(V,\sh{M}).
  \]
...
\label{I.5.11}
  ...
  we have \cref{I.4.5} that $\sh{O}\otimes V\simto P^1(\sh{L})$, and the linear form $\lambda\colon P^1(\sh{L})\to\sh{L}$ defines a section $\lambda_0$ of the projective bundle associated to the vector bundle $P^1(\sh{L})$.
...
\begin{proof}
  ...
  Since $\sh{O}$ is complete, $\dd\colon\fk{m}\to\Omega$ is surjective, and $\Ker(\dd)\simto k$.
  This gives us a field of representatives that is annihilated by $\dd$, and the choice of a uniformiser $t$ gives the desired isomorphism $k[[t]]\simto\sh{O}$.
\end{proof}

\begin{env}{1.6}
\label{II.1.6}
  If an $\sh{O}$-algebra $\sh{O}'$ is a discrete valuation ring with a field of fractions $K'$ that is algebraic over $K$, then the derivation $\dd$ extends uniquely to $\dd\colon K'\to\Omega\otimes_{\sh{O}}K'$.
  Let $e$ be the ramification index of $\sh{O}'$ over $\sh{O}$, and let $t'$ be a uniformiser of $\sh{O}'$.
  We set
  \[
    \Omega' = 1/(t')^{e-1}\Omega\otimes_{\sh{O}}\sh{O}'.
  \]
\end{env}

We can easily show, using \cref{II.1.6}, that the triple $(\sh{O}',\dd,\Omega')$ again satisfies \cref{II.1.4.1}.
...
\begin{env}{1.7}
  ...
  \begin{enumerate}
    \item[(1.7.1)]\label{II.1.7.1}
      $\sh{O}=\sh{O}_{x,X}$ (the local ring for the Zariski topology), $\Omega=(\Omega_{X/\CC}^1)_x$, and $\dd=\mbox{the differential}$ ;
    \item[(1.7.2)]\label{II.1.7.2}
      $\sh{O}=\sh{O}_{x,X^\an}$ (the local ring of germs at $x$ of holomorphic functions), $\Omega=(\Omega_{X^\an/\CC}^1)_{(x)}$, and $\dd=\mbox{the differential}$ ; or
...
\begin{env}{1.8}
\label{II.1.8}
  Under the hypotheses of \cref{II.1.4}, let $V$ be a finite-dimensional vector space over $K$, and $V_0$ a lattice in $V$, i.e. a free sub-$\sh{O}$-module of $V$ such that $KV_0=V$.
  For every homomorphism $e\colon\sh{O}^n\to V$, we define the \emph{valuation $v(e)$ of $e$} to be the largest integer $m$ such that $e(\sh{O}^n)\subset\fk{m}^m V_0$.
...
\begin{proof}
  Let $N$ be an integer such that the $r_iN$ are integers, and set $\sh{O}'=\sh{O}(\sqrt[N]{t})$.
...
\oldpage{48}
  ...
  On $\sh{O}'$, let $\omega'$ be the basis of $\Omega\otimes K'$ given by the inverse image of $\omega$, let $\tau'$ be the corresponding basis of ${\Omega'}^\vee\otimes K'$, and let $e'=e\Lambda$ be a new basis of $V'=V\otimes K'$.
  In these bases, the connection matrix is
  \[
    \Gamma' = \Lambda^{-1}\Gamma\Lambda + \Lambda^{-1}\partial_{\tau'}\Lambda.
  \]

  The matrix $\Lambda^{-1}\partial_{\tau'}\Lambda$ has coefficients in $\sh{O}'$, and so either
  \begin{enumerate}[(a)]
    \item $s\leq0$, and $\Gamma'$ has coefficients in $\sh{O}'$ ; or
  ...
  This satisfies \cref{II.1.9.2} on $\sh{O}'$ (for suitable bases), and \cref{II.1.9.6} then follows from \cref{II.1.9.3}.
\end{proof}
...
\label{II.1.13}
  \begin{enumerate}[(i)]
    ...
    \item If the connections on $V_1$ and $V_2$ are regular, then the natural connections of
      ...
      \]
      are regular.
    \item If $\sh{O}'$ is a discrete valuation ring with field of fractions $k'$ that is algebraic over the field of fractions $K$ of $\sh{O}$, and if $V'=V\otimes_K K'$, then the connection on $V'$ is regular if and only if the connection on $V$ is regular.
  \end{enumerate}
\end{proposition}
...
\label{II.1.14}
  ...
  Such a bundle defines a vector space $V_K$ over the field of fractions $K$ of the local ring $\sh{O}_{p,S}$.
