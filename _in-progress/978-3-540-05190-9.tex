...
\label{I.2.4}
  ...
  \]
  that satisfies the Leibniz identity (\hyperref[I.2.3.5]{(I.2.3.5)}) for local sections $f$ of $\cal{O}$ and $s$ of $\scr{V}$.
...
\label{I.2.7}
  ...
  \begin{rmenv}{2.7.4}
  \label{I.2.7.4}
    We define a connection on the dual $\scr{V}^\vee$ of $\scr{V}$ via \hyperref[I.2.7.3]{(I.2.7.3)} and the defining isomorphism $\scr{V}^\vee = \shHom(\scr{V},\cal{O})$.
    We have
    \[
...
\oldpage{21}
  ...
  By \hyperref[I.2.25]{(I.2.25)}, since $\HH^i(T,\cal{O}_S)=0$, we have that
  \[
...
\oldpage{22}
  ...
  If $\omega_e$ is the matrix of the connection $\nabla$ in the basis $e$, and if $f\colon\CC^n\to\simto\scr{V}$ is another basis of $\scr{V}$, with matrix $A\in\GL_n(\cal{O})$ (where $A=e^{-1}f$), then \hyperref[I.3.1.2]{(I.3.1.2)}
  \[
    \begin{aligned}
      \nabla v
...
\begin{proof}
  ...
  For every $\cal{O}$-module $\scr{M}$, we have \hyperref[I.2.17]{(I.2.17)}
  \[
...
\begin{rmenv}{5.3}
\label{I.5.3}
  By \hyperref[I.5.2]{(I.5.2)}, there exists exactly (up to isomorphism) one pair $(u,P)$ consisting of a projective line $P$ on $S$ (with structure group $\mathrm{PGL}_2(\cal{O}_S)$) and an $S$-immersion $u$ of $X_2$ into $P$.
...
\label{I.5.11}
  ...
  we have \hyperref[I.4.5]{(I.4.5)} that $\cal{O}\otimes V\simto P^1(\scr{L})$, and the linear form $\lambda\colon P^1(\scr{L})\to\scr{L}$ defines a section $\lambda_0$ of the projective bundle associated to the vector bundle $P^1(\scr{L})$.
...
\begin{proof}
  ...
  \]
  If we identify $P^1(\scr{L})$ with $\cal{O}^2$, then the matrix of the connection \hyperref[I.5.10]{(I.5.10)} defined by $E$ on $\wedge^2 P^1(\scr{L})\sim\cal{O}$ is then $-a(x)\tr M$, where $M$ is the matrix in \hyperref[I.4.8.1]{(I.4.8.1)}.
...
\oldpage{44}
  ...
  In particular, $\dd$ is continuous and extends to $\dd\colon\cal{O}^\wedge\to\Omega^\wedge$, and the triple $(\cal{O}^\wedge,\dd,\Omega^\wedge)$ again satisfies \hyperref[II.1.4.1]{(II.1.4.1)}.
\end{rmenv}
...
We can easily show, using \hyperref[II.1.6]{(II.1.6)}, that the triple $(\cal{O}',\dd,\Omega')$ again satisfies \hyperref[II.1.4.1]{(II.1.4.1)}.
...
\begin{rmenv}{1.8}
\label{II.1.8}
  Under the hypotheses of \hyperref[II.1.4]{(II.1.4)}, let $V$ be a finite-dimensional vector space over $K$, and $V_0$ a lattice in $V$, i.e. a free sub-$\cal{O}$-module of $V$ such that $KV_0=V$.
...
\oldpage{48}
  ...
  This satisfies \hyperref[II.1.9.2]{(II.1.9.2)} on $\cal{O}'$ (for suitable bases), and \hyperref[II.1.9.6]{(II.1.9.6)} then follows from \hyperref[II.1.9.3]{(II.1.9.3)}.
\end{proof}
