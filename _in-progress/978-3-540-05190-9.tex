...
\label{II.2.14}
  ...
  The ``norms'' in question will be continuous families of norms on the $\scr{V}_x$ ($x\in X^*$).
...
\label{II.2.19}
  ...
  We have
  \begin{enumerate}
    \item[{\rm(i\textsubscript{a})}] If $\|x\|$ is a norm on $X_2^*$ with respect to $X_2$, then $\|f(x)\|$ is a norm on $X_1^*$ with respect to $X_1$.
\oldpage{69}
    ...
    \item[{\rm(i\textsubscript{c})}] In particular, if {\rm(b)} is satisfied, then the norms on $X_1^*=X_2^*$ with respect to $X_1$ or $X_2$ agree.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item[{\rm(iii\textsubscript{c})}] In particular, if {\rm(b)} is satisfied, then the moderate norms on $\scr{V}_1=\scr{V}_2$ with respect to $X_1$ or $X_2$ agree.
  \end{enumerate}
\end{itenv}
...
\oldpage{70}
  Finally, if $\scr{T}_2$ is a semi-analytic triangulation of the pair $(X_2,Y_2)$, then the $f^{-1}(\sigma)$ ($\sigma\in\scr{T}_2$) form a locally finite set of semi-analytic subsets of $X_1$, and, by \hyperref[II.2.8.2]{2.8.2}, there exists, locally on $X_1$, a semi-analytic triangulation $\scr{T}_1$ of the pair $(X_1,Y_1)$ such that
  \[
