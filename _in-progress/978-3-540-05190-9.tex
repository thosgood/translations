...
\begin{rmenv}{5.1}
\label{II.5.1}
  Let $D$ be the open unit disc, $D^*=D\setminus\{0\}$, $X=D^{n+m}$, $Y_i=\pr_i^{-1}(\{0\})$, and $Y=\bigcup_{i=1}^n Y_i$.
...
\begin{proof}
  ...
  To prove that the extension pr$.
  Withoblem has only one solution, it suffices to prove this locally on $Y$ outside of a subset of codimension~$\geq2$ in $X the notation of \hyperref[II.5.1]{(5.1)}, this allows us to reduce to the case where $n=1$.
...
\oldpage{96}
  \item An extension of the form $\widetilde{\cal{V}}(\tau)$ has the property that, in a suitable basis of $\widetilde{\cal{V}}(\tau)$, the connection matrix takes the form
    \[
      \Gamma = \sum_i\Gamma_i\frac{\dd z_i}{z_i}
    \]
    where the $\Gamma_i$ are connection matrices that pairwise commute.
\end{enumerate}
\end{rmenv}
