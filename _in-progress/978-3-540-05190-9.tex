...
\label{II.2.19}
  ...
  \begin{enumerate}
    \item[{\rm(a)}] There exists a subset $K$ of $X_1$ that is proper over $X_2$ and such that $f(K_1)\supset\overline{X_2^*}$.
    \item[{\rm(b)}] $f$ is proper and induces an isomorphism $X_1^*\simto X_2^*$.
  \end{enumerate}
...
\begin{rmenv}{Remarks 2.20}
  \begin{enumerate}
    \item[{\rm(i)}] Let $X=D^{n+m}$ and $X^*=(D^*)^n\times D^m$;
    ...
    \item[{\rm(ii)}] Hironaka's resolution of singularities, along with (ii\textsubscript{c}) of \hyperref[II.2.19]{(2.19)}, allows us, in the general case, to make explicit the idea of vertical subsets, starting from the particular case of~(i).
  \end{enumerate}
\end{rmenv}
...
\oldpage{71}
  ...
  \begin{enumerate}
    \item[{\rm(i)}] A \emph{norm on $X^*$} is defined to be a norm on $(X^*)^\an$ that is adapted to $X^\an$ \hyperref[II.2.5]{(2.5)}.
    \item[{\rm(ii)}] A \emph{vertical subset of $\widetilde{X}^*$} is defined to be a subset of $\widetilde{X}^*$ that is vertical along $Y=X\setminus X^*$ \hyperref[II.2.9]{(2.9)}.
    \item[{\rm(iii)}] A \emph{moderate norm on $\scr{V}$} is defined to be a moderate norm on $\scr{V}^\an$ with respect to the meromorphic structure at infinity of $\scr{V}^\an$ \hyperref[II.2.16]{(2.16)}.
    \item[{\rm(iv)}] A continuous section $v$ of $\pi^*\scr{V}$ is said to be of \emph{moderate growth} if it is of moderate growth along $Y$ \hyperref[II.2.17]{(2.17)}.
  \end{enumerate}
\end{rmenv}
...
\label{III.1.3}
  ...
  Then the following conditions are equivalent:
  \begin{enumerate}
    \item[{\rm(i)}] $s$ is in the Nilsson class; and
    \item[{\rm(ii)}] the restriction of $s$ to every (locally closed) smooth algebraic curve along $X$ is in the Nilsson class.
  \end{enumerate}
\oldpage{123}
