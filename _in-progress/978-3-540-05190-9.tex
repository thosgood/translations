...
\label{II.2.3}
  ...
  \label{II.2.3.*}
    \begin{gathered}
      \mbox{For every compact subset $K$ of $X$, there exist constants $A,B>0$ such that}
    \\j_1^*g \leq Aj_2^*g \leq Bj_2^*g \leq Bj_1^*g
      \qquad\mbox{on $K\cap X^*$.}
    \end{gathered}
  \tag{*}
  \]
...
\oldpage{62}
    ...
    If the functions $d'_1(n)$ and $d''_1(n)$ are obtained by this procedure, then we have \todo\textbf{???}.
