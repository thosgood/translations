...
\begin{proof}
  ...
  By \hyperref[II.2.15.2]{(2.15.2)} and \hyperref[II.2.16]{(2.16)}, we can reduce to proving (iii\textsubscript{a}) in the trivial case where $\scr{V}=\cal{O}$.
...
\begin{proof}
  ...
  Let $u$ be the identity map from $\scr{V}_0|X^*$ to $\widetilde{\scr{V}}|X^*$.
  We need to show that, for every open subset $W\subset X$, for every linear form $w\in\HH^0(W,\scr{V}^\vee)$, and for every local section $e\in\HH^0(W,\scr{V}_0)$, the function $f=\langle w,u(e)\rangle$ is meromorphic along $Y\cap W$.
  ...
  To prove that \mbox{(v)$\implies$(ii)}, we can either use \hyperref[II.1.19]{(1.19)} and \hyperref[II.2.19]{(2.19)} to show that \mbox{(iii)$\implies$(ii)}, or we can note that condition~(ii) for $\scr{V}$ on $X^*\subset X$ is equivalent to condition~(ii) for $\pi^{-1}\scr{V}$ on $X^*\subset X_1$.
