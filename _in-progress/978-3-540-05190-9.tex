...
\label{I.2.22}
  ...
  We define, as in \hyperref[I.2.11]{(I.2.11)} and \hyperref[I.2.21]{(I.2.21)}, the \emph{curvature} $R\in\Omega_{X/S}^2(\shEnd(\scr{V}))$ of a relative connection.
  A relative connection is said to be \emph{integrable} if $R=0$, in which case we have the \emph{relative de Rham complex with values in $\scr{V}$}, denoted by $\Omega_{X/S}^\bullet(\scr{V})$, and defined as in \hyperref[I.2.15]{(I.2.15)} and \hyperref[I.2.21]{(I.2.21)}.
\end{rmenv}

The ``absolute'' statements \hyperref[I.2.17]{(I.2.17)}, \hyperref[I.2.18]{(I.2.18)}, and \hyperref[I.2.19]{(I.2.19)} have ``relative'' (i.e. ``with parameters'') analogues:
