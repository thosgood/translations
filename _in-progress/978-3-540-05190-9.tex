...
\begin{rmenv}{6.7}
\label{II.6.7}
  The complex $[j_*\Omega_X^\bullet(\scr{V})]^\an$ is exactly the sheaf of sections of $\Omega_{X^\an}^\bullet(\scr{V})$ that are meromorphic along $Y$;
  we denote this by $j_*^\mm\Omega_X^\bullet(\scr{V})$.
  The arrows in \hyperref[II.6.4.3]{(6.4.3)}, \hyperref[II.6.4.4]{(6.4.4)}, and \hyperref[II.6.6.1]{(6.6.1)} for $j_*^\mm\Omega_{X^\an}^\bullet(\scr{V})$ fit into a commutative diagram
  \[
  \label{II.6.7.1}
    \begin{tikzcd}
      \mathbb{H}^\bullet(\overline{X},j_*\Omega_X^\bullet(\scr{V}))
        \ar[r,"\sim"]
        \ar[r,swap,"\mbox{\hyperref[II.6.4.3]{(6.4.3)}}"]
        \ar[d,"\sim"]
        \ar[d,swap,"\mbox{\hyperref[II.6.6.1]{(6.6.1)}}"]
      & \mathbb{H}^\bullet(X,\Omega_X^\bullet(\scr{V}))
        \ar[dd,"\circlednumber{1}"]
    \\\mathbb{H}^\bullet(\overline{X},j_*^\mm\Omega_{X^\an}^\bullet(\scr{V}))
        \ar[d,swap,"\circlednumber{2}"]
    \\\mathbb{H}^\bullet(\overline{X}_\cl,j_*\Omega_{X^\an}^\bullet(\scr{V}))
        \ar[r,"\sim"]
        \ar[r,swap,"\mbox{\hyperref[II.6.4.4]{(6.4.4)}}"]
      & \mathbb{H}^\bullet(X_\cl,\Omega_{X^\an}^\bullet(\scr{V}))
      & \HH^\bullet(X_\cl,\scr{V})
        \ar[l,swap,"\sim"]
...
\label{II.6.8}
  ...
  \[
    j_*^\mm\Omega_{X^\an}^\bullet(\scr{V}) \to j_*\Omega_{X^\an}^\bullet(\scr{V})
  \]
...
Let $\scr{V}_0$ be an extension of the algebraic vector bundle $\scr{V}$ to a vector bundle on $X$ such that, with respect to $\scr{V}_0$, we have:

\begin{itenv}{6.8.1}
\label{II.6.8.1}
the connection on $\scr{V}$ presents at worst a logarithmic poly along $Y$, and the residues of the connection along the components of $Y$ never have positive integer eigenvalues.
\end{itenv}

We can take $\scr{V}_0$ to be the canonical extension \hyperref[II.5.6]{(5.6)} of $\scr{V}$.
By \hyperref[II.3.13]{(3.13)}, the complex
\[
  j_*^\mm\Omega_{X^\an}^\bullet(\scr{V}) / \Omega_{X^\an}^\bullet\langle Y\rangle(\scr{V}_0)
\]
is acyclic.
The claim of \hyperref[II.6.8]{(6.8)} is equivalent to the acyclicity of the analogous complex
\[
\label{II.6.8.2}
  j_*\Omega_{X^\an}^\bullet(\scr{V}) / \Omega_{X^\an}^\bullet\langle Y\rangle(\scr{V}_0).
\tag{6.8.2}
\]

If we take $\scr{V}_0$ to be an extension of the type $\scr{V}(\tau)$ (cf. \hyperref[II.5.4]{(5.4)}), then we can locally put $\scr{V}$ into a canonical form \hyperref[II.5.6]{(5.6.ii)}, and it then remains to prove the following lemma.
...
Let $X=D^{n+m}$, $X^*=(D^*)^n\times D^m$, $j\colon X^*\hookrightarrow X$, and $Y=X\setminus X^*$, and let $\Gamma$ be the connection on $\cal{O}^d=\scr{V}$ given by
\[
...
\[
  K = j_*\Omega_{X^*}^\bullet(\scr{V}) / \Omega_X^\bullet\langle Y\rangle(\scr{V})
\]
is acyclic at $0$.
\end{itenv}
...
\begin{proof}
  ...
  The complexes $\Omega_X^\bullet\langle Y\rangle(\scr{V})$ and $j_*\Omega_{X^*}^\bullet(\scr{V})$ are then morally tensor products of analogous complexes for $n+m=1$.
  ...
  \[
    i_0\colon \left(\Omega_X\langle0\rangle(\scr{V})\right)_0 \to \left(j_*\Omega_{X^*}(\scr{V})\right)_0
  \]
...
\oldpage{104}
  ...
  We will write a germ at $0$ of a section of $j_*\Omega_{X^*}^\bullet(\scr{V})$ (where $\scr{V}=\cal{O}$) in the form
  \[
  ...
  \[
    ...
    \end{aligned}
  \]
  These decompositions exhibit $j_*\Omega_{X^*}^\bullet(\scr{V})$ as an $(n+m)$-complex, of which $\Omega_X^*\langle Y\rangle(\scr{V})$ is an $(n+m)$-complex.
  ...
  \begin{enumerate}[(a)]
    ...
    \item $r$ is a retraction of $(j_*\Omega_{X^*}^\bullet(\scr{V}))_0$ to $(\Omega_X^\bullet\langle Y\rangle(\scr{V}))_0$.
  \end{enumerate}
  ...
  Since $1$ is homotopic to $r_i$, it is also homotopic to the composition $r$ of the $r_i$, and $r$ is an inverse up to homotopy of the inclusion morphism $i$ of $(\Omega_X^\bullet\langle Y\rangle(\scr{V}))_0$ into $j_*\Omega_{X^*}^\bullet(\scr{V})$.
  This finishes the proof of \hyperref[II.6.9]{(6.9)} and thus of \hyperref[II.6.2]{(6.2)}
\end{proof}

\begin{itenv}{Corollary 6.10}
\label{II.6.10}
  Let $X$ be a smooth scheme of finite type over $\CC$, $Y$ a normal crossing divisor on $X$, $\scr{V}$ a vector bundle on $X$, and $\Gamma$ a regular integrable connection on the restriction of $\scr{V}$ to $X^*=X\setminus Y$.
  ...
  Then, denoting by $V$ the local system defined by $\scr{V}$ on $X_\cl^*$, we have
  \[
    \mathbb{H}^\bullet(X,\Omega_X^\bullet\langle Y\rangle(\scr{V}))
    \simto \HH^\bullet(X_\cl^*,V).
  \]
\end{itenv}
...
\label{II.6.11}
  ...
  \[
    \HH^\bullet(\Gamma(X,\Omega_X^\bullet\langle Y\rangle(\scr{V})))
    \simto \HH^\bullet(X_\cl^*,V).
  \]
\end{itenv}

In other words, to calculate the cohomology of $X^*$ with coefficients in $V$, it suffices to consider the complex of algebraic differential forms on $X^*$ with coefficients in $\scr{V}$ that present only a simple logarithmic poly along $Y$.
