...
If $X$ is an algebraic variety over a number field $k$, $x\in X(k)$, and $N$ an integer, then we want to have a realisation system $\Lie\pi_1(X,x)^{(N)}_\mot$.
...
The \unsure{gradu\'{e}} for the weight filtration of $\Lie\pi_1(X,x)^{(N)}_\mot$ is the free Lie algebra on $H_1(X)_\mot$, modulo its $Z^{N+1}$ (decreasing central series).
$H_1(X)_\mot$ is the sum of two copies of the Tate motive $\QQ(1)$.
We thus deduce that $\Lie\pi_1(X,x)^{(N)}_\mot$ is an iterated extension of Tate motives $\QQ(n)$.
...
This conjecture places severe restrictions on $\Lie\pi_1(X,x)^{(N)}_\mot$, which are far from having been verified.
What we know concerns, up to now, only the quotient by the second derived group.
A large part of this article is dedicated to developing a language in which the consequences of the conjecture affecting $\Lie\pi_1(X,x)^{(N)}_\mot$ can be clearly stated.
