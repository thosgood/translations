...
\label{I.5.4}
  We define the \emph{osculating projective line bundle of $X$}, denoted by $P_\mathrm{tg}$, to be the osculating projective line bundle of $q_1\colon X_2\to X$.
\end{definition}
...
\tag{5.4.1}
\]
and, in particular, $P_\mathrm{tg}$ is endowed with a canonical section $e$, which is the image of the diagonal section of $X_2$, and we have that
\[
\label{I.5.4.2}
  e^*\Omega_{P_\mathrm{tg}/X}^1 \simeq \Omega_X^1.
\tag{5.4.2}
\]

\begin{env}{5.5}
\label{I.5.5}
  If $X$ is a projective line, then $\pr_1\colon X\times X\to X$ is a projective bundle on $X$ such that $P_\mathrm{tg}$ can be identified with the constant projective bundle of fibre $X$ on $X$ endowed with the inclusion homomorphism of $X_2$ into $X\times X$, i.e.
  \[
    \begin{tikzcd}
      X_2 \rar[hook] \drar[swap,"q_1"]
      & X\times X \dar["\pr_1"]
