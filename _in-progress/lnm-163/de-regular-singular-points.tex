...
\begin{proof}
  We first prove (i).
  To show that $\sh{V}$ is coherent, it suffices to do so locally, for $V=f^*V_0$, in which case $\sh{V}$ is the inverse image, in the sense of coherent analytic sheaves, of $\sh{V}_0$.
...
\label{I.2.23.2}
  ...
  \[
    0 \to \Gamma(f^*\sh{O}_S) \to \Gamma(\sh{O}_X) \to \Gamma(\Omega_{X/S}^1) \to \ldots
  \]
  is acyclic, since it admits the homotopy operator $H$ defined below.
\oldpage{16}
  \begin{enumerate}[a)]
    \item $H\colon \Gamma(\sh{O}_X) \to \Gamma(f^*\sh{O}_S) = \Gamma(S,\sh{O}_S)$ is the inverse image under the zero section of $f$;
  ...
  This remains true if we replace $D^{m+n}$ by a smaller polycylinder, and so $\Omega_{X/S}^\bullet$ is a resolution of $f^*\sh{O}_S$.
\end{env}
...
\label{I.2.23.3}
  ...
  \]
  then let $\Sigma=f^*\Sigma_0$ be the exact sequence of relative local systems given by the inverse image of $\Sigma_0$ (which is exact since $f^*$ is an exact functor), and let $\Omega_{X/S}^\bullet(\Sigma)$ be the corresponding exact sequence of relative de Rham complexes:
  \[
    \Omega_{X/S}^\bullet(\Sigma)\colon 0 \to \Omega_{X/S}^p\otimes_{f^*\sh{O}_S}f^*V'_0 \to \Omega_{X/S}^p\otimes_{f^*\sh{O}_S}f^*V_0 \to \Omega_{X/S}^p\otimes_{f^*\sh{O}_S}f^*V''_0 \to 0.
  \]
  This sequence is exact since $\Omega_{X/S}^p$ is flat over $f^*\sh{O}_S$, since it is locally free over $\sh{O}_X$ which is itself flat over $f^*\sh{O}_S$.

  The snake lemma applied to $\Omega_{X/S}^\bullet(\Sigma)$ shows that, if claim~(ii)
\oldpage{17}
  is satisfied for any two of relative local systems $f^*V_0$, $f^*V'_0$, and $f^*V''_0$, then it is again satisfied for the third.
