...
We can choose a subgroup of $\mathcal{C}$ with one parameter $\lambda$ (that is, an element of the dual lattice of $\mathfrak{X}(T)$) such that $\lambda$ is constant on $F$, equal to $a\in\ZZ$, and $\langle\lambda,p\rangle>a$ for all $p\in\mathcal{C}\setminus\langle F\rangle$.
...
\oldpage{516}
    ...
    \item $X^\lambda$ is a closed $T$-stable sub-variety of $X$, and $\II(X^\lambda)\subset\langle F\rangle$, and so $F$ is open in the convex hull of $\II(X^\lambda)$.
      ...
      The first claim thus follows from \cref{1.2}; the second is evident.
