...
If we now consider a point $(p,p',p'')$ of this connecting line, the corresponding quartic curve \hyperref[equation3]{(3)} has no double point.
Since this curve has no real points at all, any existing double point of the curve would have to be a point with complex coordinates; the imaginary point conjugate to this point would then be a second double point of the curve, which results in an obvious contradiction with the hypotheses made.
By following the course of the connecting line along the parameters $p$, $p'$, $p''$, we go from the definite form $F$ to the definite form $F'$ by continuously changing its real coefficients, without passing through a form with vanishing discriminant.
We now set the form $F$ equal to the sum of the squares of three real quadratic forms $\varphi$, $\psi$, $\chi$.
Then the coefficients of the forms $\varphi$, $\psi$, $\chi$ obviously remain real as long as the singular case considered before is excluded.
Therefore, if we carry out the continuous change from $F$ into $F'$ in the way described above, it necessarily also follows that form $F'$ admits the desired representation: as a sum of three squares of real forms.
Thus our theorem is proved.

However, the question suggested at the beginning of this paper reaches a satisfactory conclusion only through the rigorous justification of the following theorem.
