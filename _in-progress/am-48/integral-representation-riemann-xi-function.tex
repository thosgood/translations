...
\emph{We also thank Juan Arias de Reyna for their Spanish translation of this paper, which helped greatly.}
...
\oldpage{315}
    ...
    \begin{enumerate}[1)]
      \item For $y=y_n$ in \cref{28}, the leading term in \cref{28} dominates over the error term $\mathcal{O}$ in all of the segment $-1\leqslant x\leqslant 1$, and so $\GG(z)$ has no zeros on the horizontal sides of $R_n$, as we had previously claimed.
      \item Moving along an arc in this segment, the $e^{1\pi x/2}$ factor dominates in $\GG(z)$, or, more precisely, for arbitrary $\varepsilon$ we have
        \[
        \label{33}
          H_n < -\pi + \varepsilon
        \tag{33}
