...
\oldpage{310}
  ...
  \oldpage{311}
    ...
    For what follows, it is useful to have in mind a certain partition of the plane (cf. the above figure): the strip $-1\leq x\leq 1$ is denoted by $S$; the part of the plane where both $r\leq R$ and $|x|>1$ is denoted by $T$; the remaining part, which extends to infinity, is denoted by $U$.
    We pick the number $R=R(A)$ depending on a certain positive number $A$ such that, inside, and on the boundary of, $U$, the inequality $|\psi(z)|<1$ is satisfied for $a\leq A$.
    From \cref{18} it follows that $\GG(z)$ does not vanish inside, or on the boundary of, $U$.
\end{enumerate}
...
\begin{enumerate}[I.]
\setcounter{enumi}{3}
  \item \emph{$\GG(z)$ has no zeros on the straight lines $x=1$ or $x=-1$, i.e. on the boundary of $S$.}
