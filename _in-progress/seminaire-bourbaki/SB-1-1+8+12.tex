...
Let $V$ denote the manifold of a Lie group $G$, and denote by $\mathfrak{a}$ the subspace of $E$ given by vector fields that are \emph{invariant under left-translations} by $G$;
we denote by $\mathfrak{a}'$ the subspace of $E'$ given by \emph{left-invariant} differential forms (of degree~$1$).
Then $\mathfrak{a}$ and $\mathfrak{a}'$ are $n$-dimensional vector spaces over the field $\RR$ of reals, and are in duality.
The exterior algebra $\Lambda(\mathfrak{a}')$ can be identified with the sub-algebra of $\Lambda(E')$ given by \emph{left-invariant} exterior differential forms, and it is \emph{stable} under the operator $\dd$ of exterior differentiation.
(In particular, $\Lambda^0(\mathfrak{a}')$ is the field of constant functions, identified with $\RR$).

If $X\in\mathfrak{a}$, then the automorphism group of $V$ defined by $X$ (cf. \cref{I.4}) is the group of \emph{right}-translations by elements of the subgroup of $G$;
the orbit of the identity element.
Thus $\Lambda(\mathfrak{a})$ and $\Lambda(\mathfrak{a}')$ are \emph{stable} under $\theta(X)$ if $X\in\mathfrak{a}$.
In particular, if $X$ and $Y$ are in $\mathfrak{a}$, then $[X,Y]$ is in $\mathfrak{a}$.
The elements upon which $\theta(X)$ acts trivially (for \emph{all} $X\in\mathfrak{a}$) are those that are simultaneously invariant under left- and right-translations, and are simply called \emph{invariant elements}.
...
\label{equation-I.3}
  ...
  = 0
  \qquad\mbox{for $Y\in\mathfrak{a}$ and $\omega\in\mathfrak{a}'$.}
\tag{3}
\]
In other words, $\theta(X)$ (acting on forms) is the \emph{transpose} of $-\theta(X)$ (acting on vector fields).

The vector space $\mathfrak{a}$, endowed with the structure defined by the map $(X,Y)\mapsto[X,Y]$ from $\mathfrak{a}\times\mathfrak{a}$ to $\mathfrak{a}$, is the \emph{Lie algebra} of the group $G$.


\section{Lie algebras}
\label{I.6}

We start with an abstract Lie algebra $\mathfrak{a}$ (under the classical definition), taken over a field $K$ that is, for now, arbitrary.
Let $n$ be the dimension of the vector space $\mathfrak{a}$ over $K$.
We will construct everything from the structure of $\mathfrak{a}$, by taking the relations established above (in the case of the field $\RR$) as our definitions.

\oldpage{11}
Let $\mathfrak{a}'$ be the dual vector space of $\mathfrak{a}$.
From now on, we denote elements of $\mathfrak{a}$ by $x,y,\ldots$;
the elements of $\mathfrak{a}'$ by $x',y',\ldots$;
the elements of $\Lambda(\mathfrak{a})$ by $\alpha,\beta,\ldots$;
and the elements of $\Lambda(\mathfrak{a}')$ by $\alpha',\beta',\ldots$.
We \emph{define} $\theta(x)$, for $x\in\mathfrak{a}$, by
\[
  \begin{aligned}
    \theta(x)\cdot y
    &= [x,y];
  ...
  \end{aligned}
\]
With $\theta(x)$ defined on $\mathfrak{a}$ and $\mathfrak{a}'$, we extend it to $\Lambda(\mathfrak{a})$ and $\Lambda(\mathfrak{a}')$ by imposing the condition that $\theta(x)$ be a \emph{derivation}.
The $\theta(x)$ that acts on $\Lambda(\mathfrak{a})$ is the \emph{transpose} of the $-\theta(x)$ that acts on $\Lambda(\mathfrak{a}')$.
The elements of $\Lambda(\mathfrak{a})$ (resp. of $\Lambda(\mathfrak{a}')$) for which $\theta(x)$ acts trivially (for \emph{all} $x\in\mathfrak{a}$) are called \emph{invariant} (or \emph{bi-invariant}) elements.
The invariant elements of $\Lambda(\mathfrak{a})$ form a \emph{sub-algebra} $\mathfrak{J}$, and those of $\Lambda(\mathfrak{a}')$ form a sub-algebra $\mathfrak{J}'$.

Equation~\cref{equation-I.2} leads us to define an endomorphism $\delta$ of $\Lambda(\mathfrak{a}')$ that is zero on the scalars, and such that
\[
\label{equation-I.4}
...
\[
  ...
  = -\langle[x,y],x'\rangle,
\]
and it maps $\Lambda^p(\mathfrak{a}')$ to $\Lambda^{p+1}(\mathfrak{a}')$.

We define, on $\Lambda(\mathfrak{a})$, the endomorphism $\partial$ that is the \emph{transpose} of $-\delta$ by
\[
  \langle\partial\alpha,\alpha'\rangle
  = -\langle\alpha,\delta\alpha'\rangle.
\]
Then $\partial$ commutes with the $\theta(x)$, satisfies $\partial\partial=0$, maps $\Lambda^p(\mathfrak{a})$ to $\Lambda^{p+1}(\mathfrak{a})$, and is \emph{zero on $\mathfrak{a}$};
...
The operator $\delta$ on $\Lambda(\mathfrak{a}')$ (the \emph{algebra of cochains}) defines a \emph{cohomology algebra}, denoted by $\HH(\mathfrak{a}')$.
The operator $\partial$ on $\Lambda(\mathfrak{a})$ (the \emph{algebra of chains}) defines a \emph{homology group}, denoted by $\HH(\mathfrak{a})$;
there is not, in general, a multiplication in $\HH(\mathfrak{a})$, since $\partial$ is \emph{not} an antiderivation.

We have that $\HH(\mathfrak{a})$ and $\HH(\mathfrak{a}')$ are naturally in duality;
for each degree~$p$, $\HH^p(\mathfrak{a})$ and $\HH^p(\mathfrak{a}')$ are in duality, and thus of the same dimension.
If $\mathfrak{a}$ is the Lie algebra of a \emph{compact connected group}, then $\HH(\mathfrak{a}')$ can be identified with the cohomology algebra of the (topological) space of the group, by the de Rham theorem.
This proves that
\oldpage{12}
any two compact connected groups that are locally isomorphic have the same Betti number.
In all cases, the common dimension of $\HH^p(\mathfrak{a})$ and $\HH^p(\mathfrak{a}')$ is called the \emph{$p$-th Betti number} of the Lie algebra $\mathfrak{a}$.

\medskip
\textbf{Unimodularity.}
We say that $\mathfrak{a}$ is \emph{unimodular} if $\theta(x)$ (for \emph{all} $x\in\mathfrak{a}$), considered as an endomorphism of $\mathfrak{a}$ (the \emph{adjoint} representation) has \emph{zero trace};
...
If $\mathfrak{a}$ is unimodular, then $\alpha\mapsto\omega\llp\alpha'$ defines an isomorphism from $\HH^p(\mathfrak{a}')$ to $\HH^{n-p}(\mathfrak{a})$;
...
\[
  ...
  \\2\partial
    ...
    \qquad\mbox{if $\mathfrak{a}$ is \emph{unimodular}.}
  \end{aligned}
\]
...
If $\mathfrak{a}/\mathfrak{c}$ is semi-simple (where $\mathfrak{c}$ is the centre of $\mathfrak{a}$), then we have a canonical isomorphism from $\HH(\mathfrak{a}')$ to $\mathfrak{J}'$, and from $\HH(\mathfrak{a})$ to $\mathfrak{J}$ (and thus a multiplicative structure on $\HH(\mathfrak{a})$ in this case).
...
\textbf{Corollary.}
If $K$ is maximal and quasi-real, and $\mathfrak{a}$ is \emph{simple}, then the third Betti number is equal to $1$.
...
\[
  \begin{array}{rll}
    \mbox{$\mathfrak{a}$:} & \mbox{Lie algebra over a field $K$;} & \mbox{elements $x,y,\ldots$}
  \\\mbox{$\mathfrak{a}'$:} & \mbox{dual vector space of $\mathfrak{a}$;} & \mbox{elements $x',y',\ldots$}
  \\\mbox{$\Lambda(\mathfrak{a})$:} & \mbox{exterior algebra of $\mathfrak{a}$;} & \mbox{elements $\alpha,\beta,\ldots$}
  \\\mbox{$\Lambda(\mathfrak{a}')$:} & \mbox{exterior algebra of $\mathfrak{a}'$;} & \mbox{elements $\alpha',\beta',\ldots$}
  \end{array}
\]

\begin{itemize}
  \item The operator $\theta(x)$ is the \emph{infinitesimal transformation defined by $x\in\mathfrak{a}$} on $\Lambda(\mathfrak{a})$;
    from now on, we denote by $\theta^*(x)$ (instead of $\theta(x)$) the operator that is the transpose of $-\theta(x)$ acting on $\Lambda(\mathfrak{a}')$.
  \item  The operator $e(x)$ is the \emph{(left) exterior product with $x\in\mathfrak{a}$} on $\Lambda(\mathfrak{a})$;
    the operator $i(x)$ is the \emph{interior product with $x\in\mathfrak{a}$} on $\Lambda(\mathfrak{a}')$;
    $i(x)$ and $e(x)$ are the transpose of one another.
  \item The operator $\partial$ is the \emph{boundary} operator on $\Lambda(\mathfrak{a})$;
    the operator $\delta$ is the \emph{coboundary} operator on $\Lambda(\mathfrak{a}')$;
...
The \emph{homology vector space $\HH(\mathfrak{a})$} is defined by $\partial$ acting on $\Lambda(\mathfrak{a})$;
the \emph{cohomology algebra $\HH(\mathfrak{a}')$} is defined by $\delta$ acting on $\Lambda(\mathfrak{a'})$.
There is a multiplicative structure on $\HH(\mathfrak{a}')$ because $\delta$ is an \emph{antiderivation}.
The vector spaces $\HH(\mathfrak{a})$ and $\HH(\mathfrak{a}')$ are canonically in duality.


\section{Direct composition of two Lie algebras}
\label{II.2}
