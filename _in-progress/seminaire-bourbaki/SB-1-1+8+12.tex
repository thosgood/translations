...
The elements upon which $\theta(X)$ acts trivially (for \emph{all} $X\in\fk{a}$) are those that are simultaneously invariant under left- and right-translations, and are simply called \emph{invariant elements}.
...
The elements of $\Lambda(\fk{a})$ (resp. of $\Lambda(\fk{a}')$) for which $\theta(x)$ acts trivially (for \emph{all} $x\in\fk{a}$) are called \emph{invariant} (or \emph{bi-invariant}) elements.
