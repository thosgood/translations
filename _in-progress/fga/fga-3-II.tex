...
\begin{enumerate}[(a)]
  ...
  \item For all $x\in\fk{X}$, we have $\sh{G}_x=G(\Spec(\widehat{\sh{O}}_{\fk{X},x}))$ (i.e. isomorphism classes of finite free algebras $B$ over $\widehat{\sh{O}}_{\fk{X},x}$ endowed with an isomorphism from $B\otimes_{\widehat{\sh{O}}_{\fk{X},x}}\sh{O}_{X_0,x}$ to $(\sh{O}'_0)_x$, where $\sh{O}'_0$ is the sheaf of algebras on $X_0$ that defines $X'_0$).
