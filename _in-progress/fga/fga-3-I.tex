...
\begin{definition}{2.4}
  A morphism $\alpha\colon S'\to S$ is said to be a \emph{descent morphism} (resp. a \emph{strict descent morphism}) if it is an $\sh{F}$-descent morphism (resp. a strict $\sh{F}$-descent morphism) (cf. \cref{definition:A.1.7}), where $\sh{F}$ denotes the fibred category over $\cat{C}$ of objects of $\cat{C}$ over objects of $\cat{C}$ (\cref{example:A.1.1(1)}).
\end{definition}
...
\label{theorem:B.1(1)}
  ...
  Then $\alpha$ is a \emph{strict descent morphism} (cf. \hyperref[definition:A.1.7]{A.1, Definition~1.7}) for the fibred category $\sh{F}$ of quasi-coherent sheaves (cf. \hyperref[example:A.1.1(2)]{A.1, Example~2}).
\end{theorem}
...
\begin{enumerate}[(i)]
  ...
  \item Every quasi-coherent sheaf $\sh{F}'$ on $S'$ endowed with a descent data with respect to the morphism $\alpha\colon S'\to S$ (cf. \hyperref[definition:A.1.6]{A.1, Definition~1.6}) is isomorphic (endowed with this data) to the inverse image of a quasi-coherent sheaf $\sh{F}$ on $S$.
\end{enumerate}
...
\oldpage{190-18}
  \[
    ...
    \end{tikzcd}
  \]
  of maps of sets is \emph{exact} (cf. \cref[definition:A.1.1]{A.1, Definition~1.1}).
\end{corollary}

Also, the combination of (i) and (ii) with \cref[definition:A.1.1]{A.1, Definition~1.1} gives:

\begin{corollary}{2}
\label{corollary:B.1(2)}
  Let $\sh{G}$ be as in \hyperref{corollary:B.1(1)}.
