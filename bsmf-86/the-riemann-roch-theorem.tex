...
\begin{abstract}
  ...
  \emph{Bulletin de la Soci\'{e}t\'{e} Math\'{e}matique de France}, Volume~\textbf{86} (1958) , pp.~97--136.
...
\begin{proposition}{3}
\label{proposition3}
  For a morphism $f\colon X\to Y$ to be proper, it is necessary and sufficient that for every variety $Z$ and every closed subset $T$ of $X\times Z$, the image of $T$ in $Y\times Z$ is closed.
\end{proposition}
...
We recall only that a cycle $Z$ on $X$ of codimension $p$ [i.e. a degree-$p$ element of $A(X)$] is said to be linearly equivalent to zero if there exists a cycle $H$ on $X\times\mathbf{D}$ (where $\mathbf{D}$ denotes the projective line) such that $Z=H_a-H_b$ for two points $a,b\in\mathbf{D}$;
...
Then
\[
  \ch(x+y) = \ch(x)+\ch(y)\quad\text{and}\quad
...
\[
  c_p(f^!(x)) = f^*(c_p(x))\quad\text{and}\quad
  \ch(f^!(x)) = f^*(\ch(x)),
\]
for $x\in K(X)$.
