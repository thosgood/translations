...
\begin{enumerate}
  \item[(V1)] If $X\in\cat{V}$, and if $E$ is a vector bundle on $X$, then $\mathbb{P}(E)\in\cat{V}$.
\end{enumerate}

Suppose further that we have the following data:
\begin{enumerate}[a.]
  \item a contravariant functor $X\mapsto A(X)$ from $\cat{V}$ to the category of unital anticommutative graded rings;
  \item a functorial homomorphism $p_X\colon\PP(X)\to A^2(X)$ (for $X\in\cat{V}$);
  \item for all $X\in\cat{V}$, and for every closed algebraic subspace $Y$ of $X$, of constant codimension~$p$ in $X$, with $Y\in\cat{V}$, a group homomorphism
    \[
      i_*\colon A(Y)\to A(X)
    \]
...
\begin{enumerate}[({A}1)]
  \item Let $X\in\cat{V}$, and let $E$ be a rank-$p$ vector bundle on $X$, $\mathbb{P}(E)$ the associated projective bundle, and $\xi_E$ the element of $A(\mathbb{P}(E))$ defined by
    \[
      \xi_E = p(\cl(L_E)).
    \]
  ...
  \item Let $X\in\cat{V}$, and let $L$ be a rank-$1$ vector bundle on $X$, and $s$ a regular section of $L$ that is transversal to the zero section, and such that the space $Y$ of zeros of $s$ belongs to $\cat{V}$.
    Then
    \[
      p_X(Y) = p_X(\cl_X(L)).
    \]
  \item Let $X,Y,Z\in\cat{V}$, with $Z\subset Y\subset X$, and let $i$ and $j$ be the injection morphisms $Z\to Y$ and $Y\to X$ (respectively).
    Then $(ji)_* = j_*i_*$.
  \item Let $X,Y\in\cat{V}$, with $Y\subset X$, and let $i$ be the injection morphism of $Y$
\oldpage{141}
  into $X$.
    Then
    \[
